...
content = content.replace(/const treatmentProcess = \[[\\s\\S]*?\];/m, 'const treatmentProcess = ' + treatmentProcess + ';');
...
content = content.replace(/const programs = \[[\\s\\S]*?    }\n  \];/m, 'const programs = ' + programs + ';');
...
content = content.replace(/const whyChooseUs = \[[\\s\\S]*?    }\n  \];/m, 'const whyChooseUs = ' + whyChooseUs + ';');
...
content = content.replace(/const faqData = \[[\\s\\S]*?    }\n  \];/m, 'const faqData = ' + faqData + ';');

// Fix paragraph descriptions
content = content.replace(/Guests usually choose this center for structured consultation-led care, personalized therapy plans, and sustainable follow-up lifestyle guidance for long-term wellness outcomes./g, "Travelers often choose Goa Sian Spa for its unique fusion of authentic Ayurveda and Asian massage therapies, perfect for releasing deep-seated tension, travel fatigue, and mental stress in the vibrant, spiritual setting of Arambol, Goa.");

...
import { Activity, Award, Brain, Building2, Calendar, ChevronLeft, ChevronRight, ClipboardList, Droplet, FileSearch, Globe, Heart, HeartPulse, Home, Hospital, Images, Leaf, MapPin, MessageCircle, MessageCircleHeart, Phone, Pill, Search, ShieldCheck, Sparkles, Star, Stethoscope, TrendingUp, TreePine, Users, Utensils, Video, X } from "lucide-react";
import MarkdownContent from "@/components/MarkdownContent";
import { Card, CardContent } from "@/components/ui/card";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";

export default function TheNattikaBeachResort() {
  ...
  const founderImage = "/Center Images/The Nattika Beach Resort/Founder/Founder.jpg";
  const teamImage = "/Center Images/The Nattika Beach Resort/Founder/Team.jpg";
  ...
  const [isTestimonialVideoInView, setIsTestimonialVideoInView] = useState(false);
  const videoGalleryRef = useRef<HTMLVideoElement | null>(null);
  const testimonialIframeRef = useRef<HTMLIFrameElement | null>(null);

  const awards = [
    {
      title: "Kerala Tourism Green Leaf Certification",
      description: "Kerala Tourism’s Green Leaf rating recognizes authentic Ayurveda centers that meet rigorous treatment, hygiene, and physician-led care standards.",
      image: "/Center Images/The Nattika Beach Resort/Awards/Award 2 (Green Leaf Certification ayueveda).png"
    },
    {
      title: "Condé Nast Traveler Recognition",
      description: "Condé Nast Traveler highlights exceptional destinations through its editorial awards and curated lists for standout guest experiences.",
      image: "/Center Images/The Nattika Beach Resort/Awards/Award 3 (Condé Nast Traveller).png"
    },
    {
      title: "NABH Accredited",
      description: "NABH accreditation reflects compliance with nationally recognized standards for patient safety and quality of care.",
      image: "/Center Images/The Nattika Beach Resort/Awards/Awards 1 (NABH).webp"
    },
    {
      title: "IGBC Platinum Rating",
      description: "IGBC Platinum is a top-tier green building rating for sustainable design, energy efficiency, and responsible operations.",
      image: "/Center Images/The Nattika Beach Resort/Awards/Award 4 (IGBC Platinum Rating).jfif"
    }
  ];
  ...
  useEffect(() => {
    fetch(encodeURI("/Center Images/The Nattika Beach Resort/Photo Gallery/Photo Gallery Links.txt"))
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Nattika photo gallery:", err));

    fetch(encodeURI("/Center Videos/The Nattika Beach Resort/Video Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
        setVideos(lines);
      ...
      .catch((err) => console.error("Error loading Nattika video gallery:", err));

    fetch(encodeURI("/Center Videos/The Nattika Beach Resort/Testimonies/Testimonies Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      ...
      .catch((err) => console.error("Error loading Nattika testimonial videos:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/The Nattika Beach Resort/Wellness & Rejuvenation Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Nattika wellness content:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Medical Treatment Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Nattika medical content:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Why Choose The Nattika Beach Resort.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Nattika why-choose content:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Nattika treatment process content:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
      ...
      .catch((err) => console.error("Error loading Nattika facilities content:", err));

    fetch(encodeURI("/Center Images/The Nattika Beach Resort/Facilities/Facilities Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      ...
      .catch((err) => console.error("Error loading Nattika facilities images:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("### ")) {
            ...
            if (/expert medical team|expert healing team/i.test(t)) { if (currentGroup) groups.push(currentGroup); currentGroup = { title: t, description: "", items: [] }; section = "group"; continue; }
            if (/^dr\b/i.test(t)) { name = t; section = "founder"; continue; }
            if (/founder\s*&\s*team/i.test(t)) continue;
          }
          if (line.startsWith("**") && line.endsWith("**")) {
            const t = line.slice(2, -2);
            if (/leadership|expertise/i.test(t)) { section = "expertise"; continue; }
            if (/our founder|team info/i.test(t)) continue;
            ...
            if (!name) { name = t; section = "founder"; continue; }
          ...
          if (section === "founder") {
            if (/MD|BAMS|Ayurveda|MRCH|Hom|MBA|MSc|PhD/i.test(line) && line.length <= 70) { degrees.push(line); continue; }
            if (/CEO|Chief|Director|Physician|Managing Director|Owner/i.test(line) && role === "") { role = line; continue; }
            fdesc = fdesc ? `${fdesc} ${line}` : line;
            continue;
          }
          if (section === "group" && currentGroup) { currentGroup.description = currentGroup.description ? `${currentGroup.description} ${line}` : line; continue; }
        }
        if (currentGroup) groups.push(currentGroup);
        setTeamIntro(intro);
      ...
      .catch((err) => console.error("Error loading Nattika founder content:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Nattika reviews:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Nattika insurance content:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Nattika FAQs:", err));

    fetch("/content/Top Centers/The Nattika Beach Resort/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        let website = "";
        let transport = "";
        let section: "none" | "address" | "distance" | "transport" | "phone" | "email" | "website" = "none";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (t.includes("phone") || t.includes("contact number")) { section = "phone"; continue; }
            if (t.includes("email")) { section = "email"; continue; }
            if (t.includes("website")) { section = "website"; continue; }
          }
          if (line.startsWith("*")) {
            const bullet = line.replace(/^\*+\s*/, "");
            if (section === "distance") distances.push(bullet);
            else if (section === "phone") phones.push(bullet);
            else if (section === "email") emails.push(bullet);
            continue;
          }
          if (section === "address") address.push(line);
          else if (section === "website" && !website) website = line;
          else if (section === "transport") transport = transport ? `${transport} ${line}` : line;
        }
      ...
      .catch((err) => console.error("Error loading Nattika contact info:", err));
  }, []);
  ...
  const wellnessIconForTitle = (t: string) => {
    ...
    if (s.includes("rejuvenation")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("detox")) return <Droplet className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("stress") || s.includes("burnout")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("beauty")) return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("yoga")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("couple")) return <Users className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("weekend") || s.includes("beach")) return <MapPin className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("healthy living")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("digital")) return <Globe className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("women")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("aging") || s.includes("longevity")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("mindful") || s.includes("meditation")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    return <Home className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  };
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("arthritis") || s.includes("joint")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("spinal") || s.includes("back") || s.includes("neck")) return <Hospital className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("psoriasis") || s.includes("skin")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("diabetes")) return <Heart className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("stress") || s.includes("anxiety") || s.includes("sleep")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("post-stroke") || s.includes("stroke")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("digestive") || s.includes("gut")) return <Pill className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("migraine") || s.includes("tension")) return <ClipboardList className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("respiratory") || s.includes("lung")) return <Globe className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("fibromyalgia")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("serene") || s.includes("beach") || s.includes("coast") || s.includes("escape")) return <MapPin className={cls} />;
    if (s.includes("legacy") || s.includes("award") || s.includes("recognition")) return <Award className={cls} />;
    if (s.includes("authentic") || s.includes("classical") || s.includes("ayurveda")) return <Stethoscope className={cls} />;
    if (s.includes("nature") || s.includes("greenery") || s.includes("garden")) return <Leaf className={cls} />;
    if (s.includes("physician") || s.includes("care") || s.includes("doctor")) return <Hospital className={cls} />;
    if (s.includes("herbal") || s.includes("oils") || s.includes("therapies")) return <Sparkles className={cls} />;
    if (s.includes("panchakarma") || s.includes("detox")) return <Droplet className={cls} />;
    if (s.includes("integrated") || s.includes("yoga") || s.includes("meditation")) return <HeartPulse className={cls} />;
  ...
  const getFacilityIcon = (title: string) => {
    ...
    return t.includes("ayurveda") || t.includes("therapy") ? <Droplet className="h-7 w-7 text-white" />
      : t.includes("villa") || t.includes("pool") ? <Building2 className="h-7 w-7 text-white" />
        : t.includes("dining") || t.includes("restaurant") ? <Utensils className="h-7 w-7 text-white" />
          : t.includes("beach") ? <MapPin className="h-7 w-7 text-white" />
            : t.includes("yoga") || t.includes("meditation") ? <Activity className="h-7 w-7 text-white" />
              : t.includes("fitness") || t.includes("gym") ? <Activity className="h-7 w-7 text-white" />
                : t.includes("backwater") || t.includes("boat") ? <Globe className="h-7 w-7 text-white" />
  ...
  const buildTestimonialIframeSrc = (raw: string) => {
    ...
    if (trimmed.includes("youtube.com/watch")) {
      const idMatch = trimmed.match(/[?&]v=([^&]+)/);
      return idMatch ? `https://www.youtube.com/embed/${idMatch[1]}` : trimmed;
    }
    return trimmed;
  };

  const withYouTubeParams = (url: string) => {
    if (!url) return url;
    const hasQuery = url.includes("?");
    return `${url}${hasQuery ? "&" : "?"}enablejsapi=1&playsinline=1&mute=1&autoplay=0`;
  };

  useEffect(() => {
    const videoSection = document.getElementById("videos");
    const testimonialSection = document.getElementById("testimonial-videos");
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.target.id === "testimonial-videos") {
            setIsTestimonialVideoInView(entry.isIntersecting);
          }
        });
      },
      { threshold: 0.4 }
  ...
  }, [isVideoGalleryInView, soukyaStyleVideoIndex]);

  useEffect(() => {
    const iframe = testimonialIframeRef.current;
    if (!iframe) return;
    const message = JSON.stringify({
      event: "command",
      func: isTestimonialVideoInView ? "playVideo" : "pauseVideo",
      args: ""
    });
    iframe.contentWindow?.postMessage(message, "*");
  }, [isTestimonialVideoInView, selectedTestimonialVideo]);
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose The Nattika Beach Resort" },
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              THE NATTIKA BEACH RESORT KERALA
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Authentic Ayurveda Beachfront Retreat
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Thrissur, Kerala, India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.8</span>
                  <span className="opacity-90">(3500+)</span>
                </div>
              </div>
            ...
            <>
              <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                <img
                  ...
                  alt={`Nattika Center ${selectedImage + 1}`}
                  ...
                  <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                    <div className="relative flex items-center justify-center mb-4 pl-16 md:pl-0">
                      ...
                      <div className="text-center text-primary font-bold leading-relaxed whitespace-nowrap text-lg md:text-2xl">
                        The Nattika Beach Resort Gallery
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {images.map((img, i) => (
                        ...
                        >
                          <img src={img} alt={`Nattika ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className="max-w-6xl mx-auto">
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg">
              <MarkdownContent
                contentPath="/content/Top Centers/The Nattika Beach Resort/main content.txt"
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-base md:text-3xl font-bold text-primary mb-1 whitespace-nowrap">3500+</div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of The Nattika Beach Resort
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Discover the peaceful beachfront setting and authentic healing experience at The Nattika Beach Resort.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">
                Why Choose The Nattika Beach Resort for Your Healing Journey
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of The Nattika Beach Resort
              </h2>
                  ...
                  <div className="relative w-full aspect-[9/16] md:aspect-video">
                    {testimonialVideos[selectedTestimonialVideo] && (
                      <iframe
                        ...
                        src={withYouTubeParams(buildTestimonialIframeSrc(testimonialVideos[selectedTestimonialVideo]))}
                        title="The Nattika Beach Resort Testimonial Video"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/The Nattika Beach Resort/CTA mid.jpg"
                    alt="The Nattika Beach Resort"
                  ...
                  <p className="text-sm text-center mb-4" style={{ color: "#7F543D" }}>
                    Take the first step toward holistic healing. Our expert team will guide you with personalized plans tailored to your needs.
                  </p>
                  <div className="space-y-3">
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: '#7F543D' }}>Recognition of excellence in authentic Ayurveda, sustainability, and guest care</p>
            </div>
                    ...
                    {contactWebsite && (
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm md:text-base leading-relaxed" style={{ color: '#7F543D' }}>{contactWebsite}</p>
                        </div>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="The Nattika Beach Resort Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3923.985183783749!2d76.08009567454224!3d10.42275006561874!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ba78cb5bd2cb95f%3A0xedc9136f02d2bafc!2sThe%20Nattika%20Beach%20Ayurveda%20Resort!5e0!3m2!1sen!2sin!4v1775234681320!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: '#234A50' }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/The Nattika Beach Resort/CTA bottom.jpg"
                    alt="The Nattika Beach Resort"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at The Nattika Beach Resort</h2>
                  <div className="space-y-3">
                    <Button
                      size="lg"
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-bold text-white mb-3">Begin Your Holistic Healing Journey at The Nattika Beach Resort</h2>
                  <div className="flex flex-wrap gap-3">

...
import { MapPin, Star, Calendar, ChevronLeft, ChevronRight, Images, Video, Users, TrendingUp, Heart, Droplet, Brain, Sparkles, Activity, HeartPulse, ShieldCheck, UserCheck, Pill, Stethoscope, FileSearch, ClipboardList, Utensils, Home, Globe, Building2, TreePine, Award, Leaf, Hospital, Phone, MessageCircle, Mail, Search, X } from "lucide-react";
...
export default function BackToRoots() {
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Back to Roots Ayurveda Retreat/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Back to Roots Ayurveda Retreat/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        let fndr: { name: string; degrees: string[]; role: string; description: string } | null = null;
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (title.toLowerCase().includes("expert healing team") || title.toLowerCase().includes("ayurvedic physicians") || title.toLowerCase().includes("our team")) {
              if (currentGroup) groups.push(currentGroup);
              currentGroup = { title, description: "", items: [] };
              section = "team";
              continue;
            }
            fndr = { name: title, degrees: [], role: "", description: "" };
            section = "founder";
            continue;
          }
          if (section === "intro") {
          ...
          if (section === "founder" && fndr) {
            if (!fndr.degrees.length) {
              const degs = line.split(",").map((d) => d.trim()).filter(Boolean);
              fndr.degrees = degs;
              continue;
            }
            if (!fndr.role) {
            ...
            fndr.description = fndr.description ? `${fndr.description} ${line}` : line;
            continue;
          }
          ...
          if (section === "team") {
            if (line.startsWith("*")) {
              const bullet = line.replace(/^\*+\s*/, "");
              if (currentGroup) currentGroup.items.push(bullet);
            } else {
              if (currentGroup) currentGroup.description = currentGroup.description ? `${currentGroup.description} ${line}` : line;
            }
            continue;
          }
        }
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Back to Roots Ayurveda Retreat/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Back to Roots Ayurveda Retreat/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (t.includes("international patients")) { section = "international"; continue; }
          }
          if (section === "intro") {
            intro = intro ? `${intro} ${line}` : line;
            continue;
          ...
          if (section === "insurance") {
            if (line.startsWith("*")) insurance.push(line.replace(/^\*+\s*/, ""));
            continue;
          }
          if (section === "payment") {
            if (line.startsWith("*")) payment.push(line.replace(/^\*+\s*/, ""));
            continue;
          }
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              BACK TO ROOTS AYURVEDA RETREAT KERALA
            </li>
          </ol>
        </div>
      </nav>
      ...
      <div id="gallery" className="container mx-auto px-3 md:px-4 py-12 max-w-full">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Back to Roots
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at Back to Roots Ayurveda Retreat through our video gallery.
              </p>
            </div>
          ...
          {/* Testimonials of Back to Roots Ayurveda - Video Section */}
          <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Back to Roots Ayurveda
              </h2>
            ...
            <div className="relative max-w-4xl mx-auto px-4 md:px-0">
              <Card className="border-2 border-primary/20 shadow-xl overflow-hidden bg-white rounded-3xl">
                <CardContent className="p-0">
                  <div className="aspect-video w-full relative">
                    <video
                      src="/Center Videos/Back to Roots Ayurveda Retreat/Testimonies/Back to Roots Ayurveda testimonies.mp4"
                      controls
      ...
      <div className="container mx-auto px-3 md:px-4 max-w-full">
        <div className="max-w-6xl mx-auto mt-6">
          <div id="team" className="mb-12 rounded-3xl p-4 md:p-10" style={{ backgroundColor: '#EDE8D0' }}>
                ...
                {faqItems.map((it, idx) => (
                  <AccordionItem key={idx} value={`faq-${idx}`} className="border-2 border-primary/20 rounded-lg px-6 data-[state=open]:border-primary transition-colors bg-white">
                    ...
                    <AccordionContent className="pt-4 pb-6 bg-white">
                      <p className="text-sm md:text-base leading-relaxed" style={{ color: '#7F543D' }}>{it.answer}</p>
                    </AccordionContent>
                  </AccordionItem>
                ))}
              </Accordion>

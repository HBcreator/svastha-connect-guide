...
const parseContactInfo = (text: string): ContactInfoData => {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("**") && line.endsWith("**")) {
      ...
      if (header.includes("address")) section = "address";
      else if (header.includes("distance")) section = "distances";
      else if (header.includes("transport")) section = "transport";
      else section = "none";
      continue;
    }
    ...
    if (section === "distances") {
      if (line.startsWith("*")) {
        const bullet = line.replace(/^\*+\s*/, "").trim();
        if (bullet) distances.push(bullet);
      } else {
        distances.push(line);
      }
      continue;
    ...
    if (section === "transport") {
      transportText += (transportText ? " " : "") + line;
    }
  }

  return { address, distances, transportText: transportText.trim() };
};
...
const getFacilityIcon = (t: string) => {
  ...
  if (s.includes("farm") || s.includes("organic") || s.includes("garden") || s.includes("acre") || s.includes("himal") || s.includes("location") || s.includes("nature"))
    return <TreePine className="h-7 w-7 text-white" />;

  if (s.includes("infrastructure") || s.includes("treatment") || s.includes("therapy") || s.includes("pharmacy") || s.includes("panchakarma"))
  ...
  if (s.includes("dining") || s.includes("cuisine") || s.includes("food") || s.includes("meal") || s.includes("kitchen") || s.includes("sattvic"))
  ...
  if (s.includes("safety") || s.includes("nabh") || s.includes("standard") || s.includes("medical") || s.includes("convenience") || s.includes("wi-fi") || s.includes("wifi"))
    return <ShieldCheck className="h-7 w-7 text-white" />;

  if (s.includes("services") || s.includes("housekeeping") || s.includes("laundry") || s.includes("guest") || s.includes("transfer") || s.includes("front desk"))
    return <Users className="h-7 w-7 text-white" />;

  return <Heart className="h-7 w-7 text-white" />;
};

const AyuskamaAyurveda = () => {
  ...
  const founderImage = "/Center Images/Ayuskama Ayurveda/Founder/Founder.jpg";
  const teamImage = "/Center Images/Ayuskama Ayurveda/Founder/Team.jpg";
  ...
  const awards = [
    {
      ...
      description: "A global standard of excellence validating the high quality and authenticity of our yoga and teacher training programs.",
      image: "/Center Images/Ayuskama Ayurveda/Awards/Award 1 (Yoga Alliance USA, a global standard of excellence that validates the high quality and authenticity of our yoga and teacher training programs.).jpg",
    },
    {
      title: "Clinical & Academic Excellence",
      description: "Advanced qualifications and deep expertise that reflect our commitment to evidence-informed Ayurveda and high clinical standards.",
      image: "/Center Images/Ayuskama Ayurveda/Awards/Award 2 ( Dr. Vinod Kumar (MD, PhD Ayurveda), whose advanced qualifications and deep expertise are a testament to our clinical and academic excellence.).jpg",
    },
    {
      title: "Global Recognition & Community Trust",
      description: "Honored and trusted by our global community for authentic Ayurveda, personalized care, and transformative healing outcomes.",
      image: "/Center Images/Ayuskama Ayurveda/Awards/Award 3 ( trust and high praise from our global community, ).png",
    },
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Ayuskama" },
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("reproductive") || s.includes("gynec") || s.includes("gynaec") || s.includes("female") || s.includes("infert")) {
    ...
    return (
      <div className="flex items-center">
        {[1, 2, 3, 4, 5].map((s) => (
          <Star
            key={s}
            className={`h-4 w-4 md:h-5 md:w-5 ${s <= rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
          />
        ))}
      </div>
    );
  ...
  const selectReview = (idx: number) => {
    ...
    setTimeout(() => setIsReviewAutoPlaying(true), 8000);
  };
    ...
    const load = async () => {
      try {
        const [imgRes, vidRes] = await Promise.all([
          fetch("/Center Images/Ayuskama Ayurveda/Photo gallery/Photo Gallery Links.txt"),
          fetch("/Center Videos/Ayuskama Ayurveda/Video links.txt"),
        ]);

        const [imgText, vidText] = await Promise.all([imgRes.text(), vidRes.text()]);

        const parsedImages = imgText
          .split(/\r?\n/)
          .map((l) => l.trim())
          .filter(Boolean);

        const parsedVideos = vidText
          .split(/\r?\n/)
          .map((l) => l.trim())
          .filter(Boolean);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        setInsurancePaymentInfo(parseInsurancePaymentInfo(text));
      })
      .catch(() => {
        setInsurancePaymentInfo(null);
      });
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Frequently Asked Questions About Ayuskama Ayurveda.txt")
      .then((res) => res.text())
      .then((text) => {
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Why Choose Ayuskama Ayurveda.txt")
      .then((res) => res.text())
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let title = "";
  ...
  useEffect(() => {
    fetch("/Center Videos/Ayuskama Ayurveda/YT i frame testimonies.txt")
      .then((res) => res.text())
      .then((text) => {
        const matches = [...text.matchAll(/src="([^"]+)"/g)];
        const links = matches.map((m) => m[1]).filter(Boolean);
        setTestimonialVideos(links);
      })
      .catch(() => setTestimonialVideos([]));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Facilities & Amenities.txt")
      .then((res) => res.text())
  ...
  useEffect(() => {
    fetch("/Center Images/Ayuskama Ayurveda/Facilities/Facilities.txt")
      .then((res) => res.text())
      .then((text) => {
        const links = text
          .split(/\r?\n/)
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.replace(/\r\n/g, "\n").split("\n").map((l) => l.trim());
        let intro = "";
        ...
        let founderName = "";
        const founderDegrees: string[] = [];
        let founderRole = "";
        let founderDesc = "";
        const founderTags: string[] = [];
        ...
        let section: "intro" | "founder" | "group" = "intro";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (!seenFirstHeading) {
              ...
              founderName = parts[0]?.trim() || heading;
              founderRole = parts.slice(1).join(" - ").trim();
              continue;
            }

            if (currentGroup) groups.push(currentGroup);
            currentGroup = { title: heading, description: "", items: [] };
            section = "group";
            continue;
          }
          ...
          if (line.startsWith("*")) {
            const bullet = line.replace(/^\*+\s*/, "");
            if (section === "founder") {
              if (/\bB\.A\.M\.S\.|\bMD\b|\bMRCH\b|\bAyurved/i.test(bullet) && bullet.length <= 80) {
                founderDegrees.push(bullet);
              } else {
                founderTags.push(bullet);
              }
              continue;
            }
          ...
          if (section === "intro" && !seenFirstHeading) {
            intro = intro ? `${intro} ${line}` : line;
            continue;
          }
        ...
        setTeamIntro(intro);
        setFounder(founderName ? { name: founderName, degrees: founderDegrees, role: founderRole, description: founderDesc } : null);
        setFounderExpertise(founderTags);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Ayuskama Ayurveda/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              AYUSKAMA AYURVEDA CENTER HIMACHAL
            </li>
          </ol>
        </div>
      </nav>
        ...
        <div className="w-full px-3 md:px-4">
          <div className="w-full max-w-6xl mx-auto">
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                <h1 className="text-4xl md:text-5xl font-bold mb-4">Ayuskama Ayurveda</h1>
                <p className="text-xl mb-4 opacity-90">Premium Wellness Center</p>
        ...
        <div className="max-w-6xl mx-auto">
                ...
                <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                  {images.length > 0 && (
                    <img
                      ...
                      alt={`Ayuskama Ayurveda ${selectedImage + 1}`}
                      ...
                      return (
                        ...
                        >
                          <div className="relative w-full rounded-xl overflow-hidden" style={{ paddingBottom: "100%" }}>
                            <img
                              src={img}
                              alt={`Ayuskama Ayurveda ${actualIndex + 1}`}
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/Ayuskama Ayurveda/Main content.txt"
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">Video Gallery of Ayuskama</h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at Ayuskama Ayurveda through our video gallery.
              </p>
            </div>
                ...
                <div className="flex justify-center gap-2 mt-6 md:mt-8">
                  {videos.map((_, index) => (
                    <button
                      ...
                      className={`transition-all duration-300 rounded-full ${index === selectedCenterVideo ? "w-8 h-3 bg-primary" : "w-3 h-3 bg-gray-300 hover:bg-primary/50"}`}
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {whyChooseData?.title || "Why Choose Ayuskama Ayurveda"}
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Ayuskama Ayurveda
              </h2>
                  ...
                  <div className="aspect-video w-full relative">
                    {testimonialVideos.length > 0 && (
                      <iframe
                        ...
                        title="Ayuskama Testimonial Video"
          ...
          <div className="mb-12" id="process">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {processIntro || "Your personalized healing journey, step by step"}
              </p>
            </div>
            ...
            <div className="max-w-4xl mx-auto">
              {processSteps.map((step, idx) => {
                ...
                const icon =
                  idx === 0 ? <ClipboardList className="h-5 w-5 md:h-6 md:w-6 text-primary" /> :
                    idx === 1 ? <FileSearch className="h-5 w-5 md:h-6 md:w-6 text-primary" /> :
                      idx === 2 ? <FileSearch className="h-5 w-5 md:h-6 md:w-6 text-primary" /> :
                        idx === 3 ? <Pill className="h-5 w-5 md:h-6 md:w-6 text-primary" /> :
                          idx === 4 ? <Utensils className="h-5 w-5 md:h-6 md:w-6 text-primary" /> :
                            idx === 5 ? <Home className="h-5 w-5 md:h-6 md:w-6 text-primary" /> :
                              <Activity className="h-5 w-5 md:h-6 md:w-6 text-primary" />;

                const isLast = idx === processSteps.length - 1;

                return (
                  <div key={idx} className={`relative flex items-start gap-3 md:gap-6 ${isLast ? "" : "mb-8 md:mb-12"} group`}>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Ayuskama Ayurveda/CTA mid.jpg"
                    alt="Ayuskama Ayurveda Wellness Center"
                  ...
                  <div className="mt-4 flex items-center justify-center gap-2" style={{ color: "#7F543D" }}>
                    ...
                    <a href="tel:+919999999999" className="underline hover:text-primary">Call us</a>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at Ayuskama Ayurveda is designed and maintained to ensure high levels of safety,
                    hygiene, and quality care, so you can focus fully on your healing journey in a serene, naturally
          ...
          <div className="mb-12" id="reviews">
            ...
            <div className="relative">
              ...
              {isReviewAutoPlaying && patientReviews.length > 1 && (
                <div className="absolute top-4 right-4 bg-black/60 text-white px-3 py-1 rounded-full text-sm flex items-center gap-2">
                  <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                  Auto
                </div>
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognition of Ayuskama Ayurveda's excellence in authentic healing, education, and patient care
              </p>
            </div>
              ...
              {isAwardsMobile && (
                <>
                  <button
                    ...
                    className="absolute left-8 top-[57%] -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 rounded-full shadow-lg transition-all border-2 border-primary z-10"
                  ...
                  <button
                    ...
                    className="absolute right-8 top-[57%] -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 rounded-full shadow-lg transition-all border-2 border-primary z-10"
          ...
          {faqItems.length > 0 && (
            <div className="mb-12" id="faq">
              <div className="text-center mb-8">
                ...
                <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                  Find answers to common questions about treatments, programs, and your healing journey
                </p>
              </div>
                  ...
                  <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                    <div className="rounded-xl overflow-hidden">
                      <div className="relative w-full aspect-[800/600]">
                        <iframe
                          title="Ayuskama Ayurveda Map"
                          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d108040.47422110838!2d76.24264353011444!3d32.19709416737503!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x391b56e555555555%3A0xb0da85373111eda5!2sAyuskama%20Ayurveda%20Panchakarma%20Center!5e0!3m2!1sen!2sin!4v1770726366878!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Ayuskama Ayurveda/CTA bottom.jpg"
                    alt="Ayuskama Ayurveda Center"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                    Begin Your Holistic Healing Journey at Ayuskama Ayurveda
                  </h2>
                  ...
                  <div className="mt-6 flex items-center justify-center gap-2 text-white/90 text-sm">
                    ...
                    <a href="tel:+919999999999" className="underline hover:text-white">Call us</a>
                  </div>
                </div>
              </div>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    ...
                    <span className="text-white underline decoration-white/20 underline-offset-8">Ayuskama Ayurveda</span>
                  </h2>
          ...
          <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {images.map((img, idx) => (
                ...
                >
                  <img src={img} alt={`Ayuskama Ayurveda ${idx + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                ...
                alt={`Ayuskama Ayurveda ${lightboxImage + 1}`}
      ...
      {facilityLightboxOpen && facilityImages.length > 0 && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
              Ayuskama Facilities & Amenities
            </div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                src={facilityImages[facilityLightboxImage]}
                alt={`Ayuskama Facility ${facilityLightboxImage + 1}`}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <button
                onClick={() => setFacilityLightboxOpen(false)}

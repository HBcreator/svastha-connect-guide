  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("**") && line.endsWith("**")) {
      ...
      if (
        i < lines.length &&
        lines[i] &&
        !(lines[i].startsWith("*") || (lines[i].startsWith("**") && lines[i].endsWith("**")))
      ) {
...
const AyurvedaGram = () => {
  ...
  const [treatmentProcessTitle, setTreatmentProcessTitle] = useState("");
  const [treatmentProcessIntro, setTreatmentProcessIntro] = useState("");
  const [treatmentProcessSteps, setTreatmentProcessSteps] = useState<
    { step: number; title: string; description: string; bullets: string[] }[]
  >([]);
  ...
  const [testimonials, setTestimonials] = useState<ReviewTestimonial[]>([]);
  ...
  const awards = [
    {
      ...
      description: "Maintaining high standards in quality processes and wellness operations.",
      image: "/Center Images/AyurvedaGram/Awards/GMP.jpg",
    },
    {
      title: "Advanced Qualifications",
      description: "A commitment to continuous learning through globally recognized institutions.",
      image: "/Center Images/AyurvedaGram/Awards/advanced qualifications from renowned institutions,.webp",
    },
    {
      title: "Trust of Thousands of Guests",
      description: "Trusted by guests worldwide for authentic Ayurveda and compassionate care.",
      image: "/Center Images/AyurvedaGram/Awards/trust of thousands of guests.jpg",
    },
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose AyurvedaGram" },
  ...
  useEffect(() => {
    ...
    fetch("/Center Videos/AyurvedaGram/yt iframe.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading AyurvedaGram testimonial videos:", err));

    fetch("/content/Top Centers/AyurvedaGram/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading AyurvedaGram wellness programs:", err));

    fetch("/content/Top Centers/AyurvedaGram/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading AyurvedaGram medical programs:", err));

    fetch("/content/Top Centers/AyurvedaGram/Why Choose AyurvedaGram.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let title = "";
      ...
      .catch((err) => console.error("Error loading AyurvedaGram why choose:", err));

    fetch("/content/Top Centers/AyurvedaGram/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
        let title = "";
        let intro = "";
        const steps: { step: number; title: string; description: string; bullets: string[] }[] = [];

        let current: { step: number; title: string; description: string; bullets: string[] } | null = null;
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (!title) {
            const maybeTitle = line.match(/^\*\*(.+)\*\*$/);
            if (maybeTitle) {
              title = maybeTitle[1].trim();
              continue;
            }
          }
        ...
        if (current) steps.push(current);
        setTreatmentProcessTitle(title);
        setTreatmentProcessIntro(intro);
        setTreatmentProcessSteps(steps.sort((a, b) => a.step - b.step));
      })
      .catch((err) => console.error("Error loading AyurvedaGram treatment process:", err));

    fetch("/content/Top Centers/AyurvedaGram/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => setFacilitiesData(parseCardSection(text)))
      .catch((err) => console.error("Error loading AyurvedaGram facilities:", err));

    fetch("/Center Images/AyurvedaGram/Facilities/Facilities.txt")
      .then((res) => res.text())
      .then((text) => {
        const imgs = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading AyurvedaGram facility images:", err));

    fetch("/content/Top Centers/AyurvedaGram/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => setFounderTeamInfo(parseFounderTeamInfo(text)))
      .catch((err) => console.error("Error loading AyurvedaGram Founder & Team Info:", err));

    fetch("/content/Top Centers/AyurvedaGram/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        const items: ReviewTestimonial[] = [];
        let current: ReviewTestimonial | null = null;
        let idCounter = 1;
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (current && line && !line.startsWith("**") && !line.startsWith("*")) {
            ...
            if (!current.condition && current.title) {
              const knownConditions = ["Arthritis", "Burnout", "Back Pain", "PCOD", "PCOS", "Addiction", "Diabetes", "Insomnia", "Psoriasis", "Cancer", "Stroke", "Hypertension", "Migraines", "Anxiety", "Stress"];
      ...
      .catch((err) => console.error("Error loading AyurvedaGram reviews:", err));

    fetch("/content/Top Centers/AyurvedaGram/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading AyurvedaGram insurance content:", err));

    fetch("/content/Top Centers/AyurvedaGram/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading AyurvedaGram FAQs:", err));

    fetch("/content/Top Centers/AyurvedaGram/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading AyurvedaGram Contact Info:", err));
  }, []);
  ...
  const buildEmbedSrc = (baseUrl: string, autoplay: boolean) => {
    const sep = baseUrl.includes("?") ? "&" : "?";
    return `${baseUrl}${sep}autoplay=${autoplay ? "1" : "0"}&mute=${autoplay ? "1" : "0"}&rel=0`;
  };
        ...
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting) {
            ...
            if (playPromise && typeof (playPromise as Promise<void>).catch === "function") {
              (playPromise as Promise<void>).catch(() => null);
            }
          } else {
            videoElement.pause();
          }
  ...
  const wellnessIconForTitle = (t: string) => {
    ...
    if (s.includes("rejuvenation") || s.includes("rasayana") || s.includes("beauty") || s.includes("skin"))
    ...
    if (s.includes("digital") || s.includes("mindful") || s.includes("preventive")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("musculoskeletal") || s.includes("neurological") || s.includes("arthritis") || s.includes("stroke") || s.includes("spine") || s.includes("back") || s.includes("disc"))
    ...
    if (s.includes("respiratory") || s.includes("allergic") || s.includes("allergy")) return <TreePine className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    ...
    if (s.includes("reproductive") || s.includes("gynec") || s.includes("gynaec") || s.includes("women")) return <Users className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("pediatric") || s.includes("development")) return <UserCheck className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("digest")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    return <Stethoscope className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  };

  const whyChooseIconForTitle = (t: string) => {
    const s = t.toLowerCase();
    if (s.includes("heritage") || s.includes("museum") || s.includes("village") || s.includes("cottage"))
      return <Home className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    if (s.includes("bangalore") || s.includes("accessible") || s.includes("sanctuary") || s.includes("travel"))
      return <MapPin className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    if (s.includes("accredited") || s.includes("nabh") || s.includes("certification") || s.includes("excellence"))
      return <Award className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    if (s.includes("experts") || s.includes("team") || s.includes("physician") || s.includes("guided"))
      return <UserCheck className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    if (s.includes("pure") || s.includes("roots") || s.includes("ayurveda") || s.includes("formulation"))
      return <Leaf className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    if (s.includes("plan") || s.includes("unique") || s.includes("personal"))
  ...
  const treatmentProcessIconForTitle = (t: string) => {
    const s = t.toLowerCase();
    if (s.includes("consult") || s.includes("diagnos") || s.includes("evaluation") || s.includes("assessment")) {
      return <ClipboardList className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("blueprint") || s.includes("plan") || s.includes("roadmap") || s.includes("prescription")) {
      return <FileSearch className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("daily") || s.includes("therapy") || s.includes("treatment") || s.includes("healing")) {
    ...
    if (s.includes("harmony") || s.includes("mind") || s.includes("yoga") || s.includes("meditation") || s.includes("walk")) {
      return <Activity className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("home") || s.includes("empower") || s.includes("follow") || s.includes("journey")) {
  ...
  const getFacilityIcon = (t: string) => {
    ...
    if (s.includes("accommodation") || s.includes("room") || s.includes("suite") || s.includes("stay") || s.includes("cottage"))
      return <Home className="h-7 w-7 text-white" />;

    if (s.includes("architecture") || s.includes("design") || s.includes("heritage") || s.includes("building"))
      return <Building2 className="h-7 w-7 text-white" />;

    if (s.includes("garden") || s.includes("nature") || s.includes("village") || s.includes("environment") || s.includes("green"))
      return <TreePine className="h-7 w-7 text-white" />;

    if (s.includes("treatment") || s.includes("therapy") || s.includes("hospital") || s.includes("physician") || s.includes("consultation"))
      return <Stethoscope className="h-7 w-7 text-white" />;

    if (s.includes("yoga") || s.includes("meditation") || s.includes("shala") || s.includes("mind"))
      return <Sparkles className="h-7 w-7 text-white" />;

    if (s.includes("dining") || s.includes("food") || s.includes("cuisine") || s.includes("meal"))
      return <Utensils className="h-7 w-7 text-white" />;

    if (s.includes("safety") || s.includes("nabh") || s.includes("standard") || s.includes("accredited") || s.includes("certification"))
      return <ShieldCheck className="h-7 w-7 text-white" />;

    if (s.includes("transport") || s.includes("access") || s.includes("airport") || s.includes("travel"))
      return <MapPin className="h-7 w-7 text-white" />;

    if (s.includes("housekeeping") || s.includes("support") || s.includes("services") || s.includes("staff"))
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <nav className="bg-[#FCFBF7] border-b border-[#EDE8D0] py-3">
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center flex-wrap gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em]">
            ...
            <li className="flex items-center gap-2">
              <Link to="/top-10-ayurvedic-centers-hospitals-bangalore-hyderabad-chennai-south-india" className="text-primary/50 hover:text-primary transition-colors flex items-center gap-1">
                Top 10 Centers Bangalore, Hyderabad, Chennai & South India
              </Link>
              <ChevronRight className="h-3 w-3 text-primary/20" />
            </li>
      ...
      <div className="container mx-auto px-3 md:px-4 py-12 max-w-full">
        <div className="max-w-6xl mx-auto">
          ...
          {videos.length > 0 && (
            <div className="mb-12" id="videos">
              <div className="text-center mb-10">
                <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">Video Gallery of AyurvedaGram</h2>
                <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                  Experience the serene atmosphere and healing journey at AyurvedaGram through our video gallery.
                </p>
              </div>
            ...
            <div className="mb-12" id="why-choose">
              <div className="text-center mb-10">
                <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                  {whyChooseTitle || "Why Choose AyurvedaGram"}
                </h2>
                {whyChooseIntro && (
            ...
            <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
              <div className="text-center mb-8 md:mb-10 px-4">
                <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                  Testimonials of AyurvedaGram Center
                </h2>
          ...
          {treatmentProcessSteps.length > 0 && (
            <div className="mb-12" id="process">
              <div className="text-center mb-8 md:mb-12">
                <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                  {treatmentProcessTitle || "Treatment Process & Patient Journey"}
                </h2>
                <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                  {treatmentProcessIntro || "Your personalized healing journey at AyurvedaGram, step by step"}
                </p>
              </div>

              <div className="max-w-4xl mx-auto">
                {treatmentProcessSteps.map((stepItem, idx) => {
                  const isLast = idx === treatmentProcessSteps.length - 1;
                  return (
                    <div key={stepItem.step} className="relative flex items-start gap-3 md:gap-6 mb-8 md:mb-12 group">
                        ...
                        <CardContent className="p-4 md:p-6">
                          ...
                          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3 pl-10 md:pl-0">
                            <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-primary/10 flex items-center justify-center">
                              {treatmentProcessIconForTitle(stepItem.title)}
                            </div>
                            <div>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/AyurvedaGram/CTA.jpg"
                    alt="AyurvedaGram Wellness Center"
                      ...
                      {facilityImages.map((image, index) => (
                        <div key={index} className="w-full flex-shrink-0 px-2">
                          ...
                          >
                            <img
                              src={image}
                              alt={`AyurvedaGram Facility ${index + 1}`}
          ...
          <div className="mb-10 rounded-3xl p-4 md:p-10" style={{ backgroundColor: "#EDE8D0" }} id="team">
            <div className="text-center mb-6 md:mb-10">
              <h1 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {founderTeamInfo?.title || "Founder & Team Info"}
              </h1>
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                ...
                <CardContent className="p-4 md:p-8">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    >
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/AyurvedaGram/Founder/Founder.jfif"
                    ...
                    <div>
                      <h3 className="text-lg md:text-2xl font-bold text-primary mb-1 md:mb-2">
                        {founderTeamInfo?.founderName || "Founder"}
                      </h3>
                      ...
                      <p className="text-xs md:text-sm mt-1 text-primary/70">Founder</p>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 md:pt-4 border-t border-primary/10">
                    <p className="text-xs font-semibold text-primary mb-2">
                      {founderTeamInfo?.founderTagsTitle || "Leadership & Expertise"}
                    </p>
                ...
                <CardContent className="p-4 md:p-8 h-full md:h-[480px] md:overflow-y-auto">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    >
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/AyurvedaGram/Founder/Team.jpg"
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-3 md:gap-4 mb-3 md:mb-4">
                          <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-primary text-white flex items-center justify-center text-base md:text-xl font-bold flex-shrink-0">
                            {(testimonials[currentReview].name || "A").charAt(0)}
                          </div>
            ...
            <div className="flex justify-center gap-2 mt-6">
              {testimonials.map((_, idx) => (
                <button
                  ...
                  className={`transition-all ${idx === currentReview ? "w-8 h-3 bg-primary" : "w-3 h-3 bg-gray-300 hover:bg-primary/50"} rounded-full`}
                    ...
                    {awards.map((award, i) => (
                      <div key={i} className="w-1/3 flex-shrink-0 px-4">
                        <div className="bg-white rounded-2xl p-6 shadow-lg border-2 border-primary/10 hover:border-primary/30 transition-all h-full flex flex-col items-center">
                          ...
                          <h3 className="text-xl font-bold text-primary mb-3 text-center">{award.title}</h3>
                          <p className="text-sm text-center" style={{ color: "#7F543D" }}>{award.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                    <div className="rounded-xl overflow-hidden">
                      <div className="relative w-full aspect-[800/600]">
                        <iframe
                          title="AyurvedaGram Map"
                          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3888.033543532335!2d77.79751447427066!3d12.969705414921487!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae0df955555555%3A0x3173b315d49637f3!2sAyurvedagram!5e0!3m2!1sen!2sin!4v1769613721559!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/AyurvedaGram/CTA bottom.jpg"
                    alt="AyurvedaGram Wellness Center"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                    Begin Your Holistic Healing Journey at AyurvedaGram
                  </h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    ...
                    <span className="text-white underline decoration-white/20 underline-offset-8">AyurvedaGram</span>
                  </h2>
...

      {
        facilityLightboxOpen && (
          ...
          >
            ...
            <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
              <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
                AyurvedaGram Facilities & Amenities
              </div>
              <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
                <img
                  src={facilityImages[facilityLightboxImage]}
                  alt={`AyurvedaGram Facility ${facilityLightboxImage + 1}`}
                  className="absolute inset-0 w-full h-full object-cover"
                />
                <button
                  onClick={() => setFacilityLightboxOpen(false)}
          ...
          <div className="p-4 pb-4 bg-[#2F5B63] text-white relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-3 relative z-10">
              <div className="space-y-0.5">
                ...
                <h2 className="text-[22px] sm:text-[26px] font-extrabold leading-tight tracking-tight text-white break-words">
                  Sections of AyurvedaGram
                </h2>
              </div>
              <button
                onClick={() => setIsJumpModalOpen(false)}

...
const getFacilityIcon = (t: string) => {
  ...
  if (s.includes("recreation") || s.includes("pool") || s.includes("swimming") || s.includes("activity") || s.includes("fitness")) {
...
const treatmentIconForTitle = (t: string) => {
  const s = t.toLowerCase();
  if (s.includes("consult")) return <ClipboardList className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
  if (s.includes("plan") || s.includes("blueprint")) return <FileSearch className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
  if (s.includes("treatment") || s.includes("therapy")) return <Pill className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
  if (s.includes("diet") || s.includes("nutrition") || s.includes("cuisine") || s.includes("food")) {
    return <Utensils className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
  }
  if (s.includes("activity") || s.includes("yoga") || s.includes("meditation")) {
    return <Activity className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
  }
  if (s.includes("discharge") || s.includes("lifestyle") || s.includes("home") || s.includes("empower")) {
...
const wellnessIconForTitle = (t: string) => {
  ...
  if (s.includes("weight") || s.includes("shareera") || s.includes("fitness") || s.includes("active")) {
    return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  }
  if (s.includes("rejuvenation") || s.includes("vitality") || s.includes("energy")) {
    return <Sun className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  }
  if (s.includes("nutrition") || s.includes("diet") || s.includes("cuisine") || s.includes("food")) {
    return <Utensils className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  }
  if (s.includes("herbal") || s.includes("nature") || s.includes("goan") || s.includes("garden")) {
    return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  }
  if (s.includes("skin") || s.includes("beauty") || s.includes("glow")) {
    return <Flower2 className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  }
...
const medicalIconForTitle = (t: string) => {
  const s = t.toLowerCase();
  if (s.includes("panchakarma") || s.includes("detox")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  if (s.includes("weight")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  if (s.includes("diabetes") || s.includes("hypertension") || s.includes("cholesterol") || s.includes("cardio")) {
  ...
  if (s.includes("stress") || s.includes("mental") || s.includes("emotional") || s.includes("sleep")) {
  ...
  if (s.includes("immunity") || s.includes("recovery")) {
    return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  }
  if (s.includes("digestive") || s.includes("gut") || s.includes("ibs") || s.includes("acidity")) {
  ...
  if (s.includes("pcos") || s.includes("menopause") || s.includes("reproductive") || s.includes("gynec") || s.includes("gynaec")) {
...
const MercureGoaDevaayaResort = () => {
  ...
  const founderImage = "/Center Images/Mercure Goa Devaaya Resort/Founder/Founder.jpg";
  const teamImage = "/Center Images/Mercure Goa Devaaya Resort/Founder/team.webp";
  const awards = [
    {
      title: "TripAdvisor Travelers’ Choice 2016",
      description: "Recognized among top-rated experiences based on traveler reviews and ratings.",
      image: "/Center Images/Mercure Goa Devaaya Resort/Awards/Awards 1 (trip advisor).png",
    },
    {
      title: "TripAdvisor Premier Partner 2017",
      description: "Acknowledged for consistently delivering a trusted guest experience.",
      image: "/Center Images/Mercure Goa Devaaya Resort/Awards/Award 2 (preimier partner).png",
    },
    {
      title: "TripAdvisor Premier Partner 2018",
      description: "Recognition for continued excellence and guest satisfaction.",
      image: "/Center Images/Mercure Goa Devaaya Resort/Awards/Award 3 (premirer partnre 2018).png",
    },
    {
      title: "TripAdvisor Certificate of Excellence",
      description: "A distinction grounded in strong traveler reviews and service quality.",
      image: "/Center Images/Mercure Goa Devaaya Resort/Awards/Award 4 (certificate of excellence).png",
    },
    {
      title: "NABH Accredited",
      description: "Accreditation highlighting quality standards and patient safety in healthcare.",
      image: "/Center Images/Mercure Goa Devaaya Resort/Awards/Award 5 (NABH).webp",
    },
  ];
  ...
  useEffect(() => {
    ...
    loadList(
      "/Center Images/Mercure Goa Devaaya Resort/Photo Gallery/Photo Gallery Links.txt",
      setImages
    );
    loadList(
      "/Center Videos/Mercure Goa Devaaya Resort/Video Gallery Links.txt",
      setVideos
    );
  }, []);

  useEffect(() => {
    fetch("/Center Videos/Mercure Goa Devaaya Resort/testimonies/testimonies links.txt")
      .then((res) => res.text())
      .then((text) => {
        const items = text
          .split(/\r?\n/)
      ...
      .catch((err) => console.error("Error loading Mercure testimonial videos:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Wellness & Rejuvenation Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Mercure wellness content:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Medical Treatment Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Mercure medical content:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Why Choose Mercure Goa Devaaya Resort.txt")
      .then((res) => res.text())
      .then((text) => setWhyChooseData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Mercure why-choose content:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Mercure treatment process content:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => setFacilitiesData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Mercure facilities content:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Our Founders & Expert Leadership Team.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("### ")) {
            ...
            if (/visionary/i.test(t)) { section = "founder"; continue; }
            if (/dedicated healing team/i.test(t)) { if (currentGroup) groups.push(currentGroup); currentGroup = null; section = "teams"; continue; }
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (/our founders|expert leadership/i.test(t)) { continue; }
          ...
          if (section === "founder") {
            if (/MD|BAMS|Ayurveda|MRCH|Hom|Director|Chief|CEO/i.test(line) && line.length <= 80) {
              if (/Director|Chief|CEO/i.test(line) && role === "") { role = line; continue; }
              if (!degrees.includes(line) && line.length <= 60) { degrees.push(line); continue; }
            }
      ...
      .catch((err) => console.error("Error loading Mercure founder content:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Mercure reviews:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Mercure insurance info:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Mercure FAQs:", err));

    fetch("/content/Top Centers/Mercure Goa Devaaya Resort/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Mercure contact info:", err));
  }, []);
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Mercure" },
  ...
  useEffect(() => {
    const loadList = async (path: string, setter: (items: string[]) => void) => {
      try {
        ...
        console.warn("Failed to load list:", path, err);
        setter([]);
      }
    };

    loadList(
      "/Center Images/Mercure Goa Devaaya Resort/Facilities/Facilities links.txt",
      setFacilityImages
    );
  }, []);
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              MERCURE GOA DEVAAYA RESORT GOA
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-base md:text-lg">Divar Island, Goa, India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.7</span>
                  <span className="opacity-90">(1900+)</span>
                </div>
              </div>
        ...
        <div className="max-w-6xl mx-auto">
                  ...
                  <>
                    <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                      <img
                        ...
                        alt={`Mercure Goa Devaaya ${selectedImage + 1}`}
                            ...
                            return (
                              ...
                              >
                                <div className="relative w-full rounded-xl overflow-hidden" style={{ paddingBottom: "100%" }}>
                                  <img
                                    src={img}
                                    alt={`Mercure Goa Devaaya ${actualIndex + 1}`}
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/Mercure Goa Devaaya Resort/Main content.txt"
          ...
          <div className="mb-12 rounded-3xl px-6 py-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-base md:text-3xl font-bold text-primary mb-1 whitespace-nowrap">1900+</div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Mercure Goa Devaaya Resort
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at Mercure Goa Devaaya through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {whyChooseData?.title || "Why Choose Mercure Goa Devaaya Resort"}
              </h2>
          ...
          {/* Testimonials of Mercure Goa Devaaya Resort - Video Section */}
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Mercure Goa Devaaya Resort
              </h2>
          ...
          <div className="mb-12" id="process">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {treatmentIntro || "Your personalized healing journey, step by step."}
              </p>
            </div>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Mercure Goa Devaaya Resort/CTA mid.jpg"
                    alt="Mercure Goa Devaaya Resort"
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at Mercure Goa Devaaya Resort is designed to support safe, hygienic, and restorative care.
                    Our commitment to excellence means you receive world-class holistic treatment in a serene, naturally
          ...
          <div className="mt-16 md:mt-24" id="awards">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg px-4 mx-auto max-w-2xl" style={{ color: "#7F543D" }}>
                Recognitions spanning guest satisfaction, service quality, and clinical standards for holistic care.
              </p>
            </div>
          ...
          <div className="mt-12 md:mt-16 mb-12" id="insurance">
            <div className="text-center mb-8">
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Mercure Goa Devaaya Resort Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3843.983333568183!2d73.90596007336077!3d15.53902415341722!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bbfc08e645c34e5%3A0x146d34cbf492611d!2sMercure%20Goa%20Devaaya%20Resort!5e0!3m2!1sen!2sin!4v1774362158380!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Mercure Goa Devaaya Resort/CTA bottom.jpg"
                    alt="Mercure Goa Devaaya Resort"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                    Begin Your Holistic Healing Journey at Mercure Goa Devaaya
                  </h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    ...
                    <span className="text-white underline decoration-white/20 underline-offset-8">Mercure Goa Devaaya</span>
                  </h2>
          ...
          <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
            <div className="relative flex items-center justify-center mb-4 pl-16 md:pl-0">
              ...
              <div className="text-center text-primary font-bold leading-relaxed whitespace-nowrap text-lg md:text-2xl">
                Mercure Goa Devaaya Resort Gallery
              </div>
            </div>
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                ...
                alt={`Mercure Goa Devaaya ${lightboxImage + 1}`}
      ...
      {facilityLightboxOpen && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
              Mercure Goa Devaaya Resort Facilities & Amenities
            </div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                src={facilityImages[facilityLightboxImage]}
                alt={`Mercure Facility ${facilityLightboxImage + 1}`}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <button
                onClick={() => setFacilityLightboxOpen(false)}

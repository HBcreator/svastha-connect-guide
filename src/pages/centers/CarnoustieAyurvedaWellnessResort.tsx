...
export default function CarnoustieAyurvedaWellnessResort() {
  ...
  const founderImage = "/Center Images/Carnoustie Ayurveda/Founder/Founder.jpg";
  const teamImage = "/Center Images/Carnoustie Ayurveda/Founder/team.jpg";
  ...
  const awards = [
    {
      title: "Global Spa Awards 2025",
      description: "Recognized among leading spa destinations for exceptional wellness experiences and service.",
      image: "/Center Images/Carnoustie Ayurveda/Awards/Awards 1 (Global Spa Awards (2025).webp",
    },
    {
      title: "LUXlife Magazine Awards",
      description: "Honored for excellence in luxury wellness and hospitality.",
      image: "/Center Images/Carnoustie Ayurveda/Awards/Award 2 (LUXlife Magazine Awards).png",
    },
    {
      title: "World Luxury Spa & Restaurant Awards 2025",
      description: "Winner recognition for world-class spa and dining experiences.",
      image: "/Center Images/Carnoustie Ayurveda/Awards/Award 3 (World Luxury Spa & Restaurant Awards ().jfif",
    },
    {
      title: "NABH Accreditation",
      description: "Certified for clinical quality and patient safety by NABH.",
      image: "/Center Images/Carnoustie Ayurveda/Awards/Award 4 (NABH).jfif",
    },
  ];
  ...
  useEffect(() => {
    fetch(encodeURI("/Center Images/Carnoustie Ayurveda/Photo Gallery/Photo Gallery Links.txt"))
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Carnoustie photo gallery:", err));

    fetch(encodeURI("/Center Videos/Carnoustie Ayurveda/Videos Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
        setVideos(lines);
      ...
      .catch((err) => console.error("Error loading Carnoustie video gallery:", err));

    fetch(encodeURI("/Center Videos/Carnoustie Ayurveda/Testimonies/Testimonies Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      ...
      .catch((err) => console.error("Error loading Carnoustie testimonial videos:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Carnoustie Ayurveda/Wellness & Rejuvenation Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Carnoustie wellness content:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Medical Treatment Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Carnoustie medical content:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Why Choose Carnoustie Ayurveda.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Carnoustie why-choose content:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Carnoustie treatment process content:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
      ...
      .catch((err) => console.error("Error loading Carnoustie facilities content:", err));

    fetch(encodeURI("/Center Images/Carnoustie Ayurveda/Facilities/Facilities Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      ...
      .catch((err) => console.error("Error loading Carnoustie facilities images:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("### ")) {
            ...
            if (/expert wellness team|expert healing team/i.test(t)) { if (currentGroup) groups.push(currentGroup); currentGroup = null; section = "teams"; continue; }
      ...
      .catch((err) => console.error("Error loading Carnoustie founder content:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Carnoustie reviews:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Carnoustie Ayurveda/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Carnoustie insurance content:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Carnoustie FAQs:", err));

    fetch("/content/Top Centers/Carnoustie Ayurveda/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (section === "distance" && line.startsWith("*")) {
            distances.push(line.replace(/^\*+\s*/, ""));
            continue;
          }
          if (section === "address") {
            address.push(line);
      ...
      .catch((err) => console.error("Error loading Carnoustie contact info:", err));
  }, []);
  ...
  const wellnessIconForTitle = (t: string) => {
    ...
    if (s.includes("rejuvenation") || s.includes("rasayana")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("detox") || s.includes("cleanse")) return <Droplet className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("anti-stress") || s.includes("stress")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("beauty") || s.includes("radiance")) return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("yoga") || s.includes("meditation")) return <Compass className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("couple") || s.includes("honeymoon")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("weekend") || s.includes("escape")) return <Calendar className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("digital detox")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("healthy living")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("corporate") || s.includes("leadership")) return <Users className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("fitness") || s.includes("activity")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("weight") || s.includes("metabolic") || s.includes("diabetes")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("arthritis") || s.includes("rheumatism") || s.includes("spine") || s.includes("neck") || s.includes("sciatica")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("psoriasis") || s.includes("skin")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("stress") || s.includes("anxiety")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("parkinson") || s.includes("neuro")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("cardio") || s.includes("hypertension")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("post-cancer")) return <Stethoscope className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("anti-aging") || s.includes("longevity")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("infertility")) return <Users className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("nabh") || s.includes("hospital")) return <ShieldCheck className={cls} />;
    if (s.includes("villa") || s.includes("pool") || s.includes("privacy")) return <Home className={cls} />;
    if (s.includes("awards") || s.includes("acclaim")) return <Award className={cls} />;
    if (s.includes("personal") || s.includes("physician") || s.includes("vaidyas")) return <Stethoscope className={cls} />;
    if (s.includes("kerala") || s.includes("beach")) return <MapPin className={cls} />;
    if (s.includes("cuisine") || s.includes("gourmet")) return <Utensils className={cls} />;
    if (s.includes("holistic") || s.includes("integrated")) return <HeartPulse className={cls} />;
    if (s.includes("eco") || s.includes("sustainable")) return <Leaf className={cls} />;
    if (s.includes("discretion") || s.includes("privacy")) return <ShieldCheck className={cls} />;
    return <Sparkles className={cls} />;
  };

  const treatmentIconForTitle = (t: string) => {
    const s = t.toLowerCase();
    if (s.includes("consultation") || s.includes("physician")) return <Stethoscope className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("bespoke") || s.includes("regimen") || s.includes("plan")) return <Calendar className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("therapies") || s.includes("treatments")) return <Droplet className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("cuisine") || s.includes("food") || s.includes("meals")) return <Utensils className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("wellness") || s.includes("yoga") || s.includes("meditation")) return <HeartPulse className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("departure") || s.includes("lifestyle")) return <TrendingUp className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    return <Calendar className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
  };

  const getFacilityIcon = (title: string) => {
    const t = title.toLowerCase();
    return t.includes("ayurveda") || t.includes("hospital") ? <Droplet className="h-7 w-7 text-white" />
        ...
        : t.includes("dining") || t.includes("restaurant") ? <Utensils className="h-7 w-7 text-white" />
          : t.includes("beach") ? <MapPin className="h-7 w-7 text-white" />
            : t.includes("yoga") || t.includes("meditation") ? <Activity className="h-7 w-7 text-white" />
              : t.includes("fitness") || t.includes("gym") || t.includes("pool") ? <Activity className="h-7 w-7 text-white" />
                : t.includes("houseboat") || t.includes("backwaters") ? <Globe className="h-7 w-7 text-white" />
                  : t.includes("farm") || t.includes("garden") ? <TreePine className="h-7 w-7 text-white" />
                    : t.includes("business") || t.includes("conference") ? <Users className="h-7 w-7 text-white" />
                      : <ShieldCheck className="h-7 w-7 text-white" />;
  };

  useEffect(() => {
    if (!facilityImages.length || facilityLightboxOpen) return;
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Carnoustie" },
  ...
  const videoPosterForIndex = (idx: number) => {
    ...
    return "/Center Images/Carnoustie Ayurveda/Thumb.jpg";
  };
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Beachfront Ayurveda, Yoga & Naturopathy Retreat
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Mararikulam, Kerala, India</span>
                </div>
            ...
            <>
              <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                <img
                  ...
                  alt={`Carnoustie Center ${selectedImage + 1}`}
                ...
                <div className="flex-none w-full md:w-[calc(33.333%-0.375rem)] grid grid-cols-2 gap-3">
                  {thumbnailImages.slice(1, 5).map((img, idx) => {
                    ...
                    return (
                      ...
                      >
                        <img src={img} alt={`Carnoustie ${idx + 2}`} className="w-full h-[120px] md:h-[185px] object-cover transition-transform duration-500 group-hover:scale-110" />
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                {videos.map((video, idx) => (
                  ...
                  >
                    <video muted preload="metadata" poster={videoPosterForIndex(idx)} className="w-full h-full object-cover">
        ...
        <div className="max-w-6xl mx-auto">
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg">
              <MarkdownContent
                contentPath="/content/Top Centers/Carnoustie Ayurveda/Main Content.txt"
          ...
          <div className="mb-12 rounded-3xl px-6 py-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              ...
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-xs md:text-sm" style={{ color: "#7F543D" }}>Guest Satisfaction</div>
              </div>
            </div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Carnoustie Ayurveda & Wellness Resort
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene beachfront setting and authentic healing journey at Carnoustie through our video gallery.
              </p>
            </div>
                  ...
                  <div className="aspect-video w-full relative bg-black">
                    {videos[soukyaStyleVideoIndex] && (
                      <video
                        ...
                        controls
                        playsInline
                        poster={videoPosterForIndex(soukyaStyleVideoIndex)}
                      />
                    )}
                  </div>
                </CardContent>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">
                Why Choose Carnoustie Ayurveda & Wellness Resort for Your Healing Journey
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Carnoustie Ayurveda & Wellness Resort
              </h2>
            ...
            <div className="relative max-w-4xl mx-auto px-4 md:px-0">
              <Card className="border-2 border-primary/20 shadow-xl overflow-hidden bg-white rounded-3xl">
                <CardContent className="p-0">
                  <div className="aspect-[9/16] w-full max-w-sm md:max-w-[320px] mx-auto relative bg-black">
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Carnoustie Ayurveda/CTA bottom.jpg"
                    alt="Carnoustie Ayurveda & Wellness Resort"
                  ...
                  <p className="text-sm text-center mb-4" style={{ color: "#7F543D" }}>
                    Take the first step toward holistic healing. Our expert team will guide you with a personalized plan tailored to your needs.
                  </p>
                  <div className="space-y-3">
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center group">
                <div>
                  ...
                  <p className="text-base md:text-lg mb-6" style={{ color: "#7F543D" }}>
                    Take the first step toward holistic healing. Our team will guide you with a personalized plan tailored to your needs.
                  </p>
            ...
            >
              ...
              <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
                <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
                  Carnoustie Facilities & Amenities
                </div>
                <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
                  <img
                    ...
                    alt={`Carnoustie Facility ${facilityLightboxImage + 1}`}
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognition of excellence in luxury wellness and authentic Ayurvedic care.
              </p>
            </div>
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Carnoustie Ayurveda & Wellness Resort Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3933.561472411782!2d76.29619547300415!3d9.632974079246683!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3b087d66d9f09f1f%3A0x3bbf3f2f43f1a2ef!2sCarnoustie%20Ayurveda%20%26%20Wellness%20Resort!5e0!3m2!1sen!2sin!4v1775136495260!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Carnoustie Ayurveda/CTA mid.jpg"
                    alt="Carnoustie Ayurveda & Wellness Resort"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                    Begin Your Holistic Healing Journey at Carnoustie Ayurveda & Wellness Resort
                  </h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                ...
                <div className="bg-black/20 rounded-2xl p-4 overflow-hidden">
                  <img
                    src="/Center Images/Carnoustie Ayurveda/CTA mid.jpg"
                    alt="Carnoustie Ayurveda & Wellness Resort"
                    className="w-full h-full object-cover rounded-xl shadow-lg transition-transform duration-700 ease-out group-hover:scale-105"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                ...
                alt={`Carnoustie ${lightboxImage + 1}`}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <button
                onClick={() => setLightboxOpen(false)}

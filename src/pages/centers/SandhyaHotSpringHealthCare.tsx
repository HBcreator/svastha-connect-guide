...
const SandhyaHotSpringHealthCare = () => {
  ...
  const founderImage = "/Center Images/Sandhya Hot Spring Health Care/Founder/Founder.jpg";
  const teamImage = "/Center Images/Sandhya Hot Spring Health Care/Founder/Team.jpg";
  ...
  const awards = [
    {
      title: "Diamond of Himachal",
      description: "State-level recognition honoring outstanding contributions to wellness tourism in Himachal Pradesh.",
      image: "/Center Images/Sandhya Hot Spring Health Care/Awards/Award 1  (daimond of himachal ).png",
    },
    {
      title: "Tripadvisor Travelers’ Choice Awards",
      description: "Tripadvisor’s annual program recognizing top-rated experiences based on traveler reviews and ratings.",
      image: "/Center Images/Sandhya Hot Spring Health Care/Awards/Awards 2 (tripadvisor ).webp",
    },
    {
      title: "NABH Accredited",
      description: "Accreditation by NABH for quality standards and patient safety in healthcare services.",
      image: "/Center Images/Sandhya Hot Spring Health Care/Awards/Awards 3 (nabh).webp",
    },
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Sandhya" },
  ...
  useEffect(() => {
    const photoListPath = "/Center Images/Sandhya Hot Spring Health Care/Photo Gallery/Photo gallery Links.txt";
    ...
    setVideos([
      "/Center Videos/Sandhya Hot Spring Health Care/video 1.mp4",
      "/Center Videos/Sandhya Hot Spring Health Care/video 2.mp4",
    ]);
    loadList("/Center Images/Sandhya Hot Spring Health Care/Facilities/Facilities.txt", setFacilityImages);
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Wellness & Rejuvenation Programs.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Sandhya wellness content:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Medical Treatment Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Sandhya medical content:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Why Choose Sandhya Hot Spring Health Care.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Sandhya why-choose content:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Sandhya treatment process content:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
      ...
      .catch((err) => console.error("Error loading Sandhya facilities content:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        ...
        let founderLineIndex = 0;

        for (const line of lines) {
          if (line.startsWith("### ")) {
            const heading = line.replace(/^###\s+/, "").trim();
            if (/founder & team info/i.test(heading)) continue;
            if (/visionary founder/i.test(heading)) {
              section = "founder";
              founderLineIndex = 0;
              continue;
            }
            if (section === "founder" && !name) {
              name = heading;
              founderLineIndex = 1;
              continue;
            }
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (/visionary founder/i.test(t)) { section = "founder"; founderLineIndex = 0; continue; }
            if (/guiding principles/i.test(t)) { section = "expertise"; continue; }
            if (/expert medical/i.test(t) || /wellness team/i.test(t)) {
          ...
          if (section === "founder") {
            if (founderLineIndex === 0 && !name) {
              name = line;
              founderLineIndex = 1;
              continue;
            }
            if (founderLineIndex === 1 && degrees.length === 0) {
              degrees.push(line);
              founderLineIndex = 2;
              continue;
            }
            if (founderLineIndex === 2 && !role) {
              role = line;
              founderLineIndex = 3;
              continue;
            }
          ...
          if (section === "group" && currentGroup) {
            if (!currentGroup.description) currentGroup.description = line;
            else currentGroup.description = currentGroup.description ? `${currentGroup.description} ${line}` : line;
            continue;
          }
        }
        if (currentGroup) groups.push(currentGroup);
      ...
      .catch((err) => console.error("Error loading Sandhya founder content:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Sandhya reviews:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Sandhya insurance info:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Sandhya FAQs:", err));

    fetch("/content/Top Centers/Sandhya Hot Spring Health Care/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        const addr: string[] = [];
        const dists: string[] = [];
        let transport = "";
        let section: "address" | "distances" | "transport" | "none" = "none";
      ...
      .catch((err) => console.error("Error loading Sandhya contact info:", err));
  }, []);
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("hot spring") || s.includes("manikaran") || s.includes("himachal")) return <MapPin className={cls} />;
    if (s.includes("heritage") || s.includes("legacy")) return <ShieldCheck className={cls} />;
    if (s.includes("authentic") || s.includes("natural")) return <Leaf className={cls} />;
    if (s.includes("therapist") || s.includes("care")) return <Stethoscope className={cls} />;
    if (s.includes("detox") || s.includes("rejuvenation")) return <Droplet className={cls} />;
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              SANDHYA HOT SPRING HEALTH CARE HOSPITAL HIMACHAL
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Hot Spring Therapy & Holistic Wellness Retreat
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Manikaran, Himachal Pradesh, India</span>
                </div>
              ...
              <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                {images[selectedImage] && (
                  <img
                    ...
                    alt={`Sandhya Center ${selectedImage + 1}`}
                  ...
                  <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {images.map((img, i) => (
                        ...
                        >
                          <img src={img} alt={`Sandhya ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {lightboxOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#EDE8D0]/80 backdrop-blur-sm">
                  ...
                  <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
                    <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Sandhya Hot Spring Health Care</div>
                    <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
                      {images[lightboxImage] && (
                        <img
                          src={images[lightboxImage]}
                          alt={`Sandhya ${lightboxImage + 1}`}
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Sandhya Hot Spring Health Care
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Explore the serene atmosphere and healing experiences at Sandhya through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">
                Why Choose Sandhya Hot Spring Health Care for Your Healing Journey
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Sandhya Hot Spring Health Care
              </h2>
              ...
              <p className="text-sm md:text-lg mx-auto max-w-none leading-relaxed italic" style={{ color: "#7F543D" }}>
                Watch real guest experiences of relaxation and renewal at Sandhya.
              </p>
            </div>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Sandhya Hot Spring Health Care/CTA mid.jpg"
                    alt="Sandhya Hot Spring Health Care"
            ...
            <div className="max-w-6xl mx-auto mt-6">
              ...
              <div className="mb-12 rounded-3xl p-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="team">
                ...
                <div className="grid md:grid-cols-2 gap-4 md:gap-8 items-stretch">
                  <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all hover:shadow-xl h-full overflow-hidden">
                    <CardContent className="p-4 md:p-8 h-full md:min-h-[480px] flex flex-col">
                      ...
                      {founder?.description && (
                        <div className="w-full">
                          <p className="text-xs md:text-sm leading-relaxed mb-3 md:mb-4 w-full" style={{ color: "#7F543D" }}>
                            {founder.description}
                          </p>
                        </div>
                      )}
                        ...
                        <div className="pt-3 md:pt-4 border-t border-primary/10 mt-auto">
                          ...
                          <div className="flex flex-wrap gap-2 overflow-hidden">
                            {founderExpertise.map((e, i) => (
                              <span key={i} className="text-[11px] md:text-xs px-2 md:px-3 py-1 bg-primary/10 text-primary rounded-full max-w-full break-words">
                                {e}
                              </span>
                            ))}
                          </div>
              ...
              <div className="mb-12" id="awards">
                <div className="text-center mb-6 md:mb-10">
                  ...
                  <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                    Celebrating recognition for wellness excellence and trusted healthcare standards.
                  </p>
                </div>
                        ...
                        <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                          <div className="rounded-xl overflow-hidden">
                            <div className="relative w-full aspect-[800/600]">
                              <iframe
                                title="Sandhya Hot Spring Health Care Map"
                                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3411.03582538209!2d77.08708687375703!3d31.247431560579344!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x390573e262da23ed%3A0x401a3f258732a69a!2sSandhya%20Hot%20Spring%20Health%20Care!5e0!3m2!1sen!2sin!4v1774195707299!5m2!1sen!2sin"
                  ...
                  <div className="md:hidden">
                    <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                      ...
                      <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                        Begin Your Holistic Healing Journey at Sandhya
                      </h2>
                  ...
                  <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                    <div>
                      <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                        Begin Your <span className="text-white/90">Holistic Healing Journey</span> at <span className="text-white underline decoration-white/20 underline-offset-8">Sandhya</span>
                      </h2>
                    ...
                    <div>
                      <img
                        src="/Center Images/Sandhya Hot Spring Health Care/CTA bottom.jpg"
                        alt="Sandhya Hot Spring Health Care"
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
              Sandhya Hot Spring Health Care Facilities & Amenities
            </div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                ...
                alt={`Sandhya Facility ${facilityLightboxImage + 1}`}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <button
                onClick={() => setFacilityLightboxOpen(false)}

...
export default function IndusValleyAyurvedicCentre() {
  ...
  const [selectedSoukyaStyleVideo, setSelectedSoukyaStyleVideo] = useState(0);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Indus Valley Ayurvedic Centre/Wellness Programs.txt")
      ...
      .catch((err) => console.error("Error loading Indus Valley wellness content:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Indus Valley Ayurvedic Centre/Medical Programs.txt")
      ...
      .catch((err) => console.error("Error loading Indus Valley medical content:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Indus Valley Ayurvedic Centre/Why Choose Indus Valley.txt")
      ...
      .catch((err) => console.error("Error loading Indus Valley why-choose content:", err));
  }, []);
      ...
      .then((text) => {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            current = { number: steps.length + 1, title: line.slice(2, -2), description: "", bullets: [] };
            inSection = true;
            continue;
          }
          if (!inSection) {
      ...
      .then((text) => {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            const looksLikeFounder = /founder/i.test(t) || /^dr\b/i.test(t);
            if (looksLikeFounder && !founderName) {
              founderName = t;
              section = "founder";
              continue;
            }
          ...
          if (section === "founder") {
            founderDescription = founderDescription ? `${founderDescription} ${line}` : line;
            continue;
          }
        ...
        if (currentGroup) groups.push(currentGroup);
        setTeamIntro(intro);
        setFounder(founderName ? { name: founderName, degrees: founderDegrees, role: founderRole, description: founderDescription } : null);
        setFounderExpertise(expertise);
        setTeamGroups(groups);
      })
      .catch((err) => console.error("Error loading Indus founder content:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Indus Valley Ayurvedic Centre/Patient Stories & Reviews.txt")
      ...
      .catch((err) => console.error("Error loading Indus reviews:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Indus Valley Ayurvedic Centre/Insurance & Payment Info.txt")
      ...
      .then((text) => {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (t.includes("payment") || t.includes("booking")) {
              section = "pay";
              continue;
            }
            if (t.includes("international") || t.includes("guest")) {
              section = "intl";
              continue;
            }
          }
      ...
      .catch((err) => console.error("Error loading Indus insurance content:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Indus Valley Ayurvedic Centre/Frequently Asked Questions.txt")
      ...
      .catch((err) => console.error("Error loading Indus FAQs:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Indus Valley Ayurvedic Centre/Contact Information.txt")
      ...
      .catch((err) => console.error("Error loading Indus contact info:", err));
  }, []);
  ...
  const iconForTitle = (t: string) => {
    ...
    if (s.includes("anti-age") || s.includes("anti aging") || s.includes("anti-aging") || s.includes("rejuvenation") || s.includes("rasayana"))
    ...
    if (s.includes("yoga") || s.includes("meditation")) return <Users className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("arthritis") || s.includes("pain") || s.includes("rheumatism") || s.includes("back") || s.includes("neck"))
    ...
    if (s.includes("gastro") || s.includes("digest") || s.includes("ibs") || s.includes("acidity")) return <Pill className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("serene") || s.includes("environment") || s.includes("chamundi") || s.includes("mysore") || s.includes("mysuru")) return <MapPin className={cls} />;
    if (s.includes("legacy") || s.includes("award") || s.includes("excellence")) return <Award className={cls} />;
    if (s.includes("authentic") || s.includes("classical ayurveda") || s.includes("classical")) return <Stethoscope className={cls} />;
    if (s.includes("nature") || s.includes("eco") || s.includes("green") || s.includes("farm") || s.includes("organic")) return <Leaf className={cls} />;
    if (s.includes("physician") || s.includes("team") || s.includes("care") || s.includes("vaidyas") || s.includes("doctors")) return <Hospital className={cls} />;
    if (s.includes("herbal") || s.includes("medicine") || s.includes("food") || s.includes("farm-to-table") || s.includes("farm to table")) return <Sparkles className={cls} />;
    if (s.includes("panchakarma") || s.includes("detox")) return <Droplet className={cls} />;
    if (s.includes("integrated") || s.includes("yoga") || s.includes("meditation") || s.includes("holistic")) return <HeartPulse className={cls} />;
    if (s.includes("living") || s.includes("accommodation") || s.includes("stay") || s.includes("ashram")) return <Home className={cls} />;
    return <Heart className={cls} />;
  };

  const treatmentIconForStepNumber = (n: number) => {
    const cls = "h-5 w-5 md:h-6 md:w-6 text-primary";
    if (n === 1) return <ClipboardList className={cls} />;
    if (n === 2) return <FileSearch className={cls} />;
    if (n === 3) return <Pill className={cls} />;
    if (n === 4) return <Utensils className={cls} />;
    if (n === 5) return <Activity className={cls} />;
    return <Home className={cls} />;
  };

  const treatmentLabelForStepNumber = (n: number) => {
    if (n === 1) return "Day 1";
    if (n === 2) return "Day 1-2";
    if (n === 3) return "Ongoing";
    if (n === 4) return "Daily";
    if (n === 5) return "Throughout Stay";
  ...
  const thumbnailImages = (() => {
    ...
    for (let i = 0; i < images.length && result.length < 6; i++) {
      const url = images[i];
      if (!result.includes(url)) result.push(url);
    }
    return result;
  ...
  useEffect(() => {
    const videoElement = soukyaStyleVideoRef.current;
  ...
  }, [selectedSoukyaStyleVideo, videos.length]);
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-lg">Mysuru (Mysore), Karnataka</span>
                </div>
      ...
      <div className="container mx-auto px-3 md:px-4 py-12 max-w-full">
        <div className="max-w-6xl mx-auto">
          <div id="gallery" className="flex items-center mb-6 flex-wrap gap-3 md:gap-4">
          ...
          {!showVideoGallery ? (
            <>
              {images.length > 0 && thumbnailImages.length >= 5 ? (
                        ...
                        return (
                          ...
                          >
                            <div className="relative w-full rounded-xl overflow-hidden" style={{ paddingBottom: "100%" }}>
                              <img
                                src={img}
                                alt={`Indus Valley ${actualIndex + 1}`}
                ...
                <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] bg-muted" />
              )}
                  ...
                  <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {images.map((img, i) => (
                        ...
                        >
                          <img src={img} alt={`Indus Valley ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
                      <img
                        ...
                        alt={`Indus Valley ${lightboxImage + 1}`}
          ...
          <div id="wellness" className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }}>
          ...
          <div id="medical" className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }}>
            <div className="text-center mb-8">
                  ...
                  <CardContent className="p-0">
                    <div className="aspect-video w-full relative">
                      <video
                        ...
                        key={videos[selectedSoukyaStyleVideo]}
                        src={videos[selectedSoukyaStyleVideo]}
                ...
                {videos.length > 1 && (
                  <>
                    <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between px-2 md:-mx-8 pointer-events-none">
                      <button
                        onClick={() => setSelectedSoukyaStyleVideo((prev) => (prev - 1 + videos.length) % videos.length)}
                      ...
                      <button
                        onClick={() => setSelectedSoukyaStyleVideo((prev) => (prev + 1) % videos.length)}
                        className="bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary pointer-events-auto"
                    ...
                    <div className="flex justify-center gap-2 mt-6 md:mt-8">
                      {videos.map((_, index) => (
                        <button
                          key={index}
                          onClick={() => setSelectedSoukyaStyleVideo(index)}
                          className={`transition-all ${
                            index === selectedSoukyaStyleVideo
                              ? "w-8 h-3 bg-primary"
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">Why Choose Indus Valley for Your Healing Journey</h2>
                  ...
                  <CardContent className="p-0">
                    <div className="aspect-video w-full relative">
                      <iframe
                        ...
                        title="Indus Valley Testimonial Video"
              ...
              {treatmentSteps.map((s, idx) => (
                <div key={idx} className="relative flex items-start gap-3 md:gap-6 mb-8 md:mb-12 group">
                  <div className="hidden md:flex flex-col items-center flex-shrink-0">
                    ...
                    {idx < treatmentSteps.length - 1 && <div className="w-0.5 md:w-1 h-full bg-gradient-to-b from-primary to-primary/30 mt-2"></div>}
                  </div>
                    ...
                    <CardContent className="p-4 md:p-6">
                      ...
                      <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3 pl-10 md:pl-0">
                        <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-primary/10 flex items-center justify-center">
                          {treatmentIconForStepNumber(s.number)}
                        </div>
                        <div>
                          <h3 className="text-base md:text-xl font-bold text-primary">{s.title}</h3>
                          <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">{treatmentLabelForStepNumber(s.number)}</span>
                        </div>
                      </div>
                    ...
                    {facilityImages.map((image, index) => (
                      ...
                      >
                        ...
                        >
                          <img
                            src={image}
                            alt={`Indus Valley Facility ${index + 1}`}
                ...
                <div className="hidden md:block">
                  ...
                  >
                    {facilityImages.map((image, index) => (
                      ...
                      >
                        <div
                          className="bg-white rounded-xl p-2 shadow-lg border border-primary/10 hover:border-primary/30 transition-all"
                          onClick={() => {
                            setFacilityLightboxImage(index);
                            setFacilityLightboxOpen(true);
                          }}
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at Indus Valley is designed and maintained according to the highest standards,
          ...
          <div className="mb-12 rounded-3xl p-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="team">
            ...
            <div className="grid md:grid-cols-2 gap-4 md:gap-8 items-stretch">
              <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all">
                <CardContent className="p-4 md:p-8 h-full md:h-[480px] md:overflow-y-auto pb-8 md:pb-10">
                    ...
                    <div className="pt-3 md:pt-4 border-t border-primary/10">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {founderExpertise.map((e, i) => (
                          <span key={i} className="text-xs px-2 md:px-3 py-1 bg-primary/10 text-primary rounded-lg max-w-full break-words leading-snug">
                            {e}
                          </span>
                        ))}
                      </div>
          ...
          <div className="mb-12">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognitions highlighting our commitment to authentic Ayurveda, quality care, and guest experience
              </p>
            </div>
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
              Indus Valley Facilities & Amenities
            </div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                ...
                alt={`Indus Valley Facility ${facilityLightboxImage + 1}`}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <button
                onClick={() => setFacilityLightboxOpen(false)}
          ...
          <div className="p-4 pb-4 bg-[#2F5B63] text-white relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-3 relative z-10">
              <div className="space-y-0.5">
                ...
                <h2 className="text-[26px] font-extrabold leading-tight tracking-tight whitespace-nowrap text-white">Sections of Indus Valley</h2>
              </div>
              <button
                onClick={() => setIsJumpModalOpen(false)}

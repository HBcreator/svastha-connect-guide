...
const AyurSomaAyurvedaRoyalRetreat = () => {
  const [quoteModalOpen, setQuoteModalOpen] = useState(false);
  const [images, setImages] = useState<string[]>([]);
  const [videos, setVideos] = useState<string[]>([]);
  const [selectedImage, setSelectedImage] = useState(0);
  const [selectedVideo, setSelectedVideo] = useState(0);
  const [selectedBottomVideo, setSelectedBottomVideo] = useState(0);
  ...
  const founderImage = "/Center Images/AyurSoma Ayurveda/Founder/Founder.jpg";
  const teamImage = "/Center Images/AyurSoma Ayurveda/Founder/Team.webp";

  // Awards & Media section state
  const [currentAward, setCurrentAward] = useState(0);
  const [isAwardAutoPlaying, setIsAwardAutoPlaying] = useState(true);
  const [maxAwardIndex, setMaxAwardIndex] = useState(3);

  // Insurance & Payment Info state
  ...
  const awards = [
    {
      ...
      image: "/Center Images/AyurSoma Ayurveda/Award/Award 1 (NABH).png"
    },
    {
      title: "Green Leaf Certified",
      description: "Certified by the Kerala Tourism Department for eco-friendly practices and sustainable hospitality in Ayurvedic wellness tourism.",
      image: "/Center Images/AyurSoma Ayurveda/Award/Award 2 (GreenLeaf Kerala).png"
    },
    {
    ...
    {
      title: "Featured on The Luxury Channel",
      description: "Showcased on premium lifestyle platforms for offering world-class Ayurvedic experiences in a luxurious beachfront setting.",
      image: "/Center Images/AyurSoma Ayurveda/Award/Award 4 (The Luxury cahnnel).jpg"
    }
  ];

  // Jump sections for navigation
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose AyurSoma" },
  ...
  useEffect(() => {
    fetch("/Center Images/AyurSoma Ayurveda/Photo gallery/Photo Gallery Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
        let title = "";
    ...
    fetch("/Center Images/AyurSoma Ayurveda/Facilities/Facilities Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const imgs = text
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (/our\s+expert\s+medical\s+team/i.test(t)) {
          ...
          if (line.startsWith("*")) {
            ...
            if (section === "expertise") expertise.push(item.replace(/\*\*/g, "").trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (numberedBold) {
            if (current) items.push(current);
            current = { title: numberedBold[1].trim(), description: "", bullets: [] };
            inSteps = true;
            continue;
          }
  ...
  useEffect(() => {
    fetch("/Center Videos/AyurSoma Ayurveda/YT i frame testimonies.txt")
      .then((res) => res.text())
      .then((text) => {
        const srcs: string[] = [];
        const re = /src=\"([^\"]+)\"/g;
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Why Choose Ayursoma for Your Healing Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
        let title = "";
        ...
        for (const rawLine of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            if (current) cards.push(current);
            current = { title: line.slice(2, -2).trim(), description: "", bullets: [] };
            inCards = true;
            continue;
          }
        ...
        setWhyChooseData({
          title: title || "Why Choose AyurSoma",
          description,
          cards,
        });
      })
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  useEffect(() => {
    fetch("/Center Videos/AyurSoma Ayurveda/Video Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const vids = text
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  const wellnessIconForTitle = (title: string) => {
    ...
    if (t.includes("panchakarma") || t.includes("purification") || t.includes("shodhana")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-green-700" />;
    if (t.includes("rejuvenation") || t.includes("rasayana")) return <Zap className="h-4 w-4 md:h-5 md:w-5 text-green-700" />;
    if (t.includes("anti-stress") || t.includes("burnout") || t.includes("detox") || t.includes("mind")) return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-700" />;
    if (t.includes("weight")) return <Scale className="h-4 w-4 md:h-5 md:w-5 text-green-700" />;
  ...
  const medicalIconForTitle = (title: string) => {
    ...
    if (s.includes("skin") || s.includes("dermat") || s.includes("psoriasis")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("reproductive") || s.includes("gynec") || s.includes("gynaec") || s.includes("female")) {
      return <Users className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    }
    if (s.includes("pediatric") || s.includes("development") || s.includes("eye")) {
  ...
  const iconForTitle = (title: string) => {
    ...
    if (s.includes("pioneers") || s.includes("renowned") || s.includes("founder") || s.includes("expert") || s.includes("doctors") || s.includes("vaidya") || s.includes("vaidyas")) {
      return <UserCheck className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    }
    if (s.includes("nabh") || s.includes("accredited") || s.includes("clinical") || s.includes("standard") || s.includes("certification") || s.includes("recognition")) {
      return <Award className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    }
    if (s.includes("royalty") || s.includes("luminaries") || s.includes("trusted") || s.includes("global") || s.includes("community")) {
      return <Star className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    }
    if (s.includes("acre") || s.includes("organic") || s.includes("farm") || s.includes("nature") || s.includes("sustainable") || s.includes("environment") || s.includes("sanctuary") || s.includes("green") || s.includes("beach")) {
      return <TreePine className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    }
    if (s.includes("medicine") || s.includes("herb") || s.includes("oils") || s.includes("pharmacy") || s.includes("formulation") || s.includes("cuisine") || s.includes("diet")) {
      return <Pill className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
    }
    if (s.includes("rare") || s.includes("complex") || s.includes("conditions") || s.includes("healing") || s.includes("treatment") || s.includes("authenticity") || s.includes("lineage")) {
  ...
  const treatmentIconForTitle = (title: string) => {
    const s = title.toLowerCase();
    if (s.includes("consult") || s.includes("evaluation") || s.includes("diagn")) {
      return <ClipboardList className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("blueprint") || s.includes("plan") || s.includes("creating")) {
      return <FileSearch className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("detox") || s.includes("preparation") || s.includes("purvakarma") || s.includes("purification")) {
      return <Pill className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("core") || s.includes("cleansing") || s.includes("therap")) {
      return <Pill className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("nourish") || s.includes("rejuven") || s.includes("integration") || s.includes("diet")) {
      return <Utensils className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("empower") || s.includes("continued") || s.includes("wellness") || s.includes("home") || s.includes("plan")) {
  ...
  const getFacilityIcon = (title: string) => {
    ...
    if (s.includes("accommodation") || s.includes("room") || s.includes("suite") || s.includes("stay") || s.includes("villa") || s.includes("private")) {
    ...
    if (s.includes("services") || s.includes("housekeeping") || s.includes("laundry") || s.includes("guest") || s.includes("transfer") || s.includes("support") || s.includes("wifi")) {
  ...
  // Handle resize for awards carousel
  useEffect(() => {
    const handleResize = () => {
  ...
  // Load Insurance & Payment Info
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  // Load FAQ
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/AyurSoma Ayurveda/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        const addr: string[] = [];
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("*")) {
            const bullet = line.replace(/^\*+\s*/, "");
            if (section === "distances") dists.push(bullet);
            continue;
          }
  ...
  useEffect(() => {
    ...
    return () => clearInterval(id);
  }, [images.length, isAutoPlaying, showTopVideoGallery, lightboxOpen, showFullGallery]);
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              AYURSOMA AYURVEDA ROYAL RETREAT KERALA
            </li>
          </ol>
        </div>
      </nav>
        ...
        <div className="container mx-auto px-3 md:px-4 max-w-full">
          <div className="max-w-6xl mx-auto">
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                <h1 className="text-4xl md:text-5xl font-bold mb-4">AyurSoma</h1>
                <p className="text-xl mb-4 opacity-90">AyurSoma Ayurveda Royal Retreat</p>
        ...
        <div className="max-w-6xl mx-auto">
                ...
                <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                  {images.length > 0 && (
                    <img
                      ...
                      alt={`AyurSoma Center ${selectedImage + 1}`}
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/AyurSoma Ayurveda/Main content.txt"
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">Video Gallery of AyurSoma</h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at AyurSoma through our video gallery.
              </p>
            </div>
            ...
            <div className="relative max-w-4xl mx-auto">
                ...
                <CardContent className="p-0">
                  <div className="aspect-video w-full relative">
                    {videos.length > 0 && (
                      <video
                        ref={galleryVideoRef}
                        key={videos[selectedBottomVideo]}
                        src={videos[selectedBottomVideo]}
                        className="w-full h-full object-cover"
                        controls
                        playsInline
                      />
              ...
              <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between px-2 md:-mx-8 pointer-events-none">
                <button
                  onClick={() => setSelectedBottomVideo((prev) => (prev - 1 + videos.length) % videos.length)}
                ...
                <button
                  onClick={() => setSelectedBottomVideo((prev) => (prev + 1) % videos.length)}
                  className="bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary pointer-events-auto"
              ...
              <div className="flex justify-center gap-2 mt-6 md:mt-8">
                {videos.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => setSelectedBottomVideo(index)}
                    className={`transition-all ${index === selectedBottomVideo ? "w-8 h-3 bg-primary" : "w-3 h-3 bg-gray-300 hover:bg-primary/50"} rounded-full`}
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {whyChooseData?.title || "Why Choose AyurSoma"}
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">Testimonials of AyurSoma Center</h2>
                  ...
                  <div className="aspect-video w-full relative">
                    {testimonialVideos.length > 0 && (
                      <iframe
                        ...
                        title="AyurSoma Testimonial Video"
          ...
          <div className="mb-12" id="process">
            ...
            <div className="max-w-4xl mx-auto">
              {treatmentSteps.map((step, idx) => {
                ...
                return (
                  <div key={idx} className={`relative flex items-start gap-3 md:gap-6 ${isLast ? "group" : "mb-8 md:mb-12 group"}`}>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/AyurSoma Ayurveda/CTA mid.jpg"
                    alt="AyurSoma Wellness Center"
                  ...
                  <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentFacilityImage * 100}%)` }}>
                    {facilityImages.map((image, index) => (
                      <div key={index} className="w-full flex-shrink-0 px-2">
                        ...
                        >
                          <img src={image} alt={`AyurSoma Facility ${index + 1}`} className="w-full aspect-video object-cover rounded-lg" />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <Card key={idx} className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-t-4 border-t-primary">
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4 mb-3">
                      ...
                      <h3 className="text-lg md:text-xl font-bold text-primary leading-tight flex-1 min-h-[2.5rem] flex items-center">{card.title}</h3>
                    </div>
            ...
            <div className="mt-8 p-6 bg-primary/5 rounded-xl border-l-4 border-l-primary">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at AyurSoma is designed and maintained to support safety, hygiene, and quality care,
                    ensuring you receive world-class holistic treatment in a serene, naturally therapeutic environment.
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="AyurSoma Ayurveda Royal Retreat Map"
                            src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d7894.928183755647!2d77.0133507!3d8.3559224!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3b05a5ccb42b8855%3A0xd05610dd0b19d609!2sAyurSoma%20Ayurveda%20Royal%20Retreat!5e0!3m2!1sen!2sin!4v1770892952317!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/AyurSoma Ayurveda/CTA bottom.jpg"
                    alt="AyurSoma Ayurveda Royal Retreat"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">Begin Your Holistic Healing Journey at AyurSoma</h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    Begin Your <span className="text-white/90">Holistic Healing Journey</span> at <span className="text-white underline decoration-white/20 underline-offset-8">AyurSoma</span>
                  </h2>
          ...
          <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {images.map((img, idx) => (
                ...
                >
                  <img src={img} alt={`AyurSoma Ayurveda ${idx + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                </div>
              ))}
            </div>
          </div>
      ...
      {lightboxOpen && images.length > 0 && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">AyurSoma Ayurveda</div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img src={images[lightboxImage]} alt={`AyurSoma Ayurveda ${lightboxImage + 1}`} className="absolute inset-0 w-full h-full object-cover" />
              <button
                onClick={() => setLightboxOpen(false)}
      ...
      {facilityLightboxOpen && facilityImages.length > 0 && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">AyurSoma Facilities</div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                src={facilityImages[facilityLightboxImage]}
                alt={`AyurSoma Facility ${facilityLightboxImage + 1}`}

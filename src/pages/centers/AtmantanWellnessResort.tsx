...
import { MapPin, Star, Calendar, ChevronLeft, ChevronRight, Images, Video, Users, TrendingUp, Heart, Stethoscope, Droplet, Brain, Sparkles, Activity, ShieldCheck, Leaf, Hospital, HeartPulse, TreePine, UserCheck, Award, ClipboardList, FileSearch, Pill, Apple, Dumbbell, GraduationCap, Phone, MessageCircle, Home, Globe, MessageCircleHeart, Search, X } from "lucide-react";
...
const parseCardSection = (text: string): SectionData => {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("**") && line.endsWith("**")) {
      ...
      currentCard = { title: line.slice(2, -2).trim(), description: "", bullets: [] };
      continue;
    }
    if (line.startsWith("*")) {
...
const iconForTitle = (t: string) => {
  ...
  if (s.includes("atma") || s.includes("mana") || s.includes("tann")) return <Heart className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  if (s.includes("founder") || s.includes("vision")) return <UserCheck className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  if (s.includes("integrated") || s.includes("sciences")) return <Hospital className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  if (s.includes("crystal") || s.includes("valley") || s.includes("nature")) return <TreePine className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  if (s.includes("award") || s.includes("excellence")) return <Star className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  if (s.includes("result") || s.includes("evidence")) return <TrendingUp className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  if (s.includes("cuisine") || s.includes("nutrition")) return <Leaf className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  if (s.includes("luxurious") || s.includes("inclusive")) return <Users className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
  return <Heart className="h-6 w-6 text-primary group-hover:text-white transition-colors" />;
};

export default function AtmantanWellnessResort() {
  ...
  const [treatmentData, setTreatmentData] = useState<SectionData | null>(null);
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Atmantan" },
  ...
  const awards = [
    {
      title: "NABH Certified Wellness Institute",
      image: "/Center Images/Atmantan Wellness Resort/Awards/1 (Awards).png",
      description: "Recognized as a NABH certified wellness institute, reflecting quality standards in care and safety.",
    },
    {
      title: "Seven Stars Luxury Hospitality & Lifestyle Awards",
      image: "/Center Images/Atmantan Wellness Resort/Awards/2 (starsHospitalityAwards-logo).png",
      description: "A global luxury hospitality awards program celebrating exceptional destinations and experiences.",
    },
    {
      title: "Condé Nast Traveller Readers’ Travel Awards 2024",
      image: "/Center Images/Atmantan Wellness Resort/Awards/3 (Conde Nast Traveller Reader's Travel Awards).webp",
      description: "Winner of Favourite Destination Spa 2024 in Condé Nast Traveller Readers’ Travel Awards.",
    },
  ];

  useEffect(() => {
    fetch("/Center Images/Atmantan Wellness Resort/Photo Gallery/Photo Gallery links.txt")
      .then((r) => r.text())
      .then((t) => {
        const urls = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/Center Videos/Atmantan Wellness Resort/Video Gallery Links.txt")
      .then((r) => r.text())
      .then((t) => {
        const urls = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Atmantan wellness programs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Atmantan medical programs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Why Choose Atmantan.txt")
      .then((res) => res.text())
      .then((text) => setWhyChooseData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Why Choose Atmantan:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Videos/Atmantan Wellness Resort/Testimonies/Testimonies links.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Atmantan testimonials:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => setTreatmentData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Atmantan treatment process:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => setFacilitiesData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Atmantan facilities:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Images/Atmantan Wellness Resort/Facilities/Facilities Links.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Atmantan facility images:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        const nonEmpty = (l: string) => l && l.trim().length > 0;
        const headingIdx = lines.findIndex((l) => l.startsWith("### "));
        const founderIdx = lines.findIndex((l, i) => i > headingIdx && l.startsWith("**") && l.endsWith("**"));
        const leadershipIdx = lines.findIndex((l) => l.toLowerCase().includes("leadership & expertise"));
        const teamIdx = lines.findIndex((l) => l.toLowerCase().includes("our expert leadership team"));

        const introLines = lines.slice(headingIdx + 1, founderIdx).filter(nonEmpty);
        const intro = introLines.join(" ");

        const founderName = founderIdx >= 0 ? lines[founderIdx].slice(2, -2).trim() : "";
        const founderRole = lines.slice(founderIdx + 1).find(nonEmpty) || "";

        const founderDescLines = lines
          .slice(founderIdx + 2, leadershipIdx > 0 ? leadershipIdx : undefined)
          .filter((l) => nonEmpty(l) && !l.startsWith("*"));
        const founderDesc = founderDescLines.join(" ");

        const expertise = lines
          .slice(leadershipIdx + 1, teamIdx > 0 ? teamIdx : undefined)
          .filter((l) => l.trim().startsWith("*"))
          .map((l) => l.replace(/^\*+\s*/, "").trim());

        const teamTitle = teamIdx >= 0 ? lines[teamIdx].slice(2, -2).trim() : "";
        const afterTeam = lines.slice(teamIdx + 1).filter(nonEmpty);
        const teamSubtitle = afterTeam[0] || "";
        const teamDescLines: string[] = [];
        const teamItems: string[] = [];
        let inBullets = false;
        for (const line of afterTeam.slice(1)) {
          if (line.startsWith("*")) {
            inBullets = true;
            teamItems.push(line.replace(/^\*+\s*/, "").trim());
          } else if (!inBullets) {
            teamDescLines.push(line);
          }
        }
        const teamDesc = teamDescLines.join(" ");

        setTeamIntro(intro);
        setFounder(founderName ? { name: founderName, role: founderRole, description: founderDesc } : null);
        setFounderExpertise(expertise);
        const combinedDesc = `${teamSubtitle} ${teamDesc}`.trim();
        setTeamGroups(teamTitle ? [{ title: teamTitle, description: combinedDesc, items: teamItems }] : []);
        setCurrentTeamSlide(0);
      })
      .catch((err) => console.error("Error loading Atmantan team info:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Atmantan reviews:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        let currentSection: "none" | "payment" | "claims" | "international" = "none";
        let paymentTitle = "";
        let claimsTitle = "";
        let internationalTitle = "";
        let paymentIntro = "";
        let claimsIntro = "";
        let internationalIntro = "";
        const paymentBullets: string[] = [];
        const claimsBullets: string[] = [];
        const internationalBullets: string[] = [];
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (title.toLowerCase().includes("payment") || title.toLowerCase().includes("booking")) {
              paymentTitle = title;
              currentSection = "payment";
            } else if (title.toLowerCase().includes("insurance") || title.toLowerCase().includes("claims")) {
              claimsTitle = title;
              currentSection = "claims";
            } else if (title.toLowerCase().includes("international")) {
              internationalTitle = title;
          ...
          if (line.startsWith("*")) {
            const bullet = line.replace(/^\*+\s*/, "");
            if (currentSection === "payment") paymentBullets.push(bullet);
            if (currentSection === "claims") claimsBullets.push(bullet);
            if (currentSection === "international") internationalBullets.push(bullet);
            continue;
          }
          if (currentSection === "none") {
            intro = intro ? `${intro} ${line}` : line;
          } else if (currentSection === "payment") {
            paymentIntro = paymentIntro ? `${paymentIntro} ${line}` : line;
          } else if (currentSection === "claims") {
            claimsIntro = claimsIntro ? `${claimsIntro} ${line}` : line;
          } else if (currentSection === "international") {
            internationalIntro = internationalIntro ? `${internationalIntro} ${line}` : line;
          }
        }
        ...
        setInsuranceInternationalTitle(internationalTitle);
        setInsuranceInternationalIntro(internationalIntro);
        setInsuranceInternationalBullets(internationalBullets);
      })
      .catch((err) => console.error("Error loading Atmantan insurance info:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            if (current) items.push(current);
            current = { question: line.slice(2, -2), answer: "", bullets: [] };
            continue;
          }
          if (line.startsWith("*")) {
      ...
      .catch((err) => console.error("Error loading Atmantan FAQ:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Atmantan Wellness Resort/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (section === "transport") {
            if (line.startsWith("*")) {
              transport = transport ? `${transport} ${line.replace(/^\*+\s*/, "")}` : line.replace(/^\*+\s*/, "");
            } else {
      ...
      .catch((err) => console.error("Error loading Atmantan contact info:", err));
  }, []);
  ...
  useEffect(() => {
    const update = () => {
      ...
      setCurrentAward((prev) => (prev > Math.max(0, awards.length - (mobile ? 1 : 3)) ? 0 : prev));
    };
    update();
  ...
  useEffect(() => {
    ...
    return () => window.clearInterval(id);
  }, [isFacilityAutoPlaying, facilityImages.length]);

  const wellnessIconForTitle = (t: string) => {
    const s = t.toLowerCase();
    if (s.includes("detox") || s.includes("cleanse") || s.includes("panchakarma")) return <Droplet className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("stress") || s.includes("mind") || s.includes("soul")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("rejuvenation") || s.includes("radiance")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("fitness") || s.includes("challenge")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("immunity")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("yoga")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  };

  const medicalIconForTitle = (t: string) => {
    const s = t.toLowerCase();
    if (s.includes("diabetes") || s.includes("hypertension") || s.includes("heart") || s.includes("cardiac")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("pain") || s.includes("physio") || s.includes("rehab")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("sleep") || s.includes("insomnia")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("gut") || s.includes("digestive")) return <TreePine className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("women") || s.includes("hormonal")) return <Users className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("panchakarma") || s.includes("ayurveda")) return <Hospital className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    return <Stethoscope className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  };

  const getFacilityIcon = (t: string) => {
    const s = t.toLowerCase();
    if (s.includes("accommodation") || s.includes("villa") || s.includes("room")) return <Home className="h-6 w-6 text-white" />;
    if (s.includes("pavilion") || s.includes("spa") || s.includes("therapy")) return <HeartPulse className="h-6 w-6 text-white" />;
    if (s.includes("fitness") || s.includes("gym") || s.includes("training")) return <Activity className="h-6 w-6 text-white" />;
    if (s.includes("dining") || s.includes("cuisine") || s.includes("nutrition")) return <Leaf className="h-6 w-6 text-white" />;
    if (s.includes("hydro") || s.includes("steam") || s.includes("sauna")) return <Droplet className="h-6 w-6 text-white" />;
    if (s.includes("yoga") || s.includes("meditation") || s.includes("mind")) return <Brain className="h-6 w-6 text-white" />;
    if (s.includes("pool")) return <Droplet className="h-6 w-6 text-white" />;
    if (s.includes("physio") || s.includes("rehab")) return <Stethoscope className="h-6 w-6 text-white" />;
    if (s.includes("landscape") || s.includes("amphitheatre") || s.includes("garden")) return <TreePine className="h-6 w-6 text-white" />;
    ...
    return (
      <div className="flex gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star
            key={i}
            className={`h-4 w-4 md:h-5 md:w-5 ${i < rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`}
          />
        ))}
      </div>
    );
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Luxury wellness retreat in the Sahyadri hills overlooking Mulshi Lake
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Mulshi, Near Pune, Maharashtra, India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.7</span>
                  <span className="opacity-90">(3000+)</span>
                </div>
              </div>
        ...
        <div className="max-w-6xl mx-auto">
                ...
                <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                  {images.length > 0 && (
                    <img
                      ...
                      alt={`Atmantan Wellness Resort ${selectedImage + 1}`}
                  ...
                  <div className="flex flex-col md:flex-row gap-3 mb-6">
                    ...
                    >
                      <img
                        ...
                        alt="Atmantan Wellness Resort 1"
                        ...
                        return (
                          ...
                          >
                            <div className="relative w-full rounded-xl overflow-hidden" style={{ paddingBottom: "100%" }}>
                              <img
                                src={img}
                                alt={`Atmantan Wellness Resort ${actualIndex + 1}`}
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/Atmantan Wellness Resort/Main Content.txt"
          ...
          <div className="mb-12 rounded-3xl px-6 py-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-base md:text-3xl font-bold text-primary mb-1 whitespace-nowrap">3000+</div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Atmantan Wellness Resort
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at Atmantan through our video gallery.
              </p>
            </div>
          ...
          {/* Why Choose Atmantan Wellness Resort */}
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                {whyChooseData?.title || "Why Choose Atmantan Wellness Resort for Your Healing Journey?"}
              </h2>
          ...
          {/* Testimonials of Atmantan Wellness Resort - Video Section */}
          <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Atmantan Wellness Resort
              </h2>
              <div className="w-12 h-1 bg-primary/20 mx-auto mb-3 rounded-full hidden md:block" />
              <p className="text-sm md:text-lg mx-auto max-w-none leading-relaxed italic" style={{ color: "#7F543D" }}>
                Watch inspiring stories of recovery and wellness from Atmantan’s guests and global community.
              </p>
            </div>
                  ...
                  <div className="aspect-video w-full relative">
                    {testimonialVideos.length > 0 && (
                      <iframe
                        ...
                        title="Atmantan Testimonial Video"
          ...
          {/* Ready to Start Your Wellness Journey - CTA */}
          <div className="mb-12" id="cta">
            <div className="bg-[#EDE8D0] rounded-3xl px-5 py-6 md:px-8 md:py-10">
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/90 rounded-2xl p-4 shadow-lg border-2 border-white/60">
                  <img
                    src="/Center Images/Atmantan Wellness Resort/CTA mid.jpg"
                    alt="Atmantan Wellness Resort"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h3 className="text-xl font-extrabold text-primary text-center mb-3 leading-tight tracking-tight">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-lg font-semibold text-primary mb-2">
                    Designed to International Wellness Standards
                  </h4>
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at Atmantan is crafted to deliver world-class care, safety, and comfort, ensuring a
                    deeply restorative environment across your entire wellness journey.
                  </p>
                </div>
              </div>
            </div>
              ...
              <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all hover:shadow-xl h-full">
                <CardContent className="p-4 md:p-8 h-full md:h-[480px] flex flex-col">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    <div className="p-[3px] rounded-full flex-shrink-0 shadow-2xl" style={{ background: "conic-gradient(from 45deg, #F0E68C, #B8860B, #FFD700, #B8860B, #F0E68C)" }}>
                      <div className="flex -space-x-3 md:-space-x-4 px-1 py-1">
                        <div className="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-[2px] border-white bg-white">
                          <img
                            src="/Center Images/Atmantan Wellness Resort/Founder/Founder.jpg"
                            alt="Sharmilee Kapur"
                            className="w-full h-full object-cover"
                          />
                        </div>
                        <div className="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-[2px] border-white bg-white">
                          <img
                            src="/Center Images/Atmantan Wellness Resort/Founder/Founder Nikhil_Kapur.jpeg"
                            alt="Nikhil Kapur"
                            className="w-full h-full object-cover"
                          />
                        </div>
                    ...
                    <div>
                      ...
                      {founder?.role && (
                        <p className="text-xs md:text-sm font-semibold" style={{ color: "#7F543D" }}>{founder.role}</p>
                      )}
                    </div>
                  </div>
                  {founder?.description && (
                    ...
                    <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                      <div className="p-[3px] rounded-full flex-shrink-0 shadow-2xl aspect-square" style={{ background: "conic-gradient(from 45deg, #F0E68C, #B8860B, #FFD700, #B8860B, #F0E68C)" }}>
                        <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                          <img
                            src="/Center Images/Atmantan Wellness Resort/Founder/Team.webp"
                            alt="Team"
                            className="w-full h-full object-cover"
                          />
                        </div>
          ...
          <div className="mt-16 md:mt-24 mb-16 md:mb-24" id="awards">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg px-4 mx-auto max-w-2xl" style={{ color: "#7F543D" }}>
                Recognitions that spotlight Atmantan’s excellence in wellness, hospitality, and guest experience.
              </p>
            </div>
      ...
      <div className="container mx-auto px-3 md:px-4 py-12 max-w-full">
        <div className="max-w-6xl mx-auto">
              ...
              <Card className="mt-6 bg-primary/5 border-l-4 border-l-primary">
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="text-lg font-semibold text-primary mb-2">
                        {insuranceInternationalTitle || "For International Guests"}
                      </h4>
                ...
                >
                  ...
                  <AccordionContent className="pt-4 pb-6 bg-white">
                    ...
                    {faq.bullets.length > 0 && (
                      <ul className="space-y-2 text-sm leading-relaxed mt-3" style={{ color: "#7F543D" }}>
          ...
          <div className="mb-12" id="begin-journey">
            <div className="bg-[#2F5B63] rounded-3xl p-6 md:p-10">
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Atmantan Wellness Resort/CTA bottom.jpg"
                    alt="Atmantan Wellness Resort"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                    Begin Your Holistic Healing Journey at Atmantan
                  </h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    Begin Your <span className="text-white/90">Holistic Healing Journey</span> at <span className="text-white underline decoration-white/20 underline-offset-8">Atmantan</span>
                  </h2>
          ...
          <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {images.map((img, idx) => (
                ...
                >
                  <img src={img} alt={`Atmantan Wellness Resort ${idx + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              {images.length > 0 && (
                <img
                  ...
                  alt={`Atmantan Wellness Resort ${lightboxImage + 1}`}
      ...
      {facilityLightboxOpen && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
              Atmantan Facilities & Amenities
            </div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              {facilityImages.length > 0 && (
                <img
                  src={facilityImages[facilityLightboxImage]}
                  alt={`Atmantan Facility ${facilityLightboxImage + 1}`}
                  className="absolute inset-0 w-full h-full object-cover"
                />
              )}
              <button

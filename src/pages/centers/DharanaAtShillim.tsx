...
export default function DharanaAtShillim() {
  ...
  const founderImage = "/Center Images/Dharana At Shillim/Founder/Founder.png";
  const teamImage = "/Center Images/Dharana At Shillim/Founder/team.jpg";
  ...
  const awards = [
    {
      ...
      description: "Named among the Top 20 Destination Spas in the World in the 2025 Readers’ Choice Awards.",
      image: "/Center Images/Dharana At Shillim/Awards/1 (Condé Nast Traveler Readers' Choice Awards 2025).png",
    },
    {
      title: "World Spa Awards 2024",
      description: "Nominated for India’s Best Wellness Retreat at the World Spa Awards 2024.",
      image: "/Center Images/Dharana At Shillim/Awards/2 (world global spa ).png",
    },
    {
      title: "World Luxury Spa Awards",
      description: "Recognized among the world’s leading luxury spa destinations.",
      image: "/Center Images/Dharana At Shillim/Awards/3 (world luxury spa).png",
    },
  ];
  ...
  useEffect(() => {
    fetch("/Center Images/Dharana At Shillim/Photo Gallery/Photo Gallery Links.txt")
      .then((r) => r.text())
      .then((t) => {
        const list = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/Center Videos/Dharana At Shillim/VIdeo Gallery links.txt")
      .then((r) => r.text())
      .then((t) => {
        const list = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Dharana wellness content:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Dharana medical content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Why Choose Dharana.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Dharana why-choose content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Dharana treatment process content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
      ...
      .catch((err) => console.error("Error loading Dharana facilities content:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Images/Dharana At Shillim/Facilities/Facilities.txt")
      .then((res) => res.text())
      .then((text) => {
        const imgs = text
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (/core expertise/i.test(t) || /leadership & expertise/i.test(t)) { section = "expertise"; continue; }
      ...
      .catch((err) => console.error("Error loading Dharana founder content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Dharana reviews:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Dharana insurance content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Dharana FAQs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Dharana At Shillim/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Dharana contact info:", err));
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const newMax = isMobile ? awards.length - 1 : Math.max(Math.min(awards.length - 1, 2), 0);
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Dharana" },
  ...
  const getFacilityIcon = (title: string) => {
    ...
    return t.includes("ayurveda") || t.includes("treatment") ? <Droplet className="h-7 w-7 text-white" />
      : t.includes("doctor") ? <Stethoscope className="h-7 w-7 text-white" />
        : t.includes("cottage") || t.includes("room") || t.includes("accommodation") || t.includes("villa") ? <Building2 className="h-7 w-7 text-white" />
          : t.includes("restaurant") || t.includes("dining") || t.includes("meal") || t.includes("food") ? <Utensils className="h-7 w-7 text-white" />
            : t.includes("pool") || t.includes("recreation") || t.includes("swimming") ? <Activity className="h-7 w-7 text-white" />
              : t.includes("conference") || t.includes("group") || t.includes("institute") || t.includes("school") ? <Globe className="h-7 w-7 text-white" />
                : t.includes("travel") || t.includes("airport") || t.includes("station") ? <MapPin className="h-7 w-7 text-white" />
                  : t.includes("laundry") || t.includes("housekeeping") ? <Sparkles className="h-7 w-7 text-white" />
                    : t.includes("library") || t.includes("reading") ? <Globe className="h-7 w-7 text-white" />
                      : t.includes("garden") || t.includes("nature") || t.includes("wilderness") || t.includes("landscape") ? <TreePine className="h-7 w-7 text-white" />
                        : t.includes("steam") || t.includes("therapy") || t.includes("hydro") ? <Droplet className="h-7 w-7 text-white" />
  ...
  const testimonialVideos = [
    "/Center Videos/Dharana At Shillim/Testimonies/Testimonies 1.mp4",
    "/Center Videos/Dharana At Shillim/Testimonies/Testimonies 2.mp4",
    "/Center Videos/Dharana At Shillim/Testimonies/Testimonies 3.mp4",
  ];
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              DHARANA AT SHILLIM WELLNESS RETREAT PUNE
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Holistic Wellness & Mindful Living Retreat
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Shillim, Pune, Maharashtra, India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.8</span>
                  <span className="opacity-90">(3900+ reviews)</span>
                </div>
              </div>
                ...
                <>
                  <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                    <img
                      ...
                      alt={`Dharana At Shillim ${selectedImage + 1}`}
                      ...
                      <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                          {images.map((img, i) => (
                            ...
                            >
                              <img src={img} alt={`Dharana ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        <div className="max-w-6xl mx-auto">
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg">
              <MarkdownContent
                contentPath="/content/Top Centers/Dharana At Shillim/Main content.txt"
      ...
      <div className="container mx-auto px-3 md:px-4 pb-2 max-w-full">
        <div className="max-w-6xl mx-auto">
          <div className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-base md:text-3xl font-bold text-primary mb-1 whitespace-nowrap">3900+</div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Dharana At Shillim
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Explore the atmosphere and wellness experience at Dharana At Shillim through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">Why Choose Dharana At Shillim for Your Healing Journey</h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Dharana At Shillim
              </h2>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Dharana At Shillim/CTA mid.jpg"
                    alt="Dharana At Shillim"
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognition for excellence in holistic wellness, clinical depth, and guest experience.
              </p>
            </div>
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Dharana At Shillim Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1935815.216193799!2d73.46103819999999!3d18.6240575!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3be81db16a99b875%3A0x9b643b497c360b80!2sDharana%20at%20Shillim!5e0!3m2!1sen!2sin!4v1773319450468!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Dharana At Shillim/CTA bottom.jpg"
                    alt="Dharana At Shillim"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at Dharana At Shillim</h2>
                  <div className="space-y-3">
                    <Button
                      size="lg"
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-bold text-white mb-4">Begin Your Holistic Healing Journey at Dharana At Shillim</h2>
                  <div className="flex flex-wrap gap-4">
                    ...
                    <Button
                      size="lg"
                      variant="outline"
                      className="rounded-full border-2 border-white/60 bg-transparent text-white hover:bg-orange-500 hover:border-orange-500 active:bg-orange-500 active:border-orange-500"
                      onClick={() => setQuoteModalOpen(true)}
                    >
                      <MessageCircle className="mr-2 h-5 w-5" />
                      Chat With Us
                ...
                <div className="relative">
                  <img
                    src="/Center Images/Dharana At Shillim/CTA bottom.jpg"
                    alt="Dharana At Shillim"
                    className="w-full h-auto rounded-2xl shadow-xl border-2 border-white/30 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                </div>
              </div>
            </div>
      ...
      {facilityLightboxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#EDE8D0]/80 backdrop-blur-sm">
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Dharana At Shillim</div>
            <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-2.5 custom-scrollbar">
            {jumpSections.map((section, idx) => (
              <button
                ...
                className={`w-full group relative transition-all duration-300 p-3 rounded-xl border-2 flex items-center justify-between shadow-md hover:shadow-xl ${
                  activeSectionId === section.id
                    ? "bg-[#2F5B63] border-[#2F5B63]"
                    : "bg-white border-primary/20 hover:border-primary hover:bg-[#2F5B63]"
                }`}
              >
                <div className="flex items-center gap-4 relative z-10">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-200 ${
                    activeSectionId === section.id ? "bg-white/10" : "bg-primary/5 group-hover:bg-white/10"
                  }`}>
                    <span className={`text-xs font-black transition-all duration-200 ${
                      activeSectionId === section.id ? "text-white" : "text-primary group-hover:text-white"
                    }`}>
                      {(idx + 1).toString().padStart(2, "0")}
                    </span>
                  </div>
                  <span className={`text-sm md:text-base font-bold transition-all duration-200 text-left ${
                    activeSectionId === section.id ? "text-white" : "text-primary group-hover:text-white"
                  }`}>
                    {section.title}
                  </span>
                ...
                <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200 ${
                  activeSectionId === section.id ? "bg-white/20" : "group-hover:bg-white/20"
                }`}>
                  <ChevronRight className={`h-3.5 w-3.5 transition-all duration-200 ${
                    activeSectionId === section.id ? "text-white" : "text-primary group-hover:text-white group-hover:translate-x-0.5"
                  }`} />
                </div>
              </button>
            ))}

...
export default function KumarakomLakeResort() {
  ...
  const [maxAwardIndex, setMaxAwardIndex] = useState(4);
  ...
  const founderImage = "/Center Images/kumarakom lake resort/Founder/Founder.jpg";
  const teamImage = "/Center Images/kumarakom lake resort/Founder/team.jpg";
  const awards = [
    {
      title: "World Travel Awards Winner",
      description: "Recognized for outstanding hospitality and destination experience.",
      image: "/Center Images/kumarakom lake resort/Awards/Award 1 (World Travel Awards).jpg",
    },
    {
      title: "India's Leading Green Hotel",
      description: "Honored for eco-conscious operations and sustainable luxury standards.",
      image: "/Center Images/kumarakom lake resort/Awards/Award 2 (India's Leading Green Hotel).png",
    },
    {
      title: "Top Five-Star Hospitality",
      description: "Celebrated for premium service quality and guest satisfaction excellence.",
      image: "/Center Images/kumarakom lake resort/Awards/Award 3 (India's Best Five Star Hotel).jpg",
    },
    {
      title: "Featured by Conde Nast Traveler",
      description: "Acknowledged as a distinguished luxury and heritage destination.",
      image: "/Center Images/kumarakom lake resort/Awards/Award 4 (Cond%C3%A9 Nast Traveler).png",
    },
    {
      title: "NABH Accredited",
      description: "Trusted accreditation for quality standards in wellness and care.",
      image: "/Center Images/kumarakom lake resort/Awards/Award 5 (NABH).png",
    },
  ];
  const testimonialVideos = [
    "/Center Videos/kumarakom lake resort/testimonies/testimonies 1.mp4",
    "/Center Videos/kumarakom lake resort/testimonies/testimonies 2.mp4",
    "/Center Videos/kumarakom lake resort/testimonies/testimonies 3.mp4",
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Kumarakom" },
  ...
  useEffect(() => {
    fetch("/Center Images/kumarakom lake resort/Photo Gallery/Photo Gallery Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
  ...
  useEffect(() => {
    fetch("/Center Videos/kumarakom lake resort/VIdeo Gallery Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
  ...
  useEffect(() => {
    fetch("/Center Images/kumarakom lake resort/Facilities/Facilities Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
  ...
  useEffect(() => {
    ...
    setLightboxImage((prev) => (prev >= images.length ? 0 : prev));
  }, [images.length]);

  useEffect(() => {
    if (!facilityImages.length) return;
    setCurrentFacilityImage((prev) => (prev >= facilityImages.length ? 0 : prev));
    setFacilityLightboxImage((prev) => (prev >= facilityImages.length ? 0 : prev));
  }, [facilityImages.length]);

  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
  ...
  const iconForTitle = (t: string, idx = 0) => {
    ...
    const fallbackIcons = [
      ...
      <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-green-600" />,
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Medical Treatment Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  const medicalIconForTitle = (t: string, idx = 0) => {
    const s = t.toLowerCase();
    const fallbackIcons = [
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Why Choose Kumarakom Lake Resort.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("award") || s.includes("royal") || s.includes("globally")) return <Award className={cls} />;
    if (s.includes("history") || s.includes("heritage")) return <Home className={cls} />;
    if (s.includes("lake") || s.includes("backwater")) return <MapPin className={cls} />;
    if (s.includes("ayurveda") || s.includes("ayurmana")) return <Stethoscope className={cls} />;
    if (s.includes("luxury") || s.includes("five-star")) return <Hospital className={cls} />;
    if (s.includes("cuisine") || s.includes("gourmet")) return <Sparkles className={cls} />;
    if (s.includes("immersion") || s.includes("culture")) return <Leaf className={cls} />;
    if (s.includes("privacy") || s.includes("serenity")) return <ShieldCheck className={cls} />;
    return <Heart className={cls} />;
  };

  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (/leadership\s*&\s*expertise/i.test(t)) { section = "expertise"; continue; }
            if (/our expert leadership\s*&\s*wellness team/i.test(t)) {
          ...
          if (line.startsWith("*")) {
            ...
            if (section === "teams") {
              if (!currentGroup) {
                currentGroup = { title: "Our Expert Leadership & Wellness Team", description: "", items: [] };
              }
          ...
          if (section === "founder") {
            if (/CEO|Chief|Director|Chairman|Managing Director|Physician/i.test(line) && role === "") { role = line; continue; }
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (nameMatch && !line.toLowerCase().includes("rating:")) {
            if (current) items.push(current);
          ...
          if (current && line.toLowerCase().includes("rating:")) {
            const m = line.match(/\((\d)\/5\)/);
            if (m) current.rating = parseInt(m[1], 10);
            continue;
          }
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/kumarakom lake resort/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  const thumbnailImages = useMemo(() => {
    if (!images.length) return [];
    const rawIndices = [0, 1, 5, 11, 19, images.length - 1];
    const unique = Array.from(new Set(rawIndices.map((idx) => Math.min(Math.max(idx, 0), images.length - 1))));
    return unique.map((idx) => images[idx]).filter(Boolean);
  }, [images]);
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Traditional Kerala Heritage & Backwater Retreat
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Kumarakom, India</span>
                </div>
      ...
      <div className="container mx-auto px-3 md:px-4 pt-12 pb-12 max-w-full" id="gallery">
        <div className="max-w-6xl mx-auto">
                  ...
                  <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {images.map((img, i) => (
                        ...
                        >
                          <img src={img} alt={`Kumarakom ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {lightboxOpen && images.length > 0 && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#EDE8D0]/80 backdrop-blur-sm">
                  ...
                  <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
                    <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Kumarakom Lake Resort</div>
                    <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
                      <img
                        src={images[lightboxImage]}
                        alt={`Kumarakom ${lightboxImage + 1}`}
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg">
              <MarkdownContent
                contentPath="/content/Top Centers/kumarakom lake resort/Main content.txt"
          ...
          <div className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              ...
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-xs md:text-sm" style={{ color: "#7F543D" }}>Wellness Satisfaction</div>
              </div>
            </div>
                  ...
                  <AccordionTrigger className="hover:no-underline py-3 md:py-4 [&>svg]:text-orange-500">
                    <div className="flex items-center gap-2 md:gap-3 min-w-0">
                      <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-100 flex items-center justify-center border-2 border-orange-500">
                        {medicalIconForTitle(p.title, idx)}
                      </div>
                      <span className="text-base md:text-lg font-semibold text-primary truncate">{p.title}</span>
                    </div>
                  </AccordionTrigger>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Kumarakom Lake Resort
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Discover the atmosphere and wellness journey at Kumarakom Lake Resort through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">Why Choose Kumarakom Lake Resort for Your Healing Journey</h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Kumarakom Lake Resort
              </h2>
              ...
              <p className="text-sm md:text-lg mx-auto max-w-none leading-relaxed italic" style={{ color: "#7F543D" }}>
                Watch healing stories and real wellness journeys shared by our guests.
              </p>
            </div>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/kumarakom lake resort/CTA mid.jpg"
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognitions that reflect our heritage hospitality, wellness quality, and trusted guest experience.
              </p>
            </div>
          ...
          {faqItems.length > 0 && (
            <div className="mb-12" id="faq">
              <div className="text-center mb-8">
                ...
                <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                  Find answers to common questions about programs, stay, and your healing journey.
                </p>
              </div>
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Kumarakom Lake Resort Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3933.8038498116925!2d76.42832027300386!3d9.612151379592424!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3b08806b11120887%3A0x769331ac4b00a93f!2sKumarakom%20Lake%20Resort!5e0!3m2!1sen!2sin!4v1775670920030!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/kumarakom lake resort/CTA bottom.jpg"
                    alt="Kumarakom Lake Resort"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at Kumarakom Lake Resort</h2>
                  <div className="space-y-3">
                    <Button
                      size="lg"
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-bold text-white mb-3">Begin Your Holistic Healing Journey at Kumarakom Lake Resort</h2>
                  <div className="flex flex-wrap gap-3">

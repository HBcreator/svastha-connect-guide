...
const parseCardSection = (text: string): SectionData => {
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith("*")) {
      if (currentCard) {
        currentCard.bullets.push(line.replace(/^\*+\s*/, ""));
      }
      continue;
    }
...
const ToyamByOrchidHotels = () => {
  ...
  const founderImage = "/Center Images/Toyam By Orchid Hotels/Founder/Founder.jpg";
  const teamImage = "/Center Images/Toyam By Orchid Hotels/Founder/team.webp";

  const awards = [
    {
      title: "Tripadvisor Travelers' Choice Awards 2025",
      description: "Recognized for exceptional guest experiences and consistent traveler reviews.",
      image: "/Center Images/Toyam By Orchid Hotels/Awards/1 (tripadvisor).webp",
    },
    {
      title: "India Hospitality Excellence Awards 2024",
      description: "Celebrated for excellence in hospitality and service standards.",
      image: "/Center Images/Toyam By Orchid Hotels/Awards/2 (hospitality excellence).jpg",
    },
    {
      title: "Green Globe Certified",
      description: "Honored for sustainable practices and eco-conscious luxury hospitality.",
      image: "/Center Images/Toyam By Orchid Hotels/Awards/3 (green and globe).jpg",
    },
  ];

  const mediaItems: { image: string; pdf?: string; title: string }[] = [];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Toyam" },
  ...
  useEffect(() => {
    const photoListPath = "/Center Images/Toyam By Orchid Hotels/Photo Gallery/Photo Gallery Links.txt";
    const videoListPath = "/Center Videos/Toyam By Orchid Hotels/Video Gallery.txt";
    const facilityListPath = "/Center Images/Toyam By Orchid Hotels/Facilities/Facilities.txt";
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Why Choose Toyam.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
        let title = "";
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Facilities & Amenities.txt")
      .then((res) => res.text())
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (/our visionary founders/i.test(t)) { section = "founder"; continue; }
            if (/leadership & expertise/i.test(t)) { section = "expertise"; continue; }
            if (/our expert wellness team/i.test(t)) {
  ...
  useEffect(() => {
    if (!isMediaAutoPlaying || showAwards || mediaItems.length === 0) return;
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Toyam insurance info:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Frequently Asked Questions (FAQs).txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            const t = line.slice(2, -2).trim();
            if (/faq/i.test(t) && !current) { continue; }
            if (current) items.push(current);
      ...
      .catch((err) => console.error("Error loading Toyam FAQs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("*") && section === "distance") {
      ...
      .catch((err) => console.error("Error loading Toyam contact info:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("*") && line.endsWith("*") && !current.title) {
            current.title = cleanTitle(line);
            continue;
          }

          if (/rating:/i.test(line)) {
            const match = line.match(/(\d+(?:\.\d+)?)/);
            if (match) current.rating = Math.min(5, Math.max(1, parseFloat(match[1])));
            continue;
          }
      ...
      .catch((err) => console.error("Error loading Toyam reviews:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Toyam By Orchid Hotels/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          if (!line) continue;
          if (line.startsWith("### ") || (line.startsWith("**") && line.endsWith("**") && line.toLowerCase().includes("treatment process"))) {
            inSteps = false;
            continue;
          }
  ...
  const wellnessIconForTitle = (t: string) => {
    ...
    if (s.includes("stress") || s.includes("mental") || s.includes("mind") || s.includes("burnout") || s.includes("anxiety")) {
      return <Brain className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    }
    if (s.includes("anti-aging") || s.includes("rejuvenation") || s.includes("beauty") || s.includes("rasayana")) {
      return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    }
    if (s.includes("weight") || s.includes("metabolism") || s.includes("fitness")) {
    ...
    if (s.includes("digital") || s.includes("mindful") || s.includes("weekend") || s.includes("couples")) {
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("musculoskeletal") || s.includes("neurological") || s.includes("arthritis") || s.includes("stroke") || s.includes("migraine")) {
      return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    }
    if (s.includes("mental") || s.includes("anxiety") || s.includes("sleep")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    ...
    if (s.includes("reproductive") || s.includes("women") || s.includes("pcos") || s.includes("infertility")) {
  ...
  const treatmentIconForTitle = (t: string) => {
    ...
    if (s.includes("nature") || s.includes("garden") || s.includes("connection")) {
      return <TreePine className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("nutrition") || s.includes("diet") || s.includes("nourish")) {
      return <Leaf className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    }
    if (s.includes("follow-up") || s.includes("continued") || s.includes("aftercare")) {
  ...
  const testimonialVideos = [
    "/Center Videos/Toyam By Orchid Hotels/testimonies/testimonies 1.mp4",
    "/Center Videos/Toyam By Orchid Hotels/testimonies/testimonies 2.mp4",
  ];
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              TOYAM BY ORCHID HOTELS WELLNESS RESORT PUNE
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                <h1 className="text-4xl md:text-5xl font-bold mb-4">
                  Toyam By Orchid Hotels
                </h1>
                <p className="text-xl mb-4 opacity-90">
                  Wellness retreat in Pune (Bhor), Maharashtra
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Pune (Bhor), Maharashtra, India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.7</span>
                  <span className="opacity-90">(1200+)</span>
                </div>
              </div>
      ...
      <div className="container mx-auto px-3 md:px-4 pt-8 pb-6 md:pb-8 max-w-full">
        <div className="max-w-6xl mx-auto">
          <div className="mb-6 md:mb-8" id="gallery">
                ...
                <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                  {images.length > 0 && (
                    <img
                      ...
                      alt={`Toyam By Orchid Hotels ${selectedImage + 1}`}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/Toyam By Orchid Hotels/Main Content.txt"
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Toyam By Orchid Hotels
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at Toyam through our video gallery.
              </p>
            </div>
              ...
              <Card className="border-2 border-primary/20 shadow-xl overflow-hidden bg-white rounded-3xl">
                <CardContent className="p-0">
                  <div className="aspect-video w-full relative">
                    <video
                      key={videos[selectedVideo] || "toyam-video"}
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {whyChooseData?.title || "Why Choose Toyam?"}
              </h2>
          ...
          {/* Testimonials of Toyam By Orchid Hotels - Video Section */}
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Toyam By Orchid Hotels
              </h2>
          ...
          <div className="mb-12" id="process">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {treatmentIntro || "Your personalized healing journey at Toyam By Orchid Hotels, step by step"}
              </p>
            </div>

            <div className="max-w-4xl mx-auto">
              {treatmentSteps.map((step, index) => (
                <div key={`${step.number}-${step.title}`} className="relative flex flex-col md:flex-row items-center md:items-start gap-3 md:gap-6 mb-8 md:mb-12 group">
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Toyam By Orchid Hotels/CTA mid.webp"
                    alt="Toyam By Orchid Hotels"
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                ...
                <div>
                  <img
                    src="/Center Images/Toyam By Orchid Hotels/CTA bottom.webp"
                    alt="Toyam By Orchid Hotels"
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-lg font-semibold text-primary mb-2">
                    Facilities Built for Comfort, Safety, and Serenity
                  </h4>
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every space at Toyam is designed to support healing with clean, calm, and well-maintained environments.
                    From wellness zones to guest services, we prioritize comfort and care throughout your stay.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mb-10 rounded-3xl p-4 md:p-10" style={{ backgroundColor: "#EDE8D0" }} id="team">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {teamIntro || "Guided by visionary leadership and a dedicated wellness team"}
              </p>
            </div>
              ...
              <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all hover:shadow-xl h-full">
                <CardContent className="p-4 md:p-8 h-full md:h-[480px] flex flex-col">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    <div>
                      <h3 className="text-lg md:text-2xl font-bold text-primary mb-1 md:mb-2">{founder?.name || "Our Visionary Founders"}</h3>
                    ...
                    <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                      ...
                      <div>
                        <h3 className="text-lg md:text-2xl font-bold text-primary mb-1 md:mb-2 leading-snug break-words whitespace-normal">
                          {teamGroups[currentTeamSlide]?.title || "Our Expert Wellness Team"}
                        </h3>
                      </div>
                    </div>
              ...
              <div className="relative">
                ...
                <div className="absolute top-1/2 -translate-y-1/2 left-2 md:left-4">
                  <button
                    ...
                    className="bg-white text-primary p-2 md:p-3 rounded-full shadow-lg hover:bg-primary hover:text-white transition-colors border-2 border-primary/20"
          ...
          <div className="mt-16 md:mt-24" id="awards">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg px-4 mx-auto max-w-2xl" style={{ color: "#7F543D" }}>
                Recognition of Toyam By Orchid Hotels for excellence in wellness, hospitality, and sustainability.
              </p>
            </div>
              ...
              <div className="relative group max-w-5xl mx-auto">
                <div className="overflow-hidden px-4 md:px-10">
                  <div className="md:hidden">
                    ...
                    >
                      {awards.map((award, i) => (
                        <div key={award.title} className="w-full flex-shrink-0 px-2">
                  ...
                  <div className="hidden md:block">
                    ...
                    >
                      {awards.map((award) => (
                        <div key={award.title} className="w-1/3 flex-shrink-0 px-4">
            ...
            {!showAwards && mediaItems.length > 0 && (
                      ...
                      {mediaItems.map((item, index) => (
                        <div key={index} className="w-full flex-shrink-0 px-2">
                          <div className="bg-white rounded-2xl p-4 shadow-lg border-2 border-primary/10 hover:border-primary/30 transition-all h-full">
                            <div className="w-full aspect-square bg-primary/5 rounded-xl mb-4 p-4 flex items-center justify-center overflow-hidden">
                              <img src={item.image} alt={item.title} className="max-h-[80%] max-w-[80%] object-contain" />
                            </div>
                            <div className="text-center text-primary font-bold">{item.title}</div>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="mt-10 md:mt-16 mb-12" id="insurance">
            <div className="text-center mb-8">
          ...
          <div className="mb-12" id="faq">
            ...
            <Accordion type="single" collapsible className="space-y-4 max-w-4xl mx-auto">
              {faqs.map((faq, index) => (
                <AccordionItem key={index} value={`faq-${index}`} className="border-2 border-primary/20 rounded-lg px-6 data-[state=open]:border-primary transition-colors bg-white">
                  ...
                  <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                    <div className="rounded-xl overflow-hidden">
                      <div className="relative w-full aspect-[800/600]">
                        <iframe
                          title="Toyam By Orchid Hotels Map"
                          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d30304.12808910839!2d73.90576991558908!3d18.30087596182342!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bc2edfdde5c4f5f%3A0xf28601624c458c7f!2sToyam%20By%20Orchid%20Hotels!5e0!3m2!1sen!2sin!4v1773417483151!5m2!1sen!2sin"
              ...
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  ...
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                    Begin Your Holistic Healing Journey at Toyam
                  </h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    Begin Your <span className="text-white/90">Holistic Healing Journey</span> at <span className="text-white underline decoration-white/20 underline-offset-8">Toyam</span>
                  </h2>
          ...
          <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {images.map((img, idx) => (
                ...
                >
                  <img src={img} alt={`Toyam ${idx + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                </div>
              ))}
            </div>
          </div>
      ...
      {facilityLightboxOpen && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
              Toyam By Orchid Hotels Facilities & Amenities
            </div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                src={facilityImages[facilityLightboxImage]}
                alt={`Toyam Facility ${facilityLightboxImage + 1}`}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <button
                onClick={() => setFacilityLightboxOpen(false)}
...
export default ToyamByOrchidHotels;

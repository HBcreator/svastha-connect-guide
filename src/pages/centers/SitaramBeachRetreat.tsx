...
import { Activity, Award, Brain, Building2, Calendar, ChevronLeft, ChevronRight, ClipboardList, Droplet, FileSearch, Globe, Heart, HeartPulse, Home, Images, MapPin, MessageCircle, MessageCircleHeart, Phone, Pill, Search, ShieldCheck, Sparkles, Star, Stethoscope, TreePine, TrendingUp, Users, Utensils, Video, X } from "lucide-react";

export default function SitaramBeachRetreat() {
  ...
  const founderImage = "/Center Images/Sitaram Beach Retreat/Founder/Founder.jpg";
  const teamImage = "/Center Images/Sitaram Beach Retreat/Founder/team.jpg";
  ...
  const [maxAwardIndex, setMaxAwardIndex] = useState(2);
  ...
  const awards = [
    {
      ...
      description: "Recognized for meeting rigorous standards of patient safety, clinical care, and quality management.",
      image: "/Center Images/Sitaram Beach Retreat/Awards/Award 1  (NABH).png",
    },
    {
      title: "Travellers' Choice Award",
      description: "Honored by Tripadvisor for consistently outstanding guest experiences and reviews.",
      image: "/Center Images/Sitaram Beach Retreat/Awards/Award 2 (Travellers' Choice Award (Tripadvisor).jfif",
    },
    {
      title: "GMP Certified Pharmacy",
      description: "Powered by Sitaram Ayurveda Pharmacy’s GMP-certified manufacturing for trusted, high-quality medicines.",
      image: "/Center Images/Sitaram Beach Retreat/Awards/Award 3 (First GMP Certified Pharmacy).jpg",
    },
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Sitaram" },
  ...
  const thumbnailImages = thumbnailIndices
    .map((idx) => images[idx])
    .filter(Boolean);

  useEffect(() => {
    fetch(encodeURI("/Center Images/Sitaram Beach Retreat/Photo Gallery/Photo Gallery LInks.txt"))
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Sitaram photo gallery:", err));

    fetch(encodeURI("/Center Videos/Sitaram Beach Retreat/Video Gallery Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
        setVideos(lines);
      ...
      .catch((err) => console.error("Error loading Sitaram video gallery:", err));

    fetch("/content/Top Centers/Sitaram Beach Retreat/Wellness & Rejuvenation Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    fetch("/content/Top Centers/Sitaram Beach Retreat/Medical Treatment Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    fetch("/content/Top Centers/Sitaram Beach Retreat/Why Choose Sitaram Beach Retreat.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
        let intro = "";
    ...
    fetch(encodeURI("/Center Videos/Sitaram Beach Retreat/testimonies/testimonies Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      ...
      .catch((err) => console.error("Error loading Sitaram testimonial videos:", err));

    fetch("/content/Top Centers/Sitaram Beach Retreat/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Sitaram treatment content:", err));

    fetch("/content/Top Centers/Sitaram Beach Retreat/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
    ...
    fetch(encodeURI("/Center Images/Sitaram Beach Retreat/Facilities/Facilities Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      ...
      .catch((err) => console.error("Error loading Sitaram facilities images:", err));

    fetch("/content/Top Centers/Sitaram Beach Retreat/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (section === "intro") {
            ...
            if (/Founder|Chief|Director|Physician|Managing Director|Owner/i.test(line) && role === "") { role = line; continue; }
    ...
    fetch("/content/Top Centers/Sitaram Beach Retreat/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        const list: { id: number; name: string; location: string; condition: string; title: string; review: string; rating: number; verified: boolean }[] = [];
        ...
        for (const line of lines) {
          ...
          if (current && line.includes("Rating:")) {
            ...
            const ratingMatch = cleaned.match(/\((\d+)\s*\/\s*5\)/);
            if (ratingMatch) {
      ...
      .catch((err) => console.error("Error loading Sitaram testimonials:", err));

    fetch("/content/Top Centers/Sitaram Beach Retreat/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Sitaram insurance/payment:", err));

    fetch("/content/Top Centers/Sitaram Beach Retreat/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
    ...
    fetch("/content/Top Centers/Sitaram Beach Retreat/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    if (!isAutoPlaying || showVideoGallery || images.length <= 1) return;
    const id = setInterval(() => {
      setSelectedImage((prev) => (prev + 1) % images.length);
    }, 4000);
    return () => clearInterval(id);
  }, [isAutoPlaying, showVideoGallery, images.length]);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const newMax = isMobile ? awards.length - 1 : Math.min(2, awards.length - 1);
  ...
  const wellnessIconForTitle = (t: string) => {
    ...
    if (s.includes("stress") || s.includes("anxiety")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("yoga") || s.includes("meditation")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("panchakarma")) return <Droplet className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("lifestyle") || s.includes("consultation")) return <ClipboardList className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("weight")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("mind") || s.includes("balance")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("chronic")) return <ClipboardList className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("pain") || s.includes("injury")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("digestive") || s.includes("gut")) return <Pill className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("immunity")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("legacy") || s.includes("award") || s.includes("recognition")) return <Star className={cls} />;
    if (s.includes("authentic") || s.includes("classical") || s.includes("ayurveda")) return <Stethoscope className={cls} />;
    if (s.includes("nature") || s.includes("greenery") || s.includes("garden")) return <Heart className={cls} />;
    if (s.includes("physician") || s.includes("doctor") || s.includes("care")) return <HeartPulse className={cls} />;
    if (s.includes("detox") || s.includes("panchakarma")) return <Droplet className={cls} />;
    if (s.includes("yoga") || s.includes("meditation") || s.includes("mind")) return <Activity className={cls} />;
  ...
  const treatmentIconForTitle = (t: string) => {
    ...
    if (s.includes("wellness") || s.includes("relaxation") || s.includes("activities")) return <HeartPulse className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Authentic Ayurveda by the Kerala Coast
                </p>
              ...
              <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                {images[selectedImage] && (
                  <img
                    ...
                    alt={`Sitaram Center ${selectedImage + 1}`}
              ...
              {lightboxOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#EDE8D0]/80 backdrop-blur-sm">
                  ...
                  <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
                    <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Sitaram Beach Retreat</div>
                    <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
                      <img
                        src={images[lightboxImage]}
                        alt={`Sitaram ${lightboxImage + 1}`}
        ...
        <div className="max-w-6xl mx-auto">
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg">
              <MarkdownContent
                contentPath="/content/Top Centers/Sitaram Beach Retreat/Main content.txt"
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              ...
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-base md:text-3xl font-bold text-primary mb-1 whitespace-nowrap">4.6/5</div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Sitaram Beach Retreat
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Explore the tranquil coastal setting and authentic healing experience at Sitaram Beach Retreat.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">
                Why Choose Sitaram Beach Retreat for Your Healing Journey
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Sitaram Beach Retreat
              </h2>
                  ...
                  <div className="aspect-video w-full relative">
                    {testimonialVideos[selectedTestimonialVideo] && (
                      <iframe
                        ...
                        title="Sitaram Beach Retreat Testimonial Video"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Sitaram Beach Retreat/CTA mid.jpg"
                    alt="Sitaram Beach Retreat"
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognition of Sitaram Beach Retreat for authentic Ayurveda, guest care, and trusted clinical standards.
              </p>
            </div>
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Sitaram Beach Retreat Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3923.957425565902!2d76.07854797301626!3d10.424953265582511!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ba78cca18524227%3A0x85cd7b2927ae8165!2sSitaram%20Beach%20Retreat%20-%20Ayurvedic%20Retreat%20in%20Kerala!5e0!3m2!1sen!2sin!4v1775563138689!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Sitaram Beach Retreat/CTA bottom.jpg"
                    alt="Sitaram Beach Retreat"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at Sitaram Beach Retreat</h2>
                  <div className="space-y-3">
                    <Button
                      size="lg"
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-bold text-white mb-3">Begin Your Holistic Healing Journey at Sitaram Beach Retreat</h2>
                  <div className="flex flex-wrap gap-3">

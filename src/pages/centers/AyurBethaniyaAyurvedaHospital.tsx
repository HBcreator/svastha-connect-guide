...
export default function AyurBethaniyaAyurvedaHospital() {
  ...
  const [selectedJumpSection, setSelectedJumpSection] = useState<string | null>(null);
  ...
  const awards = [
    {
      title: "Kerala Ayurveda Excellence Recognition",
      description: "Recognized for outstanding commitment to authentic Kerala Ayurveda, patient safety, and clinical care.",
      image: "/Center Images/Ayur Bethaniya/Awards/Award 1 (kerala-logo66b210e07cdac).png",
    },
    {
      title: "Green Leaf Certification",
      description: "Acknowledged for maintaining high standards of traditional Ayurvedic therapies and holistic wellness practices.",
      image: "/Center Images/Ayur Bethaniya/Awards/Award 2 (green-leaf).png",
    },
    {
      title: "NABH Standards Alignment",
      description: "Committed to internationally recognized quality and safety protocols in healthcare delivery.",
      image: "/Center Images/Ayur Bethaniya/Awards/Award 3 (nabh-logo).png",
    },
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Ayur Bethaniya" },
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith("**Leadership") && line.endsWith("**")) {
        ...
        while (i < lines.length) {
          ...
          if (t.startsWith("*")) {
            data.founderTags.push(t.replace(/^\*+\s*/, "").trim());
          }
          i++;
        }
        continue;
      ...
      if (line.startsWith("**") && line.endsWith("**")) {
        ...
        while (i < lines.length) {
          ...
          if (t.startsWith("**") && t.endsWith("**")) break;
          if (t.startsWith("*")) {
            data.teamBullets.push(t.replace(/^\*+\s*/, "").trim());
          }
          i++;
        }
        continue;
  ...
  useEffect(() => {
    ...
    fetch("/content/Top Centers/Ayur Bethaniya/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => setFacilitiesData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Facilities:", err));

    fetch("/Center Images/Ayur Bethaniya/Facilities/Facilities LInks.txt")
      .then((res) => res.text())
      .then((text) => {
        const imgs = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Facility images:", err));

    fetch("/content/Top Centers/Ayur Bethaniya/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => setFounderTeamInfo(parseFounderTeamInfo(text)))
      .catch((err) => console.error("Error loading Founder & Team Info:", err));

    fetch("/content/Top Centers/Ayur Bethaniya/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
    ...
    fetch("/content/Top Centers/Ayur Bethaniya/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.replace(/\r\n/g, "\n").split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("*")) {
            ...
            if (section === "payment") pay.push(bullet);
            continue;
          }
    ...
    fetch("/content/Top Centers/Ayur Bethaniya/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.replace(/\r\n/g, "\n").split("\n").map((l) => l.trim());
    ...
    fetch("/content/Top Centers/Ayur Bethaniya/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.replace(/\r\n/g, "\n").split("\n").map((l) => l.trim());
        const addr: string[] = [];
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("*")) {
            const bullet = line.replace(/^\*+\s*/, "").trim();
            if (section === "distance" && bullet) dists.push(bullet);
            continue;
          }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") setFacilityLightboxOpen(false);
      if (e.key === "ArrowLeft") {
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Ayur Bethaniya/CTA-mid.jpg"
                    alt="Ayur Bethaniya Wellness Center"
                    ...
                    {facilityImages.map((image, index) => (
                      <div key={index} className="w-full flex-shrink-0 px-2">
                        ...
                        >
                          <img
                            src={image}
                            alt={`Ayur Bethaniya Facility ${index + 1}`}
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at Ayur Bethaniya is designed and maintained according to NABH accreditation standards,
          ...
          <div className="mb-10 rounded-3xl p-4 md:p-10" style={{ backgroundColor: "#EDE8D0" }} id="team">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {founderTeamInfo?.description || "Led by visionary expertise and supported by generations of traditional healing knowledge"}
              </p>
            </div>
                  ...
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    >
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/Ayur Bethaniya/Founder/Founder.jpg"
                  ...
                  <CardContent className="p-4 md:p-8 h-full md:h-[480px] md:overflow-y-auto">
                    <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                      ...
                      >
                        <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                          <img
                            src="/Center Images/Ayur Bethaniya/Founder/Team.jpg"
          ...
          <div className="mb-12" id="awards">
            ...
            <div className="relative group max-w-5xl mx-auto">
              ...
              <button
                ...
                className="absolute left-8 md:hidden top-[57%] -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 rounded-full shadow-lg transition-all border-2 border-primary z-10"
              ...
              <button
                ...
                className="absolute right-8 md:hidden top-[57%] -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 rounded-full shadow-lg transition-all border-2 border-primary z-10"
          ...
          <Card className="mb-12 border-2 border-primary overflow-hidden transition-all duration-300 hover:shadow-2xl" id="contact">
            <CardContent className="p-5 md:p-8">
              ...
              <div className="grid gap-8 md:grid-cols-[1fr_1.35fr] lg:gap-12">
                ...
                <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden md:-mt-3">
                  <div className="rounded-xl overflow-hidden">
                    <div className="relative w-full aspect-[800/600]">
                      <iframe
                        title="Ayur Bethaniya Map"
                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d8031075.573706192!2d66.98094077499997!3d10.624161299999999!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ba7e909110d347d%3A0x5058cd9bbbc1692c!2sAyur%20Bethaniya%20Ayurvedic%20Hospital!5e0!3m2!1sen!2sin!4v1769160286308!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Ayur Bethaniya/CTA-bottom.jpg"
                    alt="Ayur Bethaniya Ayurveda Hospital"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">Begin Your Holistic Healing Journey at Ayur Bethaniya</h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    ...
                    <span className="text-white underline decoration-white/20 underline-offset-8">Ayur Bethaniya</span>
                  </h2>
      ...
      >
        ...
        >
          ...
          <div className="p-4 pb-4 bg-[#2F5B63] text-white relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-3 relative z-10">
              <div className="space-y-0.5">
                ...
                <h2 className="text-[24px] sm:text-[26px] font-extrabold leading-tight tracking-tight text-white pr-3">Sections of Ayur Bethaniya</h2>
              </div>
              <button
                onClick={() => setIsJumpModalOpen(false)}
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain px-5 py-4 space-y-2.5 custom-scrollbar" style={{ WebkitOverflowScrolling: "touch" }}>
            {jumpSections.map((section, idx) => (
              <button
                key={section.id}
                onClick={() => jumpToSection(section.id)}
                className={`w-full group relative transition-all duration-300 p-3 rounded-xl border-2 flex items-center justify-between shadow-md md:hover:shadow-xl active:scale-[0.99]
                  ${selectedJumpSection === section.id ? "bg-[#2F5B63] border-primary" : "bg-white border-primary/20"}
                  md:hover:bg-[#2F5B63] md:hover:border-primary active:bg-[#2F5B63] active:border-primary
                `}
              >
                <div className="flex items-center gap-4 relative z-10">
                  <div
                    className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-200
                      ${selectedJumpSection === section.id ? "bg-white/10" : "bg-primary/5"}
                      md:group-hover:bg-white/10 group-active:bg-white/10
                    `}
                  >
                    <span
                      className={`text-xs font-black transition-all duration-200
                        ${selectedJumpSection === section.id ? "text-white" : "text-primary"}
                        md:group-hover:text-white group-active:text-white
                      `}
                    >
                  ...
                  <span
                    className={`text-sm md:text-base font-bold transition-all duration-200 text-left
                      ${selectedJumpSection === section.id ? "text-white" : "text-primary"}
                      md:group-hover:text-white group-active:text-white
                    `}
                  >
                ...
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200
                    ${selectedJumpSection === section.id ? "bg-white/20" : "bg-transparent"}
                    md:group-hover:bg-white/20 group-active:bg-white/20
                  `}
                >
                  <ChevronRight
                    className={`h-3.5 w-3.5 transition-all duration-200
                      ${selectedJumpSection === section.id ? "text-white" : "text-primary"}
                      md:group-hover:text-white group-active:text-white md:group-hover:translate-x-0.5
                    `}
                  />
                </div>
                ...
                <div
                  className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 bg-white rounded-r-full transition-all duration-200
                    ${selectedJumpSection === section.id ? "h-3/5" : "h-0"}
                    md:group-hover:h-3/5
                  `}
                />
              </button>
            ))}
      ...
      {facilityLightboxOpen && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Ayur Bethaniya Facilities</div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              {facilityImages[facilityLightboxImage] && (
                <img
                  ...
                  alt={`Ayur Bethaniya Facility ${facilityLightboxImage + 1}`}
                  className="absolute inset-0 w-full h-full object-cover"
                />
              )}
              <button
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 text-white px-3 py-1 rounded-full text-sm">
                {facilityImages.length ? `${facilityLightboxImage + 1} / ${facilityImages.length}` : "0 / 0"}
              </div>
            </div>

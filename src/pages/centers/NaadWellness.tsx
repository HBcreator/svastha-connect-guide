...
import { Calendar, MapPin, Star, Images, Video, ChevronLeft, ChevronRight, Users, TrendingUp, Heart, Stethoscope, Activity, Brain, Droplet, HeartPulse, UserCheck, ShieldCheck, Sparkles, Pill, ClipboardList, FileSearch, Phone, MessageCircle, Home, Building2, TreePine, Utensils, Award, Globe, Search, X } from "lucide-react";
...
const getFacilityIcon = (t: string) => {
  ...
  if (s.includes("architecture") || s.includes("design") || s.includes("building") || s.includes("space"))
    return <Building2 className="h-7 w-7 text-white" />;
  if (s.includes("farm") || s.includes("organic") || s.includes("garden") || s.includes("outdoor"))
    return <TreePine className="h-7 w-7 text-white" />;
  if (s.includes("infrastructure") || s.includes("treatment") || s.includes("therapy") || s.includes("clinical") || s.includes("medical"))
  ...
  if (s.includes("safety") || s.includes("nabh") || s.includes("standard") || s.includes("quality"))
...
export default function NaadWellness() {
  ...
  const [selectedFeatureVideo, setSelectedFeatureVideo] = useState(0);
  ...
  const testimonialVideos = [
    "/Center Videos/Naad Wellness/testimonies/testi 1.mp4",
    "/Center Videos/Naad Wellness/testimonies/testi 2.mp4",
  ];
  ...
  const awards = [
    {
      ...
      image: "/Center Images/Naad Wellness/Awards/1 (NABH).png",
      description: "Recognized under NABH benchmarks for patient safety and quality-driven care delivery.",
    },
    {
      title: "QCI Quality Framework",
      image: "/Center Images/Naad Wellness/Awards/2 (quality of control ).png",
      description: "Aligned with Quality Council of India principles to support consistent clinical standards.",
    },
    {
      title: "Trusted Guest Assurance",
      image: "/Center Images/Naad Wellness/Awards/3 (trust of guest ).png",
      description: "Built on transparent care, comfort, and long-term trust among domestic and international guests.",
    },
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Naad Wellness" },
  ...
  const parseInsurancePayment = (text: string) => {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith("**") && line.endsWith("**")) {
        ...
        if (heading.toLowerCase().includes("secure payment")) {
  ...
  useEffect(() => {
    fetch("/Center Images/Naad Wellness/Photo Gallery/Photo Links.txt")
      .then((r) => r.text())
      .then((t) => {
        const urls = t
          .split("\n")
        ...
        const preferred = [0, 1, 5, 11, 19, 29].filter((i) => i < urls.length);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Insurance & Payment Info.txt")
      .then((r) => r.text())
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Frequently Asked Questions.txt")
      .then((r) => r.text())
      .then((t) => parseFaq(t))
      .catch(() => setFaqItems([]));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Contact Information.txt")
      .then((r) => r.text())
  ...
  useEffect(() => {
    fetch("/Center Videos/Naad Wellness/Video links.txt")
      .then((r) => r.text())
      .then((t) => {
        const urls = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Naad wellness programs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Naad medical programs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Why Choose Naad.txt")
      .then((res) => res.text())
      .then((text) => setWhyChooseData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Naad why-choose content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => setFacilitiesData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Naad facilities content:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Images/Naad Wellness/Facilities/Facilities.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Naad facilities images:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Founder & Expert Team.txt")
      .then((res) => res.text())
      .then((text) => setFounderTeamData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Naad founder/team content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Naad reviews:", err));
  }, []);
  ...
  const renderStars = (rating: number) => (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={`h-4 w-4 md:h-5 md:w-5 ${star <= rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
        />
      ))}
    </div>
  );
  ...
  useEffect(() => {
    if (!isAwardsMobile) return;
    const id = window.setInterval(() => {
      setCurrentAward((prev) => (prev === maxAwardIndex ? 0 : prev + 1));
    }, 4000);
    return () => window.clearInterval(id);
  }, [isAwardsMobile, maxAwardIndex]);

  const goToPreviousAward = () => {
    setCurrentAward((prev) => (prev === 0 ? maxAwardIndex : prev - 1));
  };
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Naad Wellness/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            current = { number: stepIndex++, title: line.slice(2, -2), description: "", bullets: [] };
            inSteps = true;
            continue;
          }
          if (line.startsWith("*")) {
      ...
      .catch((err) => console.error("Error loading Naad treatment process:", err));
  }, []);

  const wellnessIconForTitle = (title: string) => {
    const t = title.toLowerCase();
    if (t.includes("detox")) return <Droplet className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (t.includes("stress") || t.includes("emotional") || t.includes("sleep")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (t.includes("weight")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (t.includes("immunity")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (t.includes("women") || t.includes("men")) return <UserCheck className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (t.includes("beauty") || t.includes("radiance")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  };

  const medicalIconForTitle = (title: string) => {
    const t = title.toLowerCase();
    if (t.includes("spinal") || t.includes("back")) return <Pill className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (t.includes("arthritis") || t.includes("joint")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (t.includes("diabetes") || t.includes("thyroid") || t.includes("pcos")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (t.includes("migraine") || t.includes("fatigue")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (t.includes("respiratory")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    return <Stethoscope className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  };

  const iconForTitle = (title: string) => {
    const t = title.toLowerCase();
    const cls = "h-5 w-5 text-primary group-hover:text-white transition-colors";
    if (t.includes("accredited") || t.includes("quality") || t.includes("standard")) return <ShieldCheck className={cls} />;
    if (t.includes("integrated") || t.includes("approach")) return <Stethoscope className={cls} />;
    if (t.includes("scientific") || t.includes("evidence")) return <Activity className={cls} />;
    if (t.includes("team") || t.includes("doctors")) return <Users className={cls} />;
    if (t.includes("personalized")) return <UserCheck className={cls} />;
    if (t.includes("sanctuary") || t.includes("modern")) return <HeartPulse className={cls} />;
    if (t.includes("location") || t.includes("accessible")) return <MapPin className={cls} />;
    if (t.includes("education") || t.includes("empower")) return <Sparkles className={cls} />;
    if (t.includes("seamless") || t.includes("all-inclusive")) return <Heart className={cls} />;
    return <Star className={cls} />;
  };

  const treatmentIconForTitle = (title: string) => {
    const t = title.toLowerCase();
    if (t.includes("assessment") || t.includes("consultation")) return <ClipboardList className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (t.includes("prescription") || t.includes("plan")) return <FileSearch className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (t.includes("detox")) return <Droplet className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (t.includes("therapeutic") || t.includes("intervention")) return <Stethoscope className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (t.includes("nourishment")) return <Heart className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (t.includes("path") || t.includes("forward")) return <TrendingUp className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-3 md:px-4 max-w-full">
          <div className="max-w-6xl mx-auto">
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                <h1 className="text-4xl md:text-5xl font-bold mb-4">NAAD WELLNESS</h1>
                <p className="text-xl mb-4 opacity-90">
                  Luxury Integrative Wellness Retreat
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Sonepat, Haryana, Near Delhi NCR</span>
                </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-12" id="gallery">
            <div className="flex items-center mb-6 flex-wrap gap-3 md:gap-4">
              <div className="flex items-center gap-2 md:gap-4 w-full md:w-auto">
                ...
                >
                  <Images className="mr-1.5 h-4 w-4 md:h-5 md:w-5" />
                  Photo Gallery
                </Button>
                ...
                >
                  {images.length > 0 && (
                    <img
                      ...
                      alt={`Naad Wellness ${selectedImage + 1}`}
                      ...
                      return (
                        ...
                        >
                          <div className="relative w-full rounded-xl overflow-hidden" style={{ paddingBottom: "100%" }}>
                            <img src={img} alt={`Naad Wellness ${actualIndex + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-xl" />
                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300"></div>
              ...
              <>
                <div className="relative rounded-lg overflow-hidden shadow-lg bg-black aspect-video mb-8">
                  ...
                  <div className="absolute bottom-4 right-4 bg-black/60 text-white px-3 py-1 rounded-full text-sm">
                    Video {Math.min(selectedVideo + 1, Math.max(videos.length, 1))} / {videos.length}
                  </div>
                </div>
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/Naad Wellness/Main Content.txt"
          ...
          <div className="mb-12 rounded-3xl px-6 py-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-base md:text-3xl font-bold text-primary mb-1 whitespace-nowrap">200+</div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Naad Wellness
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at Naad Wellness through our video gallery.
              </p>
            </div>
            ...
            <div className="relative max-w-4xl mx-auto">
              <Card className="border-2 border-primary/20 shadow-xl overflow-hidden bg-white rounded-3xl">
                <CardContent className="p-0">
                  <div className="aspect-video w-full relative">
                    {videos.length > 0 && (
                      <video
                        key={videos[selectedFeatureVideo]}
                        src={videos[selectedFeatureVideo]}
                        className="w-full h-full object-cover"
                        controls
                        playsInline
                      />
              ...
              <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between px-2 md:-mx-8 pointer-events-none">
                <button
                  onClick={() => setSelectedFeatureVideo((prev) => (prev - 1 + videos.length) % videos.length)}
                ...
                <button
                  onClick={() => setSelectedFeatureVideo((prev) => (prev + 1) % videos.length)}
                  className="bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary pointer-events-auto"
              ...
              <div className="flex justify-center gap-2 mt-6 md:mt-8">
                {videos.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => setSelectedFeatureVideo(index)}
                    className={`transition-all ${index === selectedFeatureVideo ? "w-8 h-3 bg-primary" : "w-3 h-3 bg-gray-300 hover:bg-primary/50"} rounded-full`}
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {whyChooseData?.title || "Why Choose Naad Wellness for Your Healing Journey?"}
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Naad Wellness Center
              </h2>
            ...
            <div className="relative max-w-4xl mx-auto px-4 md:px-0">
              <div className="relative w-full max-w-[320px] md:max-w-[380px] lg:max-w-[420px] mx-auto">
                <Card className="border-2 border-primary/20 shadow-xl overflow-hidden bg-white rounded-3xl">
                  <CardContent className="p-0">
              ...
              <div className="hidden md:block">
                <button
                  ...
                  className="absolute left-0 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary"
                ...
                <button
                  ...
                  className="absolute right-0 top-1/2 translate-x-1/2 -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary"
          ...
          <div className="mb-12" id="process">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {treatmentIntro || "Your personalized healing journey at Naad Wellness, step by step"}
              </p>
            </div>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Naad Wellness/CTA mid.jpg"
                    alt="Naad Wellness Center"
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at Naad Wellness is designed and maintained to the highest standards of safety, hygiene,
                    and quality care. Our commitment to excellence ensures you receive world-class integrative treatment in
                    a serene, naturally therapeutic environment.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mb-10 rounded-3xl p-4 md:p-10" style={{ backgroundColor: "#EDE8D0" }} id="team">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {founderTeamData?.description || "Led by visionary expertise and supported by dedicated healing professionals"}
              </p>
            </div>

            <div className="grid md:grid-cols-2 gap-4 md:gap-8 mb-6 md:mb-12 items-stretch">
              <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all hover:shadow-xl h-full md:h-[460px] overflow-hidden">
                <CardContent className="p-4 md:p-8">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    <div className="p-[3px] rounded-full flex-shrink-0 shadow-2xl aspect-square" style={{ background: "conic-gradient(from 45deg, #F0E68C, #B8860B, #FFD700, #B8860B, #F0E68C)" }}>
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/Naad Wellness/Founder/Founder.jpg"
                          alt="Naad Wellness Founder"
                          className="w-full h-full object-cover"
                        />
                      </div>
                    </div>
                    <div>
                      <h3 className="text-lg md:text-2xl font-bold text-primary mb-1 md:mb-2">
                        {founderTeamData?.cards?.[0]?.title || "The Visionary Founder"}
                      </h3>
                  ...
                  <p className="text-xs md:text-sm leading-relaxed mb-3 md:mb-4" style={{ color: "#7F543D" }}>
                    {founderTeamData?.cards?.[0]?.description || "Naad Wellness was brought to life by visionary leadership focused on clinical excellence and soulful healing."}
                  </p>
                  <div className="pt-3 md:pt-4 border-t border-primary/10">
                    <p className="text-xs font-semibold text-primary mb-2">Leadership Highlights</p>
                    <div className="flex flex-wrap gap-2">
                      {(founderTeamData?.cards?.[0]?.bullets || []).slice(0, 3).map((bullet, idx) => (
              ...
              <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all hover:shadow-xl h-full md:h-[460px] relative overflow-hidden">
                <CardContent className="p-4 md:p-8 flex flex-col h-full">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    <div className="p-[3px] rounded-full flex-shrink-0 shadow-2xl aspect-square" style={{ background: "conic-gradient(from 45deg, #F0E68C, #B8860B, #FFD700, #B8860B, #F0E68C)" }}>
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/Naad Wellness/Founder/Team.jpg"
                          alt="Naad Wellness Expert Team"
                          className="w-full h-full object-cover"
                        />
                      </div>
                    </div>
                    <div>
                      <h3 className="text-lg md:text-2xl font-bold text-primary mb-1 md:mb-2">
                        {founderTeamData?.cards?.[1]?.title || "Our Expert Medical Leadership"}
                      </h3>
                      <p className="text-xs md:text-sm font-semibold" style={{ color: "#7F543D" }}>
                        Senior Doctors & Wellness Experts
                      </p>
                    </div>
                  </div>
                  <div className="flex-1 min-h-0 overflow-y-auto pr-1 md:pr-2">
                    <p className="text-xs md:text-sm leading-relaxed mb-3 md:mb-4" style={{ color: "#7F543D" }}>
                      {founderTeamData?.cards?.[1]?.description || "Your healing journey is supervised by experienced Ayurvedic physicians and an expert multidisciplinary team."}
                    </p>

                    {(founderTeamData?.cards?.[1]?.bullets?.length || 0) > 0 && (
                      <ul className="space-y-1.5 mb-3 md:mb-4">
                        {(founderTeamData?.cards?.[1]?.bullets || []).map((bullet, idx) => (
                    ...
                    {(founderTeamData?.cards || []).length > 2 && (
                      <div className="space-y-2 pt-3 border-t border-primary/10">
                        <p className="text-xs font-semibold text-primary mb-2">Core Team Strengths:</p>
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          {(founderTeamData?.cards || []).slice(2).map((card, idx) => (
                            <div key={idx} className="flex items-center gap-2">
                              <span className="w-2 h-2 bg-primary rounded-full"></span>
                              <span className="text-xs" style={{ color: "#7F543D" }}>{card.title.replace(/\*\*/g, "")}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        {(founderTeamData?.cards || []).slice(2).map((card, idx) => (
                          <div key={idx} className="p-2.5 rounded-lg bg-primary/5 border border-primary/10">
                            <p className="text-xs font-semibold text-primary mb-1">{card.title.replace(/\*\*/g, "")}</p>
                            {card.description && (
                              <p className="text-xs leading-relaxed" style={{ color: "#7F543D" }}>
                                {card.description}
                              </p>
                            )}
                          </div>
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognition of Naad Wellness for patient safety, quality standards, and trusted care.
              </p>
            </div>
                ...
                <CardContent className="p-8">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-primary">
                      {insurancePaymentTitle || "Our Secure Payment Process"}
                    </h3>
                  </div>
                  ...
                  <ul className="space-y-3">
                    {insurancePaymentBullets.map((b, idx) => (
              ...
              <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all">
                <CardContent className="p-8">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-primary">
                      {insuranceClaimsTitle || "Support for Insurance Claims"}
                    </h3>
                  </div>
                  ...
                  <ul className="space-y-3">
                    {insuranceClaimsBullets.map((b, idx) => (
                      <li key={idx} className="flex items-start gap-2 text-sm" style={{ color: "#7F543D" }}>
                        <span className="text-primary mt-1">&#10003;</span>
                        <span>{b}</span>
                      </li>
            ...
            <Card className="mt-6 bg-primary/5 border-l-4 border-l-primary">
              <CardContent className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-lg font-semibold text-primary mb-2">
                      {insuranceInternationalTitle || "A Note for International Guests"}
                    </h4>
                    ...
                    <ul className="space-y-2">
                      {insuranceInternationalBullets.map((b, idx) => (
                        <li key={idx} className="flex items-start gap-2 text-sm" style={{ color: "#7F543D" }}>
                          <span className="text-primary mt-1">&#10003;</span>
                          <span>{b}</span>
                        </li>
                  ...
                  <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                    <div className="rounded-xl overflow-hidden">
                      <div className="relative w-full aspect-[800/600]">
                        <iframe
                          title="Naad Wellness Map"
                          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3492.615282829941!2d77.12170789999999!3d28.909778300000003!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x390dac15b171de69%3A0xd2104059b356617d!2sNaad%20Wellness!5e0!3m2!1sen!2sin!4v1773062754793!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: '#234A50' }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Naad Wellness/CTA bottom.jpg"
                    alt="Naad Wellness Center"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">Begin Your Holistic Healing Journey at Naad Wellness</h2>
                  <div className="space-y-4">
                    <button
                      className="w-full rounded-full bg-white text-primary hover:bg-white/90 text-sm sm:text-base font-bold py-3.5 flex items-center justify-center gap-2"
                      onClick={() => setQuoteModalOpen(true)}
                    >
                    ...
                    <button
                      className="w-full rounded-full border-2 border-white/60 bg-transparent text-white hover:bg-orange-500 hover:border-orange-500 active:bg-orange-500 active:border-orange-500 text-sm sm:text-base font-bold py-3.5 flex items-center justify-center gap-2"
                      onClick={() => setQuoteModalOpen(true)}
                    >
                  ...
                  <div className="mt-6 flex items-center justify-center gap-2 text-white/90 text-sm">
                    ...
                    <a href="tel:+911264222000" className="underline hover:text-white">Call us: +91 1264 222 000</a>
                  </div>
                </div>
              </div>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    Begin Your <span className="text-white/90">Holistic Healing Journey</span> at <span className="text-white underline decoration-white/20 underline-offset-8">Naad Wellness</span>
                  </h2>
                  <div className="flex flex-wrap gap-5">
                    <button 
                      className="rounded-full px-8 py-4 bg-white text-primary hover:bg-white/90 font-bold flex items-center gap-2 transition-all"
                      onClick={() => setQuoteModalOpen(true)}
                    >
                      <Phone className="h-5 w-5" />
                      Book Consultation Now
                    ...
                    <button 
                      className="rounded-full px-8 py-4 border-2 border-white/60 bg-transparent text-white hover:bg-orange-500 hover:border-orange-500 active:bg-orange-500 active:border-orange-500 font-bold flex items-center gap-2 transition-all"
                      onClick={() => setQuoteModalOpen(true)}
                    >
                      <MessageCircle className="h-5 w-5" />
                      Chat With Us
          ...
          <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {images.map((img, idx) => (
                ...
                >
                  <img src={img} alt={`Naad Wellness ${idx + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              {images.length > 0 && (
                <img
                  ...
                  alt={`Naad Wellness ${lightboxImage + 1}`}
      ...
      {facilityLightboxOpen && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">
              Naad Wellness Facilities
            </div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              {facilityImages.length > 0 && (
                <img
                  src={facilityImages[facilityLightboxImage]}
                  alt={`Naad Wellness facility ${facilityLightboxImage + 1}`}
                  className="absolute inset-0 w-full h-full object-cover"
                />
              )}
              <button

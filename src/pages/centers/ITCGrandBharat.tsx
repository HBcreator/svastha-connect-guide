...
export default function ITCGrandBharat() {
  ...
  const founderImage = "/Center Images/ITC Grand Bharat/Founder/founder.webp";
  const teamImage = "/Center Images/ITC Grand Bharat/Founder/team.webp";
  ...
  useEffect(() => {
    fetch("/Center Images/ITC Grand Bharat/Photo Gallery/Photo Gallery.txt")
      .then((r) => r.text())
      .then((t) => {
        const list = t
          .split("\n")
        ...
        const picks = [0, 1, 5, 11, 19, 29].filter((i) => i < list.length);
        const thumbnails = picks.length > 0 ? picks.map((i) => list[i]) : list.slice(0, 6);
  ...
  useEffect(() => {
    fetch("/Center Videos/ITC Grand Bharat/Video Gallery.txt")
      .then((r) => r.text())
      .then((t) => {
        const list = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Wellness & Rejuvenation Programs.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading ITC wellness content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading ITC medical content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Why Choose ITC Grand Bharat.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading ITC why-choose content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            current = { number: steps.length + 1, title, description: "", bullets: [] };
            inSection = true;
            continue;
          }
          if (!inSection) {
      ...
      .catch((err) => console.error("Error loading ITC treatment process content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
        ...
        for (const rawLine of lines) {
          ...
          if (!current) {
            if (seenHeading) introCollected.push(line);
      ...
      .catch((err) => console.error("Error loading ITC facilities content:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Images/ITC Grand Bharat/Facilities/Facilities.txt")
      .then((r) => r.text())
      .then((t) => {
        const list = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Founder and team.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("### ")) {
            ...
            if (/visionary stewards/i.test(t)) { section = "founder"; continue; }
            if (/your dedicated/i.test(t)) { if (currentGroup) groups.push(currentGroup); currentGroup = null; section = "teams"; continue; }
            if (section === "founder" && !name) { name = t; continue; }
          }
          if (line.startsWith("**") && line.endsWith("**")) {
            const t = line.slice(2, -2).trim();
            if (/our visionary leadership/i.test(t)) { continue; }
          ...
          if (section === "founder") {
            if (role === "" && line.length <= 80) { role = line; continue; }
      ...
      .catch((err) => console.error("Error loading ITC founder content:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Videos/ITC Grand Bharat/testimonies/Testimonies Links.txt")
      .then((r) => r.text())
      .then((t) => {
        const list = t
          .split("\n")
          .map((l) => l.trim())
          .filter(Boolean);
        const version = "2026-03-28";
        const withBust = list.map((u) => `${u}${u.includes("?") ? "&" : "?"}v=${version}`);
        setTestimonialVideos(withBust);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (current && /^\*\*rating:/i.test(line)) {
            const scoreMatch = line.match(/(\d)\s*\/\s*5/);
            if (scoreMatch) {
              current.rating = Number(scoreMatch[1]);
            } else {
              const starMatches = line.match(/[⭐â­]/g);
              current.rating = starMatches ? Math.min(5, starMatches.length) : 5;
            }
            continue;
          }
          if (line.startsWith("**") && line.endsWith("**")) {
            if (current) {
              items.push({ id, ...current });
              id += 1;
            }
            const heading = line.slice(2, -2);
            const [namePart, locationPart] = heading.split(" - ");
            current = {
              name: (namePart || "Guest").trim(),
          ...
          if (!current) continue;
          if (line.startsWith("*") && line.endsWith("*") && current.title === "") {
            current.title = line.replace(/^\*+|\*+$/g, "");
            continue;
          }
          current.review = current.review ? `${current.review} ${line}` : line;
        }
        if (current) items.push({ id, ...current });
        setTestimonials(items);
        setCurrentReview(0);
      })
      .catch((err) => console.error("Error loading ITC reviews:", err));
  }, []);
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("personalized") || s.includes("service") || s.includes("suites")) return <Users className={cls} />;
    if (s.includes("wellness") || s.includes("holistic")) return <HeartPulse className={cls} />;
  ...
  const getFacilityIcon = (title: string) => {
    ...
    return t.includes("ayurveda") || t.includes("treatment") || t.includes("spa") ? <Droplet className="h-7 w-7 text-white" />
      : t.includes("doctor") ? <Stethoscope className="h-7 w-7 text-white" />
        : t.includes("cottage") || t.includes("room") || t.includes("accommodation") || t.includes("suite") || t.includes("villa") ? <Building2 className="h-7 w-7 text-white" />
          : t.includes("restaurant") || t.includes("dining") || t.includes("meal") || t.includes("food") ? <Utensils className="h-7 w-7 text-white" />
            : t.includes("pool") || t.includes("recreation") || t.includes("swimming") ? <Activity className="h-7 w-7 text-white" />
              : t.includes("conference") || t.includes("group") || t.includes("event") || t.includes("banquet") || t.includes("ballroom") ? <Globe className="h-7 w-7 text-white" />
                : t.includes("travel") || t.includes("airport") || t.includes("station") ? <MapPin className="h-7 w-7 text-white" />
                  : t.includes("laundry") || t.includes("housekeeping") ? <Sparkles className="h-7 w-7 text-white" />
                    : t.includes("garden") || t.includes("nature") || t.includes("estate") || t.includes("landscape") || t.includes("hills") ? <TreePine className="h-7 w-7 text-white" />
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose ITC" },
  ...
  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading ITC insurance content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading ITC FAQs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/ITC Grand Bharat/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let section: "address" | "distance" | "transport" | "" = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            else if (t.includes("distance")) section = "distance";
            else if (t.includes("transport")) section = "transport";
            continue;
          }
          if (line.startsWith("*")) {
            const bullet = line.replace(/^\*+\s*/, "");
          ...
          if (section === "transport") transport = transport ? `${transport} ${line}` : line;
        }
        setContactAddress(address);
        setContactDistances(distances);
        setTransportText(transport);
      })
      .catch((err) => console.error("Error loading ITC contact content:", err));
  }, []);

  const awards = [
    {
      title: "International Top 100 of the World – Sustainability Awards 2025",
      description: "Recognition for sustainability leadership among top global properties.",
      image: "/Center Images/ITC Grand Bharat/Award/Award 1 (World's Top 100 Hotels ).webp",
    },
    {
      title: "World Luxury Hotel Awards 2024 Winner",
      description: "Honors excellence in luxury hospitality and guest experience worldwide.",
      image: "/Center Images/ITC Grand Bharat/Award/Award 2 (Best Luxury Hotel).png",
    },
    {
      title: "USGBC LEED Platinum Certification",
      description: "Top-tier LEED rating recognizing sustainable design and operational performance.",
      image: "/Center Images/ITC Grand Bharat/Award/Award 3 (LEED Platinum Certified).jpg",
    },
    {
      title: "Tripadvisor Travelers’ Choice Awards 2025",
      description: "Based on the quality and quantity of traveler reviews over 12 months.",
      image: "/Center Images/ITC Grand Bharat/Award/Award 4 (Travelers' Choice Award).webp",
    },
    {
      title: "NABH Accredited",
      description: "Accreditation program emphasizing quality and patient safety standards in healthcare.",
      image: "/Center Images/ITC Grand Bharat/Award/Award 5 (NABH).webp",
    },
  ];
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              ITC GRAND BHARAT WELLNESS RETREAT DELHI
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Luxury Heritage Wellness Retreat
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Gurugram (near New Delhi), India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.8</span>
                  <span className="opacity-90">(17,000+)</span>
                </div>
              </div>
                ...
                <>
                  <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                    <img
                      ...
                      alt={`ITC Grand Bharat ${selectedImage + 1}`}
                        ...
                        return (
                          ...
                          >
                            <div className="relative w-full rounded-xl overflow-hidden" style={{ paddingBottom: "100%" }}>
                              <img
                                src={img}
                                alt={`ITC Grand Bharat ${actualIndex + 1}`}
                      ...
                      <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                          {images.map((img, i) => (
                            ...
                            >
                              <img src={img} alt={`ITC Grand Bharat ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
                        ...
                        <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
                          <img
                            ...
                            alt={`ITC Grand Bharat ${lightboxImage + 1}`}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-12">
            <div className="bg-card rounded-lg border border-border shadow-sm">
              <div className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg">
                <MarkdownContent
                  contentPath="/content/Top Centers/ITC Grand Bharat/Main content.txt"
      ...
      <div className="container mx-auto px-3 md:px-4 pb-2 max-w-full">
        <div className="max-w-6xl mx-auto">
          <div className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }}>
            <div className="grid grid-cols-3 gap-3 md:gap-4 mb-8">
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-base md:text-3xl font-bold text-primary mb-1 whitespace-nowrap">17,000+</div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of ITC Grand Bharat
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Explore the atmosphere and wellness experience at ITC Grand Bharat through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">Why Choose ITC Grand Bharat for Your Healing Journey</h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of ITC Grand Bharat
              </h2>
            ...
            <div className="relative mx-auto px-4 md:px-0 max-w-[300px] md:max-w-[360px]">
              <Card className="border-2 border-primary/20 shadow-xl overflow-hidden bg-white rounded-3xl">
                <CardContent className="p-0">
                  <div className="w-full relative bg-black aspect-[9/16]">
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/ITC Grand Bharat/CTA bottom.jpg"
                    alt="ITC Grand Bharat"
                  ...
                  <p className="text-sm text-center mb-4" style={{ color: "#7F543D" }}>
                    Take the first step toward holistic wellness. Our team will guide you with personalized programs tailored to your needs.
                  </p>
                  <div className="space-y-3">
          ...
          <div className="mb-12 rounded-3xl p-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="team">
            <div className="text-center mb-6 md:mb-10">
              ...
              {teamIntro && (
                <p className="text-base md:text-lg mx-auto" style={{ color: "#7F543D" }}>{renderInlineBold(teamIntro)}</p>
              )}
            </div>
            <div className="grid md:grid-cols-2 gap-4 md:gap-8 items-stretch">
              <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all hover:shadow-xl h-full">
                <CardContent className="p-4 md:p-8 h-full md:h-[480px] flex flex-col">
                  ...
                  {founder?.description && (
                    <p className="text-xs md:text-sm leading-relaxed mb-3 md:mb-4" style={{ color: "#7F543D" }}>{renderInlineBold(founder.description)}</p>
                  )}
                ...
                <Card className="border-2 border-primary/20 hover:border-primary/50 transition-all hover:shadow-xl h-full">
                  <CardContent className="p-4 md:p-8 h-full md:h-[480px] md:overflow-y-auto">
                    ...
                    {teamGroups[currentTeamSlide]?.description && (
                      <p className="text-xs md:text-sm leading-relaxed mb-3 md:mb-4" style={{ color: "#7F543D" }}>{renderInlineBold(teamGroups[currentTeamSlide].description)}</p>
                    )}
            ...
            <div className="mb-12" id="reviews">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute left-0 md:left-3 top-1/2 -translate-y-1/2 bg-white hover:bg-primary hover:text-white text-primary p-2 md:p-3 rounded-full shadow-lg transition-all border-2 border-primary"
                ...
                <button
                  ...
                  className="absolute right-0 md:right-3 top-1/2 -translate-y-1/2 bg-white hover:bg-primary hover:text-white text-primary p-2 md:p-3 rounded-full shadow-lg transition-all border-2 border-primary"
                ...
                <div className="flex justify-center gap-2 mt-6">
                  {testimonials.map((_, idx) => (
                    <button
                      key={idx}
                      onClick={() => { setIsReviewAutoPlaying(false); setCurrentReview(idx); }}
          ...
          <div className="mb-12" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognition for excellence in sustainability, luxury hospitality, and guest experience.
              </p>
            </div>
                        ...
                        <div className="bg-white rounded-2xl p-4 md:p-6 shadow-lg border-2 border-primary/10 hover:border-primary/30 transition-all h-full flex flex-col items-center">
                          <div className="w-full aspect-square bg-primary/5 rounded-xl mb-4 p-2 flex items-center justify-center overflow-hidden">
                            <img
                              src={award.image}
                              alt={award.title}
                              className={`object-contain filter drop-shadow-md transition-transform duration-300 hover:scale-110 ${
                                award.title.startsWith("International Top 100")
                                  ? "max-h-[100%] max-w-[100%]"
                                  : "max-h-[90%] max-w-[90%]"
                              }`}
                            />
                      ...
                      <div key={i} className="w-1/3 flex-shrink-0 px-4">
                        <div className="bg-white rounded-2xl p-6 shadow-lg border-2 border-primary/10 hover:border-primary/30 transition-all h-full flex flex-col items-center">
                          ...
                          <div className="text-center">
                            ...
                            <p className="text-sm italic" style={{ color: "#7F543D" }}>"{award.description}"</p>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="ITC Grand Bharat Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3512.8676526452164!2d76.97373857427934!3d28.302332999000942!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x390d3bc188260ded%3A0xef0dab1010cac1b!2sITC%20Grand%20Bharat%2C%20Delhi%20NCR-%20LEED%20Platinum%20all%20suite%20luxury%20retreat!5e0!3m2!1sen!2sin!4v1774695087355!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/ITC Grand Bharat/CTA mid.jpg"
                    alt="ITC Grand Bharat"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at ITC Grand Bharat</h2>
                  <div className="space-y-3">
                    <Button
                      size="lg"
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-bold text-white mb-3">Begin Your Holistic Healing Journey at ITC Grand Bharat</h2>
                  <div className="flex flex-wrap gap-3">
          ...
          {facilityLightboxOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#EDE8D0]/80 backdrop-blur-sm">
              ...
              <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
                <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">ITC Grand Bharat</div>
                <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>

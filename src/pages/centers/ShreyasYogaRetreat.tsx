...
const ShreyasYogaRetreat = () => {
  ...
  const founderImage = "/Center Images/Shreyas Yoga Retreat/Founder and team/founder.jpg";
  const teamImage = "/Center Images/Shreyas Yoga Retreat/Founder and team/team.jpg";
  ...
  const awards = [
    ...
    },
    {
      ...
      description: "Recognized within the global World Luxury Spa Awards ecosystem for excellence in luxury spa and wellness.",
      image: "/Center Images/Shreyas Yoga Retreat/Awards/2 (worlds luxury spa).jpg"
    },
    {
      title: "NABH Accredited",
      description: "NABH recognition reflects alignment with structured quality systems, patient safety protocols, and continuous clinical governance practices.",
      image: "/Center Images/Shreyas Yoga Retreat/Awards/3 (NABH).png"
    },
    {
      title: "Tripadvisor Travelers' Choice",
      description: "Travelers' Choice recognition reflects consistently strong guest feedback, based on review quality and quantity over a 12-month cycle.",
      image: "/Center Images/Shreyas Yoga Retreat/Awards/4 (tripadvisior choice).png"
    }
  ];
  ...
  useEffect(() => {
    fetch("/Center Images/Shreyas Yoga Retreat/photo gallery/Photo Gallery Links.txt")
      ...
      .then((t) => {
        ...
        setSelectedImage(0);
        setLightboxImage(0);
        const pick = [0, 1, 5, 11, 19, 29].filter((i) => i < urls.length);
        setThumbnailImages(pick.map((i) => urls[i]));
      })
      .catch(() => {
        setImages([]);
        setThumbnailImages([]);
  ...
  useEffect(() => {
    fetch("/Center Videos/Shreyas Yoga Retreat/video gallery links.txt")
      .then((r) => r.text())
      .then((t) => {
        const urls = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/wellness program.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Shreyas wellness content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/medical program.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Shreyas medical content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Why Choose Shreyas Yoga Retreat.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Shreyas why-choose content:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Videos/Shreyas Yoga Retreat/testimonies/testomonies.txt")
      .then((r) => r.text())
      .then((t) => {
        const urls = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/Center Images/Shreyas Yoga Retreat/Facilities/Facilities.txt")
      .then((r) => r.text())
      .then((t) => {
        const urls = t
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            if (current) steps.push(current);
            autoStep += 1;
            current = { number: autoStep, title: line.slice(2, -2), description: "", bullets: [] };
            inSection = true;
            continue;
          }
          if (!inSection) {
      ...
      .catch((err) => console.error("Error loading Shreyas treatment content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
      ...
      .catch((err) => console.error("Error loading Shreyas facilities content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("### ")) {
            ...
            if (/founder/i.test(t)) { section = "founder"; continue; }
            if (/expert wellness team|expert team|our expert/i.test(t)) { if (currentGroup) groups.push(currentGroup); currentGroup = null; section = "teams"; continue; }
          }
          if (line.startsWith("**") && line.endsWith("**")) {
            const t = line.slice(2, -2);
            if (/leadership & expertise/i.test(t)) { section = "expertise"; continue; }
            if (/founder|team info/i.test(t)) { continue; }
            if (section === "founder") {
              name = t;
              continue;
            }
          ...
          } else if (section === "teams" && currentGroup) {
            currentGroup.description = currentGroup.description ? `${currentGroup.description} ${line}` : line;
          }
        }
        if (currentGroup) groups.push(currentGroup);
        setTeamIntro(intro);
        setFounder({ name, degrees, role, description: fdesc });
        setFounderExpertise(expertise);
        setTeamGroups(groups);
        setCurrentTeamSlide(0);
      })
      .catch((err) => console.error("Error loading Shreyas founder content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (header.toLowerCase().startsWith("rating")) continue;
            pushCurrent();
            const [namePart, locationPart] = header.split(" - ");
            current = {
              name: (namePart || "").trim(),
          ...
          if (line.toLowerCase().startsWith("**rating")) {
            ...
            if (current) current.rating = match ? parseInt(match[1], 10) : 5;
            continue;
          }
          if (current) {
      ...
      .catch((err) => console.error("Error loading Shreyas reviews:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Shreyas insurance content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Shreyas FAQs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Shreyas Yoga Retreat/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Shreyas contact info:", err));
  }, []);
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("weight") || s.includes("metabolic")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("diabetes") || s.includes("thyroid") || s.includes("hormone")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("pain") || s.includes("joint") || s.includes("arthritis")) return <Pill className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("skin") || s.includes("beauty")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("neurology") || s.includes("brain") || s.includes("stress")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("luxury") || s.includes("excellence") || s.includes("awards")) return <Award className={cls} />;
    if (s.includes("ashram") || s.includes("elegant")) return <Home className={cls} />;
    if (s.includes("authentic") || s.includes("holistic yoga") || s.includes("yoga")) return <HeartPulse className={cls} />;
    if (s.includes("personal") || s.includes("tailored")) return <UserCheck className={cls} />;
    if (s.includes("cuisine") || s.includes("gourmet") || s.includes("farm-to-table")) return <Utensils className={cls} />;
    if (s.includes("sanctuary") || s.includes("nature") || s.includes("gardens")) return <Leaf className={cls} />;
    if (s.includes("integrated") || s.includes("ayurveda") || s.includes("naturopathic")) return <Stethoscope className={cls} />;
    if (s.includes("seva") || s.includes("giving")) return <Heart className={cls} />;
    if (s.includes("global") || s.includes("acclaimed")) return <Hospital className={cls} />;
  ...
  useEffect(() => {
    if (!isReviewAutoPlaying || testimonials.length <= 1) return;
    const id = setInterval(() => {
      ...
      <div className="container mx-auto px-3 md:px-4 py-12 max-w-full" id="gallery">
        <div className="max-w-6xl mx-auto w-full">
            ...
            <>
              <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                <img
                  ...
                  alt={`Shreyas Center ${selectedImage + 1}`}
                  ...
                  <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {images.map((img, i) => (
                        ...
                        >
                          <img src={img} alt={`Shreyas ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="mb-12 mt-6" id="videos" ref={videoGallerySectionRef}>
            <div className="text-center mb-10">
              ...
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Discover the atmosphere and healing experience at Shreyas Yoga Retreat through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12 mt-6" id="testimonial-videos" ref={testimonialSectionRef}>
            ...
            <div className="relative max-w-sm md:max-w-md mx-auto px-4 md:px-0">
              <Card className="border-2 border-primary/20 shadow-xl overflow-hidden bg-white rounded-3xl">
                <CardContent className="p-0">
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Shreyas Yoga Retreat/CTA mid.jpg"
                    alt="Shreyas Yoga Retreat"
          ...
          <div className="mb-12 mt-6" id="awards">
            <div className="text-center mb-6 md:mb-10">
              ...
              <p className="text-base md:text-lg px-4" style={{ color: "#7F543D" }}>
                Recognition of Shreyas Retreat across awards and respected media features.
              </p>
            </div>
                    ...
                    {contactPhones.length > 0 && (
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <ul className="space-y-1 text-sm md:text-base leading-relaxed" style={{ color: "#7F543D" }}>
                    ...
                    {contactWebsite && (
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm md:text-base leading-relaxed break-all" style={{ color: "#7F543D" }}>{contactWebsite}</p>
                        </div>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Shreyas Yoga Retreat Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d6725.3353728951315!2d77.40461460880167!3d13.15816072252238!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae250f3a266e0d%3A0x82b5420eadaab237!2sShreyas%20Yoga%20Retreat!5e0!3m2!1sen!2sin!4v1772891061998!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Shreyas Yoga Retreat/CTA bottom.jpg"
                    alt="Shreyas Yoga Retreat"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at Shreyas Yoga Retreat</h2>
                  <div className="space-y-3">
                    <Button
                      size="lg"
                  ...
                  {contactPhones.length > 0 && (
                    <div className="mt-4 flex items-center justify-center gap-2 text-white/90 text-sm">
                      ...
                      <a href={`tel:${contactPhones[0].replace(/[^\d+]/g, "")}`} className="underline hover:text-white">
                        Call us: {contactPhones[0]}
                      </a>
                    </div>
                  )}
                </div>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-bold text-white mb-3">Begin Your Holistic Healing Journey at Shreyas Yoga Retreat</h2>
                  <div className="flex flex-wrap gap-3">

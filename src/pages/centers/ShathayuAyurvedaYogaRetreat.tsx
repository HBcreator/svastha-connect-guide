...
export default function ShathayuAyurvedaYogaRetreat() {
  ...
  const founderImage = "/Center Images/Shathayu Ayurveda Yoga Retreat/Founder/founder.jpg";
  const teamImage = "/Center Images/Shathayu Ayurveda Yoga Retreat/Founder/team.jpg";
  ...
  const awards = [
    {
      ...
      description: "Recognized for adhering to quality benchmarks in patient safety, clinical excellence, and holistic care delivery.",
      image: "/Center Images/Shathayu Ayurveda Yoga Retreat/Awards/Award 1 (NABH).webp",
    },
    {
      title: "ISO Certification",
      description: "Certified for consistent quality standards and structured processes that support reliable patient experiences.",
      image: "/Center Images/Shathayu Ayurveda Yoga Retreat/Awards/Award 2 (ISO certified).webp",
    },
    {
      title: "Tripadvisor Recognition",
      description: "Celebrated by guests for service quality, wellness outcomes, and an exceptional retreat experience.",
      image: "/Center Images/Shathayu Ayurveda Yoga Retreat/Awards/Award 3 (tripadvisor).jpg",
    },
  ];
  ...
  useEffect(() => {
    ...
    fetch("/content/Top Centers/Shathayu Ayurveda Yoga Retreat/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Shathayu founder content:", err));

    fetch("/content/Top Centers/Shathayu Ayurveda Yoga Retreat/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Shathayu reviews:", err));

    fetch("/content/Top Centers/Shathayu Ayurveda Yoga Retreat/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            const t = line.slice(2, -2).trim();
            if (/insurance/i.test(t)) section = "ins";
            else if (/payment/i.test(t)) section = "pay";
            else if (/international/i.test(t)) section = "intl";
            continue;
          }
          if (line.startsWith("*")) {
            const b = line.replace(/^\*+\s*/, "");
            if (section === "ins") ins.push(b);
            else if (section === "pay") pay.push(b);
            continue;
          }
      ...
      .catch((err) => console.error("Error loading Shathayu insurance content:", err));

    fetch("/content/Top Centers/Shathayu Ayurveda Yoga Retreat/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            const t = line.slice(2, -2).trim();
            if (/address/i.test(t)) section = "address";
            else if (/distance/i.test(t)) section = "distances";
            else if (/transport/i.test(t)) section = "transport";
            else section = "none";
            continue;
          }
      ...
      .catch((err) => console.error("Error loading Shathayu contact info:", err));

    fetch("/content/Top Centers/Shathayu Ayurveda Yoga Retreat/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Shathayu FAQs:", err));
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Shathayu" },
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-lg">Udupi, Banglore, India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.8</span>
                  <span className="opacity-90">(500+)</span>
                </div>
              </div>
                  ...
                  <CardContent className="p-6">
                    <div className="space-y-3">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary group-hover:scale-110 transition-all duration-300 flex-shrink-0 [&>svg]:text-primary group-hover:[&>svg]:text-white [&>svg]:transition-colors">
          ...
          <div className="mb-12" id="awards">
            ...
            <div className="relative group max-w-5xl mx-auto">
              ...
              <button
                ...
                className="absolute left-8 top-[57%] -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 rounded-full shadow-lg transition-all border-2 border-primary z-10 md:hidden"
              ...
              <button
                ...
                className="absolute right-8 top-[57%] -translate-y-1/2 bg-white/90 hover:bg-primary hover:text-white text-primary p-2 rounded-full shadow-lg transition-all border-2 border-primary z-10 md:hidden"
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Shathayu Ayurveda Yoga Retreat Map"
                            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3882.72489315023!2d77.71250147412597!3d13.305117907480348!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae1d7f8b7fda03%3A0x28a8106b0305f722!2sShathayu%20Ayurveda%20Yoga%20Retreat!5e0!3m2!1sen!2sin!4v1771842294774!5m2!1sen!2sin"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Shathayu Ayurveda Yoga Retreat/CTA bottom.jpg"
                    alt="Shathayu Ayurveda Yoga Retreat"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
          ...
          <div className="p-4 pb-4 bg-[#2F5B63] text-white relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-3 relative z-10">
              <div className="space-y-0.5">
                ...
                <h2 className="text-[26px] font-extrabold leading-tight tracking-tight whitespace-nowrap text-white">Sections of Shathayu</h2>
              </div>
              <button
                onClick={() => setIsJumpModalOpen(false)}

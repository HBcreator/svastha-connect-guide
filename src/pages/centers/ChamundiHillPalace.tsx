...
export default function ChamundiHillPalace() {
  ...
  const [isReviewAutoPlaying] = useState(true);
  ...
  const jumpSections = [
    ...
    { id: "videos", title: "Video Gallery of Chamundi Hill Palace" },
    { id: "why-choose", title: "Why Choose Chamundi Hill Palace" },
    { id: "testimonial-videos", title: "Testimonials of Chamundi Hill Palace Center" },
    { id: "treatment-process", title: "Treatment Process & Patient Journey" },
    { id: "facilities", title: "Facilities & Amenities" },
    { id: "team", title: "Founder & Team Info" },
    { id: "patient-stories", title: "Patient Stories & Reviews" },
  ...
  const awards = [
    {
      ...
      description: "Recognized for delivering authentic Ayurvedic therapies and maintaining high standards of wellness care.",
      image: "/Center Images/Chamundi Hill Palace/Awards/Award 1 (greenleaf).webp",
    },
    {
      title: "Travelers' Choice",
      description: "Honored by global guests for outstanding experiences, hospitality, and consistently high ratings.",
      image: "/Center Images/Chamundi Hill Palace/Awards/Award 2 (travelerschoice).webp",
    },
    {
      title: "Tripadvisor Recognition",
      description: "Celebrated for excellence in guest satisfaction and a strong track record of positive wellness outcomes.",
      image: "/Center Images/Chamundi Hill Palace/Awards/Award 3 (tripadvisor).webp",
    },
    {
      title: "Tripadvisor 2024",
      description: "A renewed recognition reflecting ongoing commitment to quality care, service, and transformational healing.",
      image: "/Center Images/Chamundi Hill Palace/Awards/Award 4 (tripadvisor2024).webp",
    },
  ];
  ...
  useEffect(() => {
    fetch("/Center Images/Chamundi Hill Palace/Photo Gallery/Photo Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const imgs = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Chamundi Hill Palace images:", err));

    fetch("/Center Videos/Chamundi Hill Palace/Gallery Video links.txt")
      .then((res) => res.text())
      .then((text) => {
        const vids = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Chamundi Hill Palace videos:", err));

    fetch("/content/Top Centers/Chamundi Hill Palace/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    fetch("/content/Top Centers/Chamundi Hill Palace/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    fetch("/content/Top Centers/Chamundi Hill Palace/Why Choose Chamundi Hill Palace.txt")
      .then((res) => res.text())
      .then((text) => setWhyChooseData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Why Choose:", err));

    fetch("/Center Videos/Chamundi Hill Palace/Testimonies Video Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const matches = Array.from(text.matchAll(/src\s*=\s*"([^"]+)"/g)).map((m) => m[1]);
        const unique = Array.from(new Set(matches)).filter(Boolean);
        setTestimonialVideos(unique);
        setSelectedTestimonialVideo(0);
      })
      .catch((err) => console.error("Error loading testimonial videos:", err));

    fetch("/content/Top Centers/Chamundi Hill Palace/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          const stepTitleMatch = line.match(/^\d+\.\s*\*\*(.+)\*\*$/);
          if (stepTitleMatch) {
            if (current) items.push(current);
            current = { title: stepTitleMatch[1].trim(), description: "", bullets: [] };
            inSteps = true;
            continue;
          }
    ...
    fetch("/content/Top Centers/Chamundi Hill Palace/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => setFacilitiesData(parseCardSection(text)))
      .catch((err) => console.error("Error loading Facilities:", err));

    fetch("/Center Images/Chamundi Hill Palace/Facilities/Facilities Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const imgs = text
          .split("\n")
    ...
    fetch("/content/Top Centers/Chamundi Hill Palace/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => setFounderTeamData(parseFounderTeamSection(text)))
      .catch((err) => console.error("Error loading Founder & Team Info:", err));

    fetch("/content/Top Centers/Chamundi Hill Palace/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
    ...
    fetch("/content/Top Centers/Chamundi Hill Palace/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
    ...
    fetch("/content/Top Centers/Chamundi Hill Palace/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
    ...
    fetch("/content/Top Centers/Chamundi Hill Palace/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  const processIconForTitle = (t: string) => {
    ...
    if (s.includes("daily") || s.includes("therapy") || s.includes("treatment") || s.includes("medicine") || s.includes("monitor")) {
    ...
    if (s.includes("mind") || s.includes("yoga") || s.includes("meditation") || s.includes("harmony") || s.includes("mountain")) {
  ...
  const getFacilityIcon = (t: string) => {
    ...
    if (s.includes("architecture") || s.includes("design") || s.includes("building") || s.includes("palace"))
      return <Building2 className="h-7 w-7 text-white" />;

    if (s.includes("farm") || s.includes("organic") || s.includes("garden") || s.includes("tea") || s.includes("plantation"))
      return <TreePine className="h-7 w-7 text-white" />;

    if (s.includes("infrastructure") || s.includes("treatment") || s.includes("therapy") || s.includes("pharmacy") || s.includes("vaidya"))
    ...
    if (s.includes("dining") || s.includes("cuisine") || s.includes("food") || s.includes("meal"))
      return <Utensils className="h-7 w-7 text-white" />;

    if (s.includes("recreation") || s.includes("pool") || s.includes("swimming") || s.includes("activity") || s.includes("walk")) {
    ...
    if (s.includes("services") || s.includes("housekeeping") || s.includes("laundry") || s.includes("guest") || s.includes("transfer") || s.includes("transport"))
  ...
  const withAutoplayParams = (url: string) => {
    ...
    return `${url}${joiner}autoplay=${isTestimonialsInView ? "1" : "0"}&mute=0&rel=0`;
  };
  ...
  useEffect(() => {
    if (!lightboxOpen && !showFullGallery && !facilityLightboxOpen) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (facilityLightboxOpen) {
        if (e.key === "ArrowLeft") setFacilityLightboxImage((prev) => (prev - 1 + facilityImages.length) % facilityImages.length);
        if (e.key === "ArrowRight") setFacilityLightboxImage((prev) => (prev + 1) % facilityImages.length);
      }
    };

    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [lightboxOpen, showFullGallery, facilityLightboxOpen, images.length, facilityImages.length]);
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-3 md:px-4 max-w-full">
          <div className="max-w-6xl mx-auto">
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                <h1 className="text-4xl md:text-5xl font-bold mb-4">Chamundi Hill Palace</h1>
                <p className="text-xl mb-4 opacity-90">Ayurvedic Resort</p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Edakkunnam, Kerala</span>
                </div>
        ...
        <div className="max-w-6xl mx-auto">
            ...
            {!showTopVideoGallery ? (
                ...
                <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                  {images.length > 0 && (
                    <img
                      ...
                      alt={`Chamundi Hill Palace ${selectedImage + 1}`}
              ...
              <>
                <div className="relative rounded-lg overflow-hidden shadow-lg bg-black aspect-video mb-8">
                  <video
                    key={videos[selectedVideo] || selectedVideo}
                    controls
          ...
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/Chamundi Hill Palace/Main content.txt"
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">Video Gallery of Chamundi Hill Palace</h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene atmosphere and holistic healing journey at Chamundi Hill Palace through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {whyChooseData?.title || "Why Choose Chamundi Hill Palace?"}
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Chamundi Hill Palace Center
              </h2>
            ...
            <div className="relative max-w-4xl mx-auto px-4 md:px-0">
                ...
                <CardContent className="p-0">
                  <div className="aspect-video w-full relative">
                    {testimonialVideos.length > 0 && (
                      <iframe
                        key={`${testimonialVideos[selectedTestimonialVideo]}-${isTestimonialsInView ? "in" : "out"}`}
                        src={withAutoplayParams(testimonialVideos[selectedTestimonialVideo])}
                        title="Chamundi Hill Palace Testimonial Video"
              ...
              <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between px-2 md:-mx-8 pointer-events-none">
                <button
                  onClick={() =>
                    setSelectedTestimonialVideo((prev) =>
                      testimonialVideos.length ? (prev - 1 + testimonialVideos.length) % testimonialVideos.length : 0
                    )
                  }
                  className="bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary pointer-events-auto"
                ...
                <button
                  onClick={() =>
                    setSelectedTestimonialVideo((prev) => (testimonialVideos.length ? (prev + 1) % testimonialVideos.length : 0))
                  }
                  className="bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary pointer-events-auto"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Chamundi Hill Palace/CTA.jpg"
                    alt="Chamundi Hill Palace Wellness Center"
          ...
          <div className="mb-12" id="facilities">
            ...
            <div className="max-w-7xl mx-auto relative mb-10">
              <button
                onClick={() =>
                  setCurrentFacilityImage((prev) => (facilityImages.length ? (prev - 1 + facilityImages.length) % facilityImages.length : 0))
                }
              ...
              <button
                onClick={() => setCurrentFacilityImage((prev) => (facilityImages.length ? (prev + 1) % facilityImages.length : 0))}
                  ...
                  >
                    {facilityImages.map((image, index) => (
                      <div key={index} className="w-full flex-shrink-0 px-2">
                        ...
                        >
                          <img src={image} alt={`Chamundi Facility ${index + 1}`} className="w-full aspect-video object-cover rounded-lg" />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at Chamundi Hill Palace is designed and maintained according to the highest standards, ensuring safety, hygiene, and quality care. Our commitment to excellence means you receive world-class holistic treatment in a serene, naturally therapeutic environment.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mb-10 rounded-3xl p-4 md:p-10" style={{ backgroundColor: "#EDE8D0" }} id="team">
            <div className="text-center mb-6 md:mb-10">
              <h1 className="text-2xl md:text-4xl font-bold text-primary mb-3">{founderTeamData?.title || "Founder & Team Info"}</h1>
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                  ...
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    >
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/Chamundi Hill Palace/founder/founder.jpg"
                ...
                <CardContent className="p-4 md:p-8">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    >
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/Chamundi Hill Palace/founder/Team.jpg"
          ...
          <div className="mb-12" id="patient-stories">
          ...
          <div className="mb-12" id="cta-bottom">
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Chamundi Hill Palace/CTA bottom.jpg"
                    alt="Chamundi Hill Palace"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at Chamundi Hill Palace</h2>
                  <div className="space-y-3">
                    <Button size="lg" className="w-full rounded-full bg-white text-primary hover:bg-white/90 text-sm sm:text-base" onClick={() => setQuoteModalOpen(true)}>
                      <Phone className="mr-2 h-5 w-5" />
                      Book Consultation Now
                    </Button>
                    <Button
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h3 className="text-2xl md:text-4xl font-bold text-white mb-3">Begin Your Holistic Healing Journey at Chamundi Hill Palace</h3>
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <Button size="lg" variant="outline" className="rounded-full px-6 border-2 border-white/60 bg-transparent text-white hover:bg-orange-500 hover:border-orange-500" onClick={() => setQuoteModalOpen(true)}>
                      <MessageCircle className="mr-2 h-5 w-5" />
                      Chat With Us
                    </Button>
                  </div>
      ...
      {facilityLightboxOpen && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Chamundi Hill Palace</div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 text-white px-3 py-1 rounded-full text-sm">
                {facilityImages.length > 0 ? `${facilityLightboxImage + 1} / ${facilityImages.length}` : "0 / 0"}
              </div>
            </div>

            {/* Navigation Buttons - Mobile Only (Moved inside the card) */}
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 text-white px-3 py-1 rounded-full text-sm">
                {images.length > 0 ? `${lightboxImage + 1} / ${images.length}` : "0 / 0"}
              </div>
            </div>
            <div className="flex md:hidden items-center justify-between mt-4">
              <Button
      ...
      {/* Floating Quote Action (Bottom Right) - Hidden when lightbox/gallery is open */}
      {!lightboxOpen && !showFullGallery && !facilityLightboxOpen && (
        <button
          onClick={() => setQuoteModalOpen(true)}
        ...
        >
          ...
          <div className="p-4 text-center border-t border-primary/5 bg-[#F9F8F4]">
            ...
            <p className="text-[10px] font-bold text-primary/40 uppercase tracking-[0.3em] select-none">
              Royal Heritage Healing Sanctuary
            </p>
          </div>
        </div>
      </div>

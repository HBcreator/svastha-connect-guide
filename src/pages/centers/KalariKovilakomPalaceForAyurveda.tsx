...
export default function KalariKovilakomPalaceForAyurveda() {
  ...
  const founderImage = "/Center Images/Kalari Kovilakom/Founder/Founder.jpg";
  const teamImage = "/Center Images/Kalari Kovilakom/Founder/team stock image.jpg";
  ...
  const awards = [
    {
      title: "National Tourism Awards (Heritage Category)",
      description: "CGH Earth’s heritage portfolio has been recognized at India’s National Tourism Awards for excellence in heritage hospitality.",
      image: "/Center Images/Kalari Kovilakom/Awards/Award 1 (National Tourism Award).webp",
    },
    {
      title: "Condé Nast Traveler Gold List 2018",
      description: "Kalari Kovilakom is featured in Condé Nast Traveler’s Gold List 2018 for standout stays worldwide.",
      image: "/Center Images/Kalari Kovilakom/Awards/Award 2 (Condé Nast Traveler Gold List).jpg",
    },
    {
      title: "NABH Accreditation",
      description: "NABH accreditation reflects nationally recognized standards for patient safety and clinical quality.",
      image: "/Center Images/Kalari Kovilakom/Awards/Award 3 (NABH Accreditation).jpg",
    },
    {
      title: "SKÅL Sustainable Tourism Award",
      description: "CGH Earth received the SKÅL Sustainable Tourism Award in the Rural Accommodation category for responsible practices.",
      image: "/Center Images/Kalari Kovilakom/Awards/Award 4 (SKAL Sustainable Tourism Award ).jpg",
    },
    {
      title: "Tripadvisor Travelers’ Choice",
      description: "Travelers’ Choice recognizes properties in the top 10% of listings based on consistently great reviews.",
      image: "/Center Images/Kalari Kovilakom/Awards/Award 5 (Tripadvisor Travelers' Choice Award).jfif",
    },
  ];
  ...
  useEffect(() => {
    fetch(encodeURI("/Center Images/Kalari Kovilakom/Photo Gallery/Photo Gallery Links.txt"))
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Kalari photo gallery:", err));

    fetch(encodeURI("/Center Videos/Kalari Kovilakom/Video Gallery Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
        setVideos(lines);
      ...
      .catch((err) => console.error("Error loading Kalari video gallery:", err));

    fetch(encodeURI("/Center Videos/Kalari Kovilakom/Testimonies/Testimonies Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Kalari Kovilakom/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Kalari wellness content:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Kalari medical content:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Why Choose Kalari Kovilakom.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Kalari why-choose content:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Kalari treatment process content:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
      ...
      .catch((err) => console.error("Error loading Kalari facilities content:", err));

    fetch(encodeURI("/Center Images/Kalari Kovilakom/Facilities/Facilities Links.txt"))
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      ...
      .catch((err) => console.error("Error loading Kalari facilities images:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("### ")) {
            ...
            if (/visionary founder/i.test(t)) {
              section = "founder";
              continue;
            }
            if (/our expert healing team/i.test(t)) {
          ...
          if (section === "founder") {
            if (/MD|BAMS|Ayurveda|MRCH|Hom|MBA|MSc|PhD/i.test(line) && line.length <= 70) {
              degrees.push(line);
              continue;
            }
            if (/CEO|Chief|Director|Physician|Co-Founder/i.test(line) && role === "") {
              role = line;
              continue;
            }
      ...
      .catch((err) => console.error("Error loading Kalari founder content:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Kalari reviews:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Kalari insurance content:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Kalari FAQs:", err));

    fetch("/content/Top Centers/Kalari Kovilakom/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Kalari contact info:", err));
  }, []);
  ...
  const wellnessIconForTitle = (t: string) => {
    ...
    if (s.includes("deep healing")) return <Droplet className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("transformative")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("anti-stress") || s.includes("stress")) return <Moon className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("healthy aging") || s.includes("aging") || s.includes("rasayana")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("weight")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("purification")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("self-discovery")) return <Compass className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("mindful living") || s.includes("mindful")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("inner stillness") || s.includes("stillness")) return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("pure diet") || s.includes("diet")) return <Utensils className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("surrender")) return <Home className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("deep")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  };

  const medicalIconForTitle = (t: string) => {
    const s = t.toLowerCase();
    if (s.includes("stress") || s.includes("anxiety") || s.includes("insomnia")) return <Moon className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("degenerative") || s.includes("arthritic") || s.includes("joint") || s.includes("spondyl")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("metabolic") || s.includes("weight") || s.includes("gut")) return <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("panchakarma") || s.includes("detox")) return <Droplet className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("autoimmune") || s.includes("skin") || s.includes("psoriasis") || s.includes("eczema")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("rasayana") || s.includes("anti-aging") || s.includes("rejuvenation")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("chemotherapy")) return <Pill className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("digestive") || s.includes("ibs") || s.includes("acidity")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("gynec") || s.includes("hormonal") || s.includes("pcos") || s.includes("menop")) return <Users className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("neuro")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("liver") || s.includes("gallbladder")) return <Leaf className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  ...
  const whyIconForTitle = (t: string) => {
    ...
    if (s.includes("purest") || s.includes("authentic")) return <Stethoscope className={cls} />;
    if (s.includes("hospital") || s.includes("nabh")) return <ShieldCheck className={cls} />;
    if (s.includes("prescribed") || s.includes("path")) return <ClipboardList className={cls} />;
    if (s.includes("historic") || s.includes("palace")) return <Home className={cls} />;
    if (s.includes("lifestyle") || s.includes("discipline")) return <HeartPulse className={cls} />;
    if (s.includes("herbal") || s.includes("medicines")) return <Leaf className={cls} />;
    if (s.includes("transformation")) return <Sparkles className={cls} />;
    if (s.includes("serenity") || s.includes("silence")) return <Compass className={cls} />;
    if (s.includes("legacy") || s.includes("cgh")) return <Award className={cls} />;
  ...
  const getFacilityIcon = (title: string) => {
    ...
    return t.includes("ayurveda") || t.includes("treatment") ? <Droplet className="h-7 w-7 text-white" />
      : t.includes("doctor") ? <Stethoscope className="h-7 w-7 text-white" />
        : t.includes("palace") || t.includes("room") || t.includes("accommodation") ? <Building2 className="h-7 w-7 text-white" />
          : t.includes("dining") || t.includes("meal") || t.includes("diet") ? <Utensils className="h-7 w-7 text-white" />
            : t.includes("yoga") || t.includes("meditation") || t.includes("chanting") ? <Activity className="h-7 w-7 text-white" />
              : t.includes("library") || t.includes("study") ? <Globe className="h-7 w-7 text-white" />
                : t.includes("garden") || t.includes("courtyard") || t.includes("nature") ? <TreePine className="h-7 w-7 text-white" />
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose Kalari" },
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              KALARI KOVILAKOM AYURVEDA HOSPITAL KERALA
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-xl mb-4 opacity-90">
                  Authentic Ayurveda in a Heritage Palace
                </p>
                <div className="flex items-center gap-2 mb-4">
                  <MapPin className="h-5 w-5" />
                  <span className="text-lg">Palakkad, Kerala, India</span>
                </div>
            ...
            <>
              <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                <img
                  ...
                  alt={`Kalari Center ${selectedImage + 1}`}
                  ...
                  <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {images.map((img, i) => (
                        ...
                        >
                          <img src={img} alt={`Kalari ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {lightboxOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#EDE8D0]/80 backdrop-blur-sm">
                  ...
                  <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
                    <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Kalari Kovilakom</div>
                    <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
                      <img
                        src={images[lightboxImage]}
                        alt={`Kalari ${lightboxImage + 1}`}
        ...
        <div className="max-w-6xl mx-auto">
          <Card className="mb-12">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg">
              <MarkdownContent
                contentPath="/content/Top Centers/Kalari Kovilakom/Main content.txt"
          ...
          <div className="mb-12 rounded-3xl p-6 md:p-6 lg:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
            <div className="grid grid-cols-3 gap-2 md:gap-6 max-w-3xl mx-auto mb-8 md:mb-10 overflow-hidden">
              ...
              <div className="text-center p-2.5 md:p-4 bg-white/60 rounded-xl">
                ...
                <div className="text-xs md:text-sm" style={{ color: "#7F543D" }}>Wellness Outcomes</div>
              </div>
            </div>
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                Video Gallery of Kalari Kovilakom
              </h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the heritage atmosphere and authentic Ayurveda at Kalari Kovilakom through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">
                Why Choose Kalari Kovilakom for Your Healing Journey
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos">
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Kalari Kovilakom
              </h2>
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/Kalari Kovilakom/CTA mid.jpg"
                    alt="Kalari Kovilakom"
                    ...
                    <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                      <div className="rounded-xl overflow-hidden">
                        <div className="relative w-full aspect-[800/600]">
                          <iframe
                            title="Kalari Kovilakom Map"
                            src="https://maps.google.com/maps?q=Kalari%20Kovilakom%2C%20Kollengode%2C%20Palakkad%2C%20Kerala&t=&z=13&ie=UTF8&iwloc=&output=embed"
            ...
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="max-w-sm mx-auto bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/Kalari Kovilakom/CTA bottom.jpg"
                    alt="Kalari Kovilakom"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-bold text-white text-center mb-4">Begin Your Holistic Healing Journey at Kalari Kovilakom</h2>
                  <div className="space-y-3">
                    <Button
                      size="lg"
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-bold text-white mb-3">Begin Your Holistic Healing Journey at Kalari Kovilakom</h2>
                  <div className="flex flex-wrap gap-3">

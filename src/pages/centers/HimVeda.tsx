...
const parseCardSection = (text: string): SectionData => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("* ")) {
      if (currentCard) currentCard.bullets.push(line.replace("* ", "").trim());
      continue;
    }
...
const parseFounderTeamInfo = (text: string): FounderTeamInfoData => {
  ...
  while (i < lines.length) {
    ...
    if (line === "**Our Physician-Founders**") {
      i++;
      while (i < lines.length && !lines[i]) i++;
      if (i < lines.length) {
        data.founderName = lines[i];
        i++;
      }
      ...
      while (i < lines.length) {
        ...
        if (t.startsWith("**Leadership") && t.endsWith("**")) break;
        if (t.startsWith("**Our Expert Healing Team**")) break;
        if (t.startsWith("**") && t.endsWith("**")) break;
        data.founderBio = data.founderBio ? `${data.founderBio} ${t}` : t;
        i++;
      }
      continue;
    }
    ...
    if (line === "**Our Expert Healing Team**") {
...
const getFacilityIcon = (t: string) => {
  ...
  if (s.includes("farm") || s.includes("organic") || s.includes("garden") || s.includes("acre") || s.includes("forest") || s.includes("nature"))
    return <TreePine className="h-7 w-7 text-white" />;

  if (s.includes("infrastructure") || s.includes("treatment") || s.includes("therapy") || s.includes("pharmacy") || s.includes("clinical"))
    return <Stethoscope className="h-7 w-7 text-white" />;

  if (s.includes("yoga") || s.includes("meditation") || s.includes("hall") || s.includes("mindful") || s.includes("pavilion"))
  ...
  if (s.includes("services") || s.includes("housekeeping") || s.includes("laundry") || s.includes("guest") || s.includes("transfer") || s.includes("support"))
...
const HimVeda = () => {
  ...
  const awards = [
    {
      ...
      description: "Rated by guests for exceptional wellness experiences and trusted healing results.",
      image: "/Center Images/HimVeda/Awards/Award 1 (Tripadvisor).png",
    },
    {
      title: "Trustindex Verified",
      description: "Verified reviews and reputation across leading platforms, reflecting consistent guest satisfaction.",
      image: "/Center Images/HimVeda/Awards/Award 2 ( Trustindex verified status,).jfif",
    },
    {
      title: "Proven Clinical Success",
      description: "Recognized for strong outcomes in supporting complex, chronic health journeys through authentic Ayurveda.",
      image: "/Center Images/HimVeda/Awards/Award 3 (proven success in treating complex chronic conditions ).webp",
    },
  ];
  ...
  const jumpSections = [
    ...
    { id: "why-choose", title: "Why Choose HimVeda" },
    ...
    const fetchMedia = async () => {
      try {
        const [imgRes, vidRes] = await Promise.all([
          fetch("/Center Images/HimVeda/Photo Gallery/Photo Links.txt"),
          fetch("/Center Videos/HimVeda/Videos links.txt"),
        ]);

        const [imgText, vidText] = await Promise.all([
          imgRes.text(),
          vidRes.text(),
        ]);

        const imageLinks = imgText
          .split("\n")
          .map((l) => l.trim())
          .filter(Boolean);
        const videoLinks = vidText
          .split("\n")
          .map((l) => l.trim())
          .filter(Boolean);
  ...
  useEffect(() => {
    fetch("/Center Videos/HimVeda/Testimonies/testimonies.txt")
      .then((res) => res.text())
      .then((text) => {
        const links = text
          .split("\n")
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        const addr: string[] = [];
        const dists: string[] = [];
        let transport = "";
        let section: "none" | "addr" | "dist" | "transport" = "none";
        ...
        for (const line of lines) {
          ...
          if (section === "dist" && line.startsWith("*")) {
            const cleaned = line.replace(/^\*+\s*/, "").trim();
            if (cleaned) dists.push(cleaned);
            continue;
          }
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**Insurance Coverage**")) {
            section = "ins";
            continue;
          }
          if (line.startsWith("**Payment Options**")) {
            section = "pay";
            continue;
          }
          if (line.startsWith("**For International Patients**")) {
            section = "intl";
            continue;
          }
          ...
          if (line.startsWith("*")) {
            const cleaned = line.replace(/^\*+\s*/, "").trim();
            if (!cleaned) continue;
            if (section === "ins") ins.push(cleaned);
            else if (section === "pay") pay.push(cleaned);
            continue;
          }
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => setFounderTeamInfo(parseFounderTeamInfo(text)))
      .catch(() => setFounderTeamInfo(null));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Facilities & Amenities.txt")
      .then((res) => res.text())
  ...
  useEffect(() => {
    fetch("/Center Images/HimVeda/Facilities/Facilities.txt")
      .then((res) => res.text())
      .then((text) => {
        const links = text
          .split("\n")
  ...
  }, [isAwardsMobile, isAwardAutoPlaying, maxAwardIndex]);
  ...
  const selectReview = (index: number) => {
    ...
    setTimeout(() => setIsReviewAutoPlaying(true), 4000);
  };
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Wellness Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("*")) {
            const cleaned = line.replace(/^\*+\s*/, "").trim();
            if (current && cleaned) current.bullets.push(cleaned);
            continue;
          }
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Why Choose HimVeda.txt")
      .then((res) => res.text())
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n");
        let title = "";
  ...
  useEffect(() => {
    fetch("/content/Top Centers/HimVeda/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              HIMVEDA HERITAGE WELLNESS CENTER HIMACHAL
            </li>
          </ol>
        </div>
      </nav>
      ...
      <div className="w-full px-3 md:px-4 py-12">
        <div className="w-full max-w-6xl mx-auto">
                ...
                <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                  {images.length > 0 && (
                    <img
                      ...
                      alt={`HimVeda Center ${selectedImage + 1}`}
          ...
          <Card className="mb-12" id="content">
            <CardContent className="px-4 md:px-8 py-6 md:py-8 prose md:prose-lg max-w-none prose-p:text-justify prose-p:leading-relaxed prose-p:text-base md:prose-p:text-lg prose-strong:text-primary">
              <MarkdownContent
                contentPath="/content/Top Centers/HimVeda/Main content.txt"
          ...
          <div className="mb-12 rounded-3xl px-4 py-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="wellness">
          ...
          <div className="mb-12 rounded-3xl px-4 py-8 md:p-12" style={{ backgroundColor: "#EDE8D0" }} id="medical">
            <div className="text-center mb-8">
          ...
          <div className="mb-12" id="videos">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">Video Gallery of HimVeda</h2>
              <p className="text-base md:text-lg mx-auto px-4 max-w-4xl" style={{ color: "#7F543D" }}>
                Experience the serene Himalayan setting and authentic healing atmosphere at HimVeda through our video gallery.
              </p>
            </div>
          ...
          <div className="mb-12" id="why-choose">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-bold text-primary mb-3">
                {whyChooseData?.title || "Why Choose HimVeda"}
              </h2>
          ...
          <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of HimVeda Center
              </h2>
            ...
            <div className="relative max-w-4xl mx-auto px-4 md:px-0">
              ...
              <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between px-2 md:-mx-8 pointer-events-none">
                <button
                  onClick={() =>
                    setSelectedTestimonialVideo((prev) =>
                      testimonialVideos.length === 0 ? 0 : (prev - 1 + testimonialVideos.length) % testimonialVideos.length
                    )
                  }
                  className="bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary pointer-events-auto"
                ...
                <button
                  onClick={() =>
                    setSelectedTestimonialVideo((prev) =>
                      testimonialVideos.length === 0 ? 0 : (prev + 1) % testimonialVideos.length
                    )
                  }
                  className="bg-white/90 hover:bg-primary hover:text-white text-primary p-2 md:p-4 rounded-full shadow-lg transition-all border-2 border-primary pointer-events-auto"
          ...
          <div className="mb-12" id="process">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg mx-auto px-4" style={{ color: "#7F543D" }}>
                {processIntro || "Your personalized healing journey at HimVeda, step by step"}
              </p>
            </div>
                      ...
                      <CardContent className="p-4 md:p-6">
                        ...
                        <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3 pl-10 md:pl-0">
                          ...
                          <div>
                            ...
                            <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">Step {s.stepNumber}</span>
                          </div>
                        </div>
                        ...
                        {s.bullets.length > 0 && (
                          <ul className="space-y-1.5 md:space-y-2">
                            {s.bullets.map((b, bi) => (
                              <li key={bi} className="flex items-start gap-2 text-xs md:text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                                <span className="text-primary mt-1">&#10003;</span>
                                <span>{b}</span>
                              </li>
                            ))}
          ...
          <div className="mb-12">
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#EDE8D0" }}>
              <div className="md:hidden">
                <div className="w-full bg-white/80 rounded-2xl p-4 shadow-lg border-2 border-primary/30">
                  <img
                    src="/Center Images/HimVeda/CTA mid.jpg"
                    alt="HimVeda Wellness Center"
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: "#7F543D" }}>
                    Every facility at HimVeda is designed and maintained to ensure high standards of safety, hygiene, and quality care,
                    supporting your healing journey in a serene, naturally therapeutic environment.
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    >
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/HimVeda/Founder/Founder.jpg"
                    ...
                    <div>
                      <h3 className="text-lg md:text-2xl font-bold text-primary mb-1 md:mb-2">{founderTeamInfo?.founderName || ""}</h3>
                ...
                <CardContent className="p-4 md:p-8">
                  <div className="flex items-start gap-3 md:gap-4 mb-4 md:mb-6">
                    ...
                    >
                      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-[2px] border-white bg-white">
                        <img
                          src="/Center Images/HimVeda/Founder/Team.jpg"
                    ...
                    <div>
                      <h3 className="text-lg md:text-2xl font-bold text-primary mb-1 md:mb-2">{founderTeamInfo?.teamTitle || ""}</h3>
          ...
          <div className="mb-12" id="awards">
            ...
            <div className="relative group max-w-5xl mx-auto">
              ...
              {isAwardsMobile && maxAwardIndex > 0 && (
                <>
                  <button
                    onClick={() => {
                      setIsAwardAutoPlaying(false);
                      goToPreviousAward();
                      setTimeout(() => setIsAwardAutoPlaying(true), 4000);
                    }}
                  ...
                  <div className="rounded-2xl bg-white/70 p-1 shadow-lg border-2 border-primary/20 overflow-hidden">
                    <div className="rounded-xl overflow-hidden">
                      <div className="relative w-full aspect-[800/600]">
                        <iframe
                          title="HimVeda Map"
                          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3376.269218240726!2d76.31989587472712!3d32.196975613404305!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x391b5100aef8d6fb%3A0xe09877aab2fcca5f!2sHimVeda!5e0!3m2!1sen!2sin!4v1770456757637!5m2!1sen!2sin"
          ...
          <div className="mb-12" id="bottom-cta">
            <div className="rounded-3xl p-6 md:p-10" style={{ backgroundColor: "#234A50" }}>
              <div className="md:hidden">
                <div className="w-full bg-black/30 rounded-2xl p-4 shadow-lg border-2 border-white/20">
                  <img
                    src="/Center Images/HimVeda/CTA bottom.jpg"
                    alt="HimVeda Wellness Center"
                    className="w-full h-auto rounded-xl mb-4 object-cover transition-transform duration-700 ease-out hover:scale-105"
                  />
                  <h2 className="text-xl font-extrabold text-white text-center mb-8 leading-tight tracking-tight">
                    Begin Your Holistic Healing Journey at HimVeda
                  </h2>
              ...
              <div className="hidden md:grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 className="text-2xl md:text-4xl font-extrabold text-white mb-10 leading-tight tracking-tight">
                    ...
                    <span className="text-white underline decoration-white/20 underline-offset-8">HimVeda</span>
                  </h2>
      ...
      {facilityLightboxOpen && facilityImages.length > 0 && (
        ...
        >
          ...
          <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">HimVeda Facilities & Amenities</div>
            <div className="relative rounded-lg overflow-hidden w-full" style={{ paddingBottom: "56.25%" }}>
              <img
                src={facilityImages[facilityLightboxImage]}
                alt={`HimVeda Facility ${facilityLightboxImage + 1}`}
                className="absolute inset-0 w-full h-full object-cover"
              />
              <button
                onClick={() => setFacilityLightboxOpen(false)}

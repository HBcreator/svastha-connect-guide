...
import { MapPin, Star, Calendar, ChevronLeft, ChevronRight, Images, Video, X, Users, TrendingUp, Heart, Droplet, Brain, Sparkles, ShieldCheck, HeartPulse, Activity, UserCheck, Stethoscope, Pill, Award, Leaf, Hospital, Home, Utensils, Globe, TreePine, Building2, ClipboardList, FileSearch, Phone, Mail, MessageCircle, MessageCircleHeart, Search } from "lucide-react";
...
function VideoThumbnail({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const onLoadedMetadata = () => {
        try {
          videoEl!.currentTime = Math.min(0.1, Math.max(0, (videoEl!.duration || 0) / 100));
        } catch {
          // ignore
        }
      };
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative aspect-video rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-105 hover:shadow-md ${active ? "ring-2 ring-primary" : ""}`}
      aria-label="Select video"
    >
      {thumb ? (
        <img src={thumb} alt="Video thumbnail" className="w-full h-full object-cover" />
      ) : (
        <video muted preload="metadata" playsInline className="w-full h-full object-cover">
...
export default function KrishnenduAyurvedaHospital() {
  ...
  useEffect(() => {
    fetch("/Center Images/Krishnendu Ayurveda Hospital/Photo Gallery/Photo Gallery Links.txt")
      ...
      .catch((err) => console.error("Error loading Krishnendu photo gallery links:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Videos/Krishnendu Ayurveda Hospital/yt i frame testimonies.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Krishnendu testimonials:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Frequently Asked Questions.txt")
      .then((res) => res.text())
      .then((text) => {
      ...
      .catch((err) => console.error("Error loading Krishnendu FAQs:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Contact Information.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Krishnendu contact info:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Videos/Krishnendu Ayurveda Hospital/video gallery links.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Krishnendu video gallery links:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Wellness & Rejuvenation Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Krishnendu wellness content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Medical Programs.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Krishnendu medical programs content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Why Choose Krishnendu.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Krishnendu why-choose content:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Treatment Process & Patient Journey.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            stepNo += 1;
            current = { number: stepNo, title: line.slice(2, -2), description: "", bullets: [] };
            inSection = true;
            continue;
          }
          if (!inSection) {
      ...
      .catch((err) => console.error("Error loading Krishnendu treatment process content:", err));
  }, []);

  useEffect(() => {
    fetch("/Center Images/Krishnendu Ayurveda Hospital/Facilities and Amenities/Facilities adn Amenities Links.txt")
      .then((res) => res.text())
      .then((text) => {
        const urls = text
          .split("\n")
      ...
      .catch((err) => console.error("Error loading Krishnendu facilities image links:", err));
  }, []);

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Facilities & Amenities.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Krishnendu facilities content:", err));
  }, []);
  ...
  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Founder & Team Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("**") && line.endsWith("**")) {
            ...
            if (/our\s+expert\s+healing\s+team/i.test(t)) {
          ...
          if (section === "founder") {
            if (!role && line.length <= 80) {
              role = line;
              continue;
            }
            if (/MD|BAMS|Ayurveda|MRCH|Hom/i.test(line) && line.length <= 80) {
              degrees.push(line);
              continue;
            }
      ...
      .catch((err) => console.error("Error loading Krishnendu founder & team content:", err));
  }, []);
  ...
  const iconForTitle = (t: string) => {
    ...
    if (s.includes("stress") || s.includes("sleep") || s.includes("calm")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("anti-aging") || s.includes("rejuvenation") || s.includes("radiance")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("weight")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("immunity") || s.includes("boost")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("beauty") || s.includes("skin")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("relax") || s.includes("pain") || s.includes("muscle")) return <UserCheck className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    if (s.includes("health") || s.includes("holistic")) return <ShieldCheck className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
    return <Heart className="h-4 w-4 md:h-5 md:w-5 text-green-600" />;
  };

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Patient Stories & Reviews.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
      ...
      .catch((err) => console.error("Error loading Krishnendu reviews:", err));
  }, []);
  ...
  const medicalIconForTitle = (t: string) => {
    ...
    if (s.includes("spine") || s.includes("back") || s.includes("disc") || s.includes("sciatica")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("knee") || s.includes("arthritis") || s.includes("joint") || s.includes("pain")) return <HeartPulse className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("neck") || s.includes("cervical")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("skin") || s.includes("psoriasis") || s.includes("eczema")) return <Sparkles className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("diabetes") || s.includes("cholesterol") || s.includes("hypertension") || s.includes("lifestyle")) return <Activity className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("stroke") || s.includes("paralysis") || s.includes("rehab")) return <Brain className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("women") || s.includes("pcod") || s.includes("pcos") || s.includes("infert")) return <Heart className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    if (s.includes("digest") || s.includes("ibs") || s.includes("gerd") || s.includes("gut")) return <Pill className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
    return <Stethoscope className="h-4 w-4 md:h-5 md:w-5 text-blue-600" />;
  };

  useEffect(() => {
    fetch("/content/Top Centers/Krishnendu Ayurveda Hospital/Insurance & Payment Info.txt")
      .then((res) => res.text())
      .then((text) => {
        const lines = text.split("\n").map((l) => l.trim());
        let intro = "";
      ...
      .catch((err) => console.error("Error loading Krishnendu insurance content:", err));
  }, []);

  const whyIconForTitle = (t: string) => {
    const s = t.toLowerCase();
    const cls = "h-6 w-6 text-primary group-hover:text-white transition-colors";
    if (s.includes("heritage") || s.includes("century") || s.includes("legacy") || s.includes("generations")) return <Award className={cls} />;
    if (s.includes("accredited") || s.includes("nabh") || s.includes("quality") || s.includes("safety")) return <ShieldCheck className={cls} />;
    if (s.includes("unique") || s.includes("personal") || s.includes("plan")) return <ClipboardList className={cls} />;
    if (s.includes("ecosystem") || s.includes("integrated") || s.includes("yoga") || s.includes("meditation")) return <HeartPulse className={cls} />;
    if (s.includes("experts") || s.includes("team") || s.includes("physician") || s.includes("doctor")) return <Hospital className={cls} />;
    if (s.includes("sanctuary") || s.includes("serene") || s.includes("natural") || s.includes("village") || s.includes("paddy")) return <TreePine className={cls} />;
    if (s.includes("facilities") || s.includes("rooms") || s.includes("cottages") || s.includes("pool") || s.includes("gym")) return <Building2 className={cls} />;
    if (s.includes("medicines") || s.includes("pharmacy") || s.includes("in-house") || s.includes("fresh")) return <Sparkles className={cls} />;
    if (s.includes("results") || s.includes("track record") || s.includes("success")) return <TrendingUp className={cls} />;
    if (s.includes("journey") || s.includes("home") || s.includes("follow") || s.includes("support")) return <Home className={cls} />;
    if (s.includes("diet") || s.includes("nutrition") || s.includes("meals")) return <Utensils className={cls} />;
    if (s.includes("world") || s.includes("global") || s.includes("worldwide") || s.includes("ship")) return <Globe className={cls} />;
    if (s.includes("location") || s.includes("map") || s.includes("near")) return <MapPin className={cls} />;
  ...
  const treatmentIconForTitle = (t: string) => {
    ...
    if (s.includes("blueprint") || s.includes("protocol") || s.includes("roadmap") || s.includes("plan")) return <FileSearch className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("daily") || s.includes("therap")) return <Pill className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("cuisine") || s.includes("meals") || s.includes("diet") || s.includes("nutrition")) return <Utensils className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("integration") || s.includes("mind") || s.includes("yoga") || s.includes("meditation") || s.includes("physio")) return <HeartPulse className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
    if (s.includes("home") || s.includes("journey") || s.includes("empower")) return <Home className="h-5 w-5 md:h-6 md:w-6 text-primary" />;
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <ol className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] overflow-x-auto whitespace-nowrap pb-1 -mb-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <li className="text-primary/90 font-black shrink-0">
              KRISHNENDU AYURVEDA HOSPITAL KERALA
            </li>
          </ol>
        </div>
      </nav>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-lg">Alappuzha, Kerala, India</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-lg font-semibold">4.9</span>
                  <span className="opacity-90">(1,500+ reviews)</span>
                </div>
              </div>
      ...
      <div className="container mx-auto px-3 md:px-4 max-w-full">
        <div id="gallery" className="max-w-6xl mx-auto mt-10">
                ...
                <>
                  <div className="relative mb-8 rounded-lg overflow-hidden shadow-lg w-full h-[200px] md:h-[500px] lg:h-[400px] group">
                    <img
                      ...
                      alt={`Krishnendu Center ${selectedImage + 1}`}
                  ...
                  <div className="container mx-auto px-4 py-10" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {images.map((img, i) => (
                        ...
                        >
                          <img src={img} alt={`Krishnendu ${i + 1}`} className="absolute inset-0 w-full h-full object-cover rounded-lg" />
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {lightboxOpen && images.length > 0 && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#EDE8D0]/80 backdrop-blur-sm">
                  ...
                  <div className="bg-background/90 rounded-xl shadow-2xl p-4 w-full max-w-5xl">
                    <div className="text-center text-primary text-2xl font-bold mb-3 leading-relaxed">Krishnendu Ayurveda Hospital</div>
                    <div className="relative rounded-lg overflow-hidden shadow-lg w-full" style={{ paddingBottom: "56.25%" }}>
                      <img src={images[lightboxImage]} alt={`Krishnendu ${lightboxImage + 1}`} className="absolute inset-0 w-full h-full object-cover" />
                      <button
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {videos.map((video, idx) => (
                      <VideoThumbnail key={idx} src={video} active={selectedVideo === idx} onClick={() => setSelectedVideo(idx)} />
                    ))}
                  </div>
                </>
              )}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {whyItems.length > 0 && (
            <div id="why-choose" className="mb-12">
              <div className="text-center mb-10">
                <h2 className="text-xl md:text-4xl font-bold text-primary mb-3">Why Choose Krishnendu Ayurveda Hospital for Your Healing Journey</h2>
          ...
          {/* Testimonials of Krishnendu Ayurveda Hospital - Video Section */}
          <div className="mb-12" id="testimonial-videos" ref={testimonialSectionRef}>
            <div className="text-center mb-8 md:mb-10 px-4">
              <h2 className="text-xl md:text-4xl font-extrabold text-primary mb-2 leading-tight tracking-tight">
                Testimonials of Krishnendu Ayurveda Hospital
              </h2>
                  ...
                  <div className="aspect-video w-full relative">
                    {testimonialVideos.length > 0 && (
                      <iframe
                        ...
                        title="Krishnendu Testimonial Video"
